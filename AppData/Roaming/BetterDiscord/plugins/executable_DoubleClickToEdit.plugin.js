...
const React = BdApi.React,
  ...
  walkable = ["child", "memoizedProps", "sibling"];
...
module.exports = class DoubleClickToEdit {
  ...
  start() {
    try {
      ...
      this.messagesWrapper = Webpack.getModule(
        Filters.byKeys("empty", "messagesWrapper"),
      ).messagesWrapper;
      ...
      //Reply functions
      this.replyToMessage = Webpack.getModule(
        (m) =>
          m
            ?.toString?.()
            ?.replace("\n", "")
            ?.search(/(channel:e,message:n,shouldMention:!)/) > -1,
        { searchExports: true },
      );
      this.getChannel = Webpack.getModule(
        Filters.byKeys("getChannel", "getDMFromUserId"),
      ).getChannel;
      ...
      //Settings
      this.FormSwitch = Webpack.getModule(Filters.byStrings("labelRow", "checked"), {
        searchExports: true,
      });
      this.RadioGroup = Webpack.getModule((m) => Filters.byKeys("NOT_SET", "NONE")(m?.Sizes), {
        searchExports: true,
      });
      this.FormItem = Webpack.getModule(
        (m) => Filters.byStrings("titleId", "errorId", "setIsFocused")(m?.render),
        { searchExports: true },
      );
      ...
      //Load settings
      //Edit
      this.doubleClickToEditModifier =
        Data.load(config.info.slug, "doubleClickToEditModifier") ?? false;
      ...
      this.doubleClickToReplyModifier =
        Data.load(config.info.slug, "doubleClickToReplyModifier") ?? false;
  ...
  getSettingsPanel() {
    ...
    return () => {
      //Edit
      const [editEnableModifier, setEditEnableModifier] = React.useState(
          this.doubleClickToEditModifier,
        ),
        [editModifier, setEditModifier] = React.useState(this.editModifier),
        //Reply
        [reply, setReply] = React.useState(this.doubleClickToReply),
        [replyEnableModifier, setReplyEnableModifier] = React.useState(
          this.doubleClickToReplyModifier,
        ),
      ...
      return [
        ...
        React.createElement(
          this.FormItem,
          { disabled: !editEnableModifier, title: "Modifer to hold to edit a message" },
        ...
        React.createElement(
          this.FormItem,
          { disabled: !copy, title: "Modifier to hold before copying text" },
          React.createElement(this.RadioGroup, {
            disabled: !copy,
            value: copyModifier,
            options: [
  ...
  handler(e) {
    ...
    if (
      typeof e?.target?.className !== typeof "" ||
    ...
    if (messageDiv.classList.contains(this.selectedClass)) return;
    ...
    if (copyKeyHeld) this.copyToClipboard(document.getSelection().toString());
    ...
    const message =
      Utils.findInTree(instance, (m) => m?.baseMessage, { walkable: walkable })?.baseMessage ??
    ...
    if (
      (this.doubleClickToEditModifier ? editKeyHeld : true) &&
      message.author.id === this.CurrentUserStore.getCurrentUser().id
    )
