...
var Config_default = {
  info: {
    ...
    description: "All in one better spotify-discord experience.",
    source:
      "https://raw.githubusercontent.com/Skamt/BDAddons/main/SpotifyEnhance/SpotifyEnhance.plugin.js",
...
var getInternalInstance = /* @__PURE__ */ (() =>
  Api.ReactUtils.getInternalInstance.bind(Api.ReactUtils))();

// common/Utils/Logger.js
Logger.patchError = (patchId) => {
  console.error(
    `%c[${Config_default.info.name}] %cCould not find module for %c[${patchId}]`,
    "color: #3a71c1;font-weight: bold;",
    "",
...
var Events = { START: "START", STOP: "STOP" };
...
var { zustand } = getMangled(
  Filters.bySource("useSyncExternalStoreWithSelector", "useDebugValue", "subscribe"),
  { _: Filters.byStrings("subscribe"), zustand: () => true },
);
var subscribeWithSelector = getModule(
  Filters.byStrings("getState", "equalityFn", "fireImmediately"),
  { searchExports: true },
);
...
function create(initialState) {
  ...
  Object.defineProperty(Store2, "state", { configurable: false, get: () => Store2.getState() });
  return Store2;
}
...
function fit({ width, height, gap = 0.8 }) {
  ...
  return { width, height, maxHeight: height * gap, maxWidth: width * gap };
}
...
var MessageActions_default = getModule(Filters.byKeys("jumpToMessage", "_sendMessage"), {
  searchExports: false,
});

// MODULES-AUTO-LOADER:@Modules/Dispatcher
var Dispatcher_default = getModule(Filters.byKeys("dispatch", "_dispatch"), {
  searchExports: true,
});
...
function sendMessageDirectly(content, channelId) {
  if (
    !MessageActions_default?.sendMessage ||
    typeof MessageActions_default.sendMessage !== "function"
  )
    return;
  if (!channelId) channelId = SelectedChannelStore_default.getChannelId();
  return MessageActions_default.sendMessage(
    channelId,
    { validNonShortcutEmojis: [], content },
    void 0,
    getReply(channelId),
  );
}
  ...
  return (content) => {
    if (!ComponentDispatch)
      ComponentDispatch = getModule(
        (m) => m.dispatchToLastSubscribed && m.emitter.listeners("INSERT_TEXT").length,
        { searchExports: true },
      );
    if (!ComponentDispatch) return;
    setTimeout(() =>
      ComponentDispatch.dispatchToLastSubscribed("INSERT_TEXT", { plainText: content }),
    );
  };
})();
...
var Timer = class _Timer {
  ...
  start() {
    ...
    this.running = true;
    this.timerId = this.counter.call(
      window,
      () => {
...
async function wrappedFetch(url, options) {
  ...
  if (!response.ok) {
    ...
    throw result?.error || { message: "Unknown error", status: response.status };
  }
...
function buildFetchRequestOptions(builderObj) {
  const options = { method: builderObj.method, headers: builderObj.headers };
...
var RefreshToken_default = getModule(Filters.byStrings("CONNECTION_ACCESS_TOKEN"), {
  searchExports: true,
});
...
var activityPanelClasses = getModule(Filters.byKeys("activityPanel", "panels"), {
  searchExports: false,
});
var getFluxContainer = /* @__PURE__ */ (() => {
  ...
  function tryGetFluxContainer() {
    ...
    if (!instance) return;
    const res = findInTree(instance, (a) => a?.type?.prototype?.hasParty, {
      walkable: ["child", "sibling"],
    });
    if (!res) return;
    return res;
  }
...
var parsers = {
  ...
  playlist(obj) {
    return {
      ...
      owner: { name: obj.owner.display_name, id: obj.owner.id },
    };
  },
  album(obj) {
    return {
...
async function _requestHandler(action) {
  ...
  do {
    ...
    if (!SpotifyAPI_default.accountId)
      throw new Error("Can't refresh expired access token Unknown account ID");
    const [tokenRefreshError, tokenRefreshResponse] = await promiseHandler(
      RefreshToken_default(SpotifyAPI_default.accountId),
    );
...
${reason}`,
  ...
  return (type, id, description) =>
    requestHandler(() => SpotifyAPI_default[prop](type, id))
      .then(() => {
...
async function fetchRessource(type, id) {
  const [err, data] = await promiseHandler(
    requestHandler(() => SpotifyAPI_default.getRessource(type, id)),
  );
  ...
  {},
  {
    get(_, prop) {
      switch (prop) {
        ...
        case "volume":
          return (...args) =>
            requestHandler(() => SpotifyAPI_default[prop].apply(SpotifyAPI_default, args)).catch(
              (reason) => {
    ...
    subscribeWithSelector((set, get) => {
      return {
        ...
        async fetchPlayerState() {
          const [err, playerState] = await promiseHandler(
            SpotifyAPIWrapper_default.getPlayerState(),
          );
        ...
        getAlbum() {
          ...
          return { ...media.album, url: media.album.external_urls.spotify };
        },
        getFullSongName() {
          const state = get();
  ...
  ),
  {
    init() {
      ...
      this.idleTimer = new Timer(
        () => Store.state.setDeviceState(false),
        5 * 60 * 1e3,
        Timer.TIMEOUT,
      );
...
Object.defineProperty(Store, "state", { configurable: false, get: () => Store.getState() });
...
function path(props, d) {
  return /* @__PURE__ */ React_default.createElement("path", { ...props, d });
}
var AddToQueueIcon = /* @__PURE__ */ svg(
  { viewBox: "-1 -1 18 18" },
  "M16 15H2v-1.5h14V15zm0-4.5H2V9h14v1.5zm-8.034-6A5.484 5.484 0 0 1 7.187 6H13.5a2.5 2.5 0 0 0 0-5H7.966c.159.474.255.978.278 1.5H13.5a1 1 0 1 1 0 2H7.966zM2 2V0h1.5v2h2v1.5h-2v2H2v-2H0V2h2z",
);
var ArrowIcon = /* @__PURE__ */ svg(
  null,
...
var ExternalLinkIcon = /* @__PURE__ */ svg(
  { viewBox: "0 0 16 16" },
  "M1 2.75A.75.75 0 0 1 1.75 2H7v1.5H2.5v11h10.219V9h1.5v6.25a.75.75 0 0 1-.75.75H1.75a.75.75 0 0 1-.75-.75V2.75z",
  "M15 1v4.993a.75.75 0 1 1-1.5 0V3.56L8.78 8.28a.75.75 0 0 1-1.06-1.06l4.72-4.72h-2.433a.75.75 0 0 1 0-1.5H15z",
);
var ImageIcon = /* @__PURE__ */ svg(
  { viewBox: "-50 -50 484 484" },
  "M341.333,0H42.667C19.093,0,0,19.093,0,42.667v298.667C0,364.907,19.093,384,42.667,384h298.667 C364.907,384,384,364.907,384,341.333V42.667C384,19.093,364.907,0,341.333,0z M42.667,320l74.667-96l53.333,64.107L245.333,192l96,128H42.667z",
);
var ListenAlongIcon = /* @__PURE__ */ svg(
  null,
  "M11.8 14a6.1 6.1 0 0 0 0 6H3v-2c0-2.7 5.3-4 8-4h.8zm-.8-2c-2.2 0-4-1.8-4-4s1.8-4 4-4 4 1.8 4 4-1.8 4-4 4zm6 1c2.2 0 4 1.8 4 4s-1.8 4-4 4-4-1.8-4-4 1.8-4 4-4zm-1 6.2l3-2.2-3-2.2v4.4z",
);
var ListenIcon = /* @__PURE__ */ svg(
  null,
  "M22 16.53C22 18.3282 20.2485 19.7837 18.089 19.7837C15.9285 19.7837 14.5396 18.3277 14.5396 16.53C14.5396 14.7319 15.9286 13.2746 18.089 13.2746C18.7169 13.2746 19.3089 13.4013 19.8353 13.6205V5.814L9.46075 7.32352V18.7449C9.46075 20.5424 7.70957 22 5.54941 22C3.38871 22 2 20.5443 2 18.7456C2 16.9481 3.3892 15.4898 5.54941 15.4898C6.17823 15.4898 6.76966 15.6162 7.29604 15.836C7.29604 11.3608 7.29604 8.5366 7.29604 4.1395L21.9996 2L22 16.53Z",
);
var MuteVolumeIcon = /* @__PURE__ */ svg(
  { viewBox: "0 0 16 16" },
...
var MessageComponentAccessories = getModule(Filters.byPrototypeKeys("renderPoll"), {
  searchExports: true,
});
var urlRegex = /((?:https?|steam):\/\/[^\s<]+[^<.,:;"'\]\s])/g;
var MessageStateContext = React.createContext(null);
Plugin_default.on(Events.START, () => {
  if (!MessageComponentAccessories)
    return Logger_default.patchError("patchMessageComponentAccessories");
  const unpatches = [
...
var ErrorBoundary = class extends React_default.Component {
  ...
  componentDidCatch(error, info) {
    ...
    console.error(
      `%c[${Config_default?.info?.name || "Unknown Plugin"}] %cthrew an exception at %c[${this.props.id}]
`,
      "color: #3a71c1;font-weight: bold;",
      "",
      "color: red;font-weight: bold;",
  ...
  renderErrorBoundary() {
    return /* @__PURE__ */ React_default.createElement(
      "div",
      { style: { background: "#292c2c", padding: "20px", borderRadius: "10px" } },
      /* @__PURE__ */ React_default.createElement(
        "b",
        { style: { color: "#e0e1e5" } },
        "An error has occured while rendering ",
        /* @__PURE__ */ React_default.createElement(
          "span",
          { style: { color: "orange" } },
          this.props.id,
  ...
  renderFallback() {
    ...
    return /* @__PURE__ */ React_default.createElement(this.props.fallback, {
      id: this.props.id,
      plugin: Config_default?.info?.name || "Unknown Plugin",
    });
  }
...
var MessageHeader_default =
  getModuleAndKey(Filters.byStrings("userOverride", "withMentionPrefix"), {
    searchExports: false,
  }) || {};

// MODULES-AUTO-LOADER:@Modules/useStateFromStores
var useStateFromStores_default = getModule(Filters.byStrings("getStateFromStores"), {
  searchExports: true,
});
...
var Tooltip_default2 = ({ note, position, children }) => {
  return /* @__PURE__ */ React.createElement(
    Tooltip_default,
    { text: note, position: position || "top" },
    (props) => React.cloneElement(children, { ...props, ...children.props }),
  );
};
...
function SpotifyActivityIndicator({ userId }) {
  ...
  const spotifyActivity = useStateFromStores_default([PresenceStore_default], () =>
  ...
  return /* @__PURE__ */ React.createElement(
    Tooltip_default2,
    { note: `${spotifyActivity.details} - ${spotifyActivity.state}` },
    /* @__PURE__ */ React.createElement(SpotifyIcon, {
      width: "20",
      height: "20",
  ...
  const unpatch = Patcher.after(module2, key, (_, [{ message }], ret) => {
    ...
    ret.props.children.push(
      ...
      React.createElement(
        ...
        /* @__PURE__ */ React.createElement(SpotifyActivityIndicator, { userId }),
      ),
    );
  });
  Plugin_default.once(Events.STOP, unpatch);
...
var ManaButton =
  /* @__PURE__ */ getModule(Filters.byStrings(`"data-mana-component":"button"`), {
    searchExports: true,
  }) || ButtonComponentFallback;
var ManaTextButton =
  /* @__PURE__ */ getModule(Filters.byStrings(`"data-mana-component":"text-button"`), {
    searchExports: true,
  }) || ButtonComponentFallback;
...
function ListenAlong({ userSyncActivityState }) {
  ...
  return /* @__PURE__ */ React_default.createElement(
    Tooltip_default2,
    { note: tooltip },
    /* @__PURE__ */ React_default.createElement(ActivityControlButton, {
...
var SpotifyActivityControls_default = ({ activity, user }) => {
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className: "spotify-activity-controls" },
    /* @__PURE__ */ React_default.createElement(Play, { userPlayActivityState }),
    /* @__PURE__ */ React_default.createElement(
      Tooltip_default2,
      { note: "Add to queue" },
      /* @__PURE__ */ React_default.createElement(ActivityControlButton, {
...
var ActivityComponent = getModuleAndKey(
...
Plugin_default.on(Events.START, () => {
  ...
  const unpatch = Patcher.after(module2, key, (_, [{ user, activity }]) => {
    ...
    return /* @__PURE__ */ React.createElement(
      ErrorBoundary,
      { id: "SpotifyEmbed" },
      /* @__PURE__ */ React.createElement(SpotifyActivityControls_default, { user, activity }),
    );
  });
  Plugin_default.once(Events.STOP, unpatch);
});
...
var EmbedStyleEnum = { KEEP: "KEEP", REPLACE: "REPLACE", HIDE: "HIDE" };
...
var SpotifyControls_default = ({ id, type, embed: { thumbnail, rawTitle, url } }) => {
  ...
  const listenBtn =
    type !== "show" &&
    /* @__PURE__ */ React_default.createElement(ControlBtn, {
      disabled: !isActive,
...
var RenderLinkComponent = getModule((m) => m.type?.toString?.().includes("MASKED_LINK"), {
  searchExports: false,
});
var ImageModal = getModule(reactRefMemoFilter("type", "renderLinkComponent"), {
  searchExports: true,
});
...
var useSomeScalingHook = getModule(
  Filters.byStrings("reducedMotion.enabled", "useSpring", "respect-motion-settings"),
  { searchExports: true },
);
  ...
  return /* @__PURE__ */ React_default.createElement(
    ...
    /* @__PURE__ */ React_default.createElement(
      ...
      /* @__PURE__ */ React_default.createElement(ImageModal, {
        ...
        media: { ...rest, type: "IMAGE", url, proxyUrl: url },
      }),
      !x &&
        /* @__PURE__ */ React_default.createElement(
          "div",
          { className: "imageModalOptions" },
          /* @__PURE__ */ React_default.createElement(
            RenderLinkComponent,
            { className: "downloadLink", href: url },
            "Open in Browser",
          ),
        ),
    ),
...
var Modals = /* @__PURE__ */ getMangled(
  /* @__PURE__ */ Filters.bySource("MODAL_ROOT", "transitionState"),
  {
...
var openModal = (children, tag, { className, ...modalRootProps } = {}) => {
  ...
  return ModalActions.openModal((props) => {
    return /* @__PURE__ */ React.createElement(
      ErrorBoundary,
      { id, plugin: Config_default.info.name },
      /* @__PURE__ */
      React.createElement(
        Modals.ModalRoot,
        {
...
var PreviewPlayer_default = class extends React_default.Component {
  ...
  render() {
    ...
    return /* @__PURE__ */ React_default.createElement(
      ...
      /* @__PURE__ */ React_default.createElement(
        "div",
        { onClick: playPauseHandler, className: playPauseClassName },
        playPauseIcon,
      ),
    );
  }
...
var TrackTimeLine_default = () => {
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className: "spotify-player-timeline" },
    /* @__PURE__ */ React_default.createElement(Slider_default, {
    ...
    /* @__PURE__ */ React_default.createElement(
      "div",
      { className: "spotify-player-timeline-progress" },
      formatMsToTime(position),
    ),
    /* @__PURE__ */ React_default.createElement(Duration, { duration, position }),
  );
};
...
function Duration({ duration, position }) {
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { onClick: clickHandler, className: "spotify-player-timeline-duration" },
...
var SpotifyEmbed_default = ({ id, type }) => {
  ...
  const banner = { bannerSm: thumbnail?.[2], bannerMd: thumbnail?.[1], bannerLg: thumbnail?.[0] };
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className, style: bannerStyleObj },
    /* @__PURE__ */
    React_default.createElement(
      Tooltip_default2,
      { note: "View" },
      /* @__PURE__ */ React_default.createElement("div", {
        onClick: () => {
          ...
          openModal(
            ...
            React_default.createElement(
              "div",
              { className: "spotify-banner-modal" },
              /* @__PURE__ */ React_default.createElement(ImageComponent, {
                url: url2,
                ...fit(rest),
              }),
            ),
    ...
    React_default.createElement(
      ...
      /* @__PURE__ */ React_default.createElement(
        "p",
        { className: "spotify-embed-description" },
        rawDescription,
      ),
    ),
    type &&
      ...
      /* @__PURE__ */ React_default.createElement(
        "div",
        { className: "spotify-embed-controls" },
        ((isThis && isActive && !isPlaying) || (!isThis && isActive)) && [listenBtn, queueBtn],
        isThis &&
          isActive &&
          isPlaying &&
          /* @__PURE__ */ React_default.createElement(TrackTimeLine_default, null),
        /* @__PURE__ */ React_default.createElement(
          Tooltip_default2,
          { note: "Copy link" },
          /* @__PURE__ */ React_default.createElement(
        ...
        preview_url &&
          /* @__PURE__ */ React_default.createElement(PreviewPlayer_default, { src: preview_url }),
      ),
    /* @__PURE__ */
    React_default.createElement(
      Tooltip_default2,
      ...
      /* @__PURE__ */ React_default.createElement(
        "div",
        { onClick: () => Store.Utils.openSpotifyLink(url), className: "spotify-embed-spotifyIcon" },
        /* @__PURE__ */
  ...
  switch (spotifyEmbed) {
    case EmbedStyleEnum.KEEP:
      return [
        ...
        /* @__PURE__ */
        React_default.createElement(SpotifyControls_default, { id, type, embed: embedObject }),
      ];
    ...
    case EmbedStyleEnum.HIDE:
      return /* @__PURE__ */ React_default.createElement(SpotifyControls_default, {
        id,
        type,
        embed: embedObject,
      });
...
var DiscordPopout = /* @__PURE__ */ (() =>
  getModule((a) => a?.prototype?.render && a.Animation, { searchExports: true }))();
var ChannelComponent = getModule(Filters.byComponentType(Filters.byStrings("hasActiveThreads")), {
  searchExports: true,
});
var Anchor = /* @__PURE__ */ (() => getModule(Filters.byKeys("Anchor")).Anchor)();
var RadioGroup = /* @__PURE__ */ (() =>
  getMangled('data-toggleable-component":"radiogroup', {
    radioGroup: Filters.byStrings("label", "required"),
  }).radioGroup)();
...
var SpotifyPlayerControls_default = () => {
  ...
  const [isPlaying, shuffle, repeat, volume] = Store(
    (_) => [_.isPlaying, _.shuffle, _.repeat, _.volume],
    shallow,
  );
  ...
  return /* @__PURE__ */ React_default.createElement(
        ...
        HoverPopout,
        {
          popout: (e) =>
            /* @__PURE__ */ React_default.createElement(
              ...
              ContextMenu.buildMenuChildren(shareMenu),
            ),
        },
        /* @__PURE__ */ React_default.createElement(SpotifyPlayerButton, {
    ...
    [
      ...
      .filter(Boolean)
      .map(SpotifyPlayerButton),
    playerButtons[PlayerButtonsEnum.VOLUME] &&
      /* @__PURE__ */ React_default.createElement(Volume, { volume }),
  );
};
  ...
  return /* @__PURE__ */ React_default.createElement(
    HoverPopout,
    {
      popout: () =>
        /* @__PURE__ */ React_default.createElement(
          "div",
          { className: "spotify-player-controls-volume-slider-wrapper" },
          ...
          /* @__PURE__ */ React_default.createElement(
            "div",
            { className: "spotify-player-controls-volume-label" },
            val,
          ),
        ),
      position: "top",
    ...
    /* @__PURE__ */
    React_default.createElement(SpotifyPlayerButton, {
      className: "spotify-player-controls-volume",
      onClick: volumeMuteHandler,
      value: val
        ? /* @__PURE__ */ React_default.createElement(VolumeIcon, null)
        : /* @__PURE__ */ React_default.createElement(MuteVolumeIcon, null),
    }),
  );
}
...
function Artist({ artists }) {
  ...
  return /* @__PURE__ */ React_default.createElement(
    ...
    /* @__PURE__ */ React_default.createElement(
      "div",
      { className: "spotify-player-artist ellipsis" },
      `on ${artists[0].name}`,
    ),
  );
}
  ...
  const thumbnailClickHandler = () => {
    ...
    openModal(
      ...
      React_default.createElement(
        "div",
        { className: "spotify-banner-modal" },
        /* @__PURE__ */ React_default.createElement(ImageComponent, { url, ...fit(rest) }),
      ),
    );
  };
  return /* @__PURE__ */ React_default.createElement(
...
var TrackMediaDetails_default = ({ name, artists, mediaType }) => {
  if (mediaType !== "track") {
    return /* @__PURE__ */ React_default.createElement(
      "div",
      { className: "spotify-player-media" },
      /* @__PURE__ */ React_default.createElement(
        "div",
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className: "spotify-player-media" },
    /* @__PURE__ */ React_default.createElement(TrackBanner, null),
    /* @__PURE__ */ React_default.createElement(
      Tooltip_default2,
      { note: name },
      /* @__PURE__ */ React_default.createElement(
        Anchor,
        { href: songUrl, className: "spotify-player-title ellipsis" },
        name,
      ),
    ),
    /* @__PURE__ */ React_default.createElement(Artist, { artists }),
    /* @__PURE__ */ React_default.createElement(
      HoverPopout,
      {
        popout: (e) =>
      ...
      React_default.createElement(
        "div",
        { className: "spotify-player-album ellipsis" },
        `on ${albumName}`,
      ),
    ),
  );
...
var SpotifyPlayer_default = React_default.memo(function SpotifyPlayer() {
  ...
  const [player, playerBannerBackground] = Settings_default(
    (_) => [_.player, _.playerBannerBackground],
    shallow,
  );
  const [playerCompactMode, setplayerCompactMode] =
    Settings_default.useSetting("playerCompactMode");
  ...
  return /* @__PURE__ */ React_default.createElement(
    ...
    /* @__PURE__ */
    React_default.createElement(TrackMediaDetails_default, {
      mediaType,
      name: media?.name,
    ...
    React_default.createElement(
      Tooltip_default2,
      { note: playerCompactMode ? "Maximize" : "Minimize" },
      /* @__PURE__ */ React_default.createElement(
        "div",
        { onClick: minmaxClickHandler, className: "spotify-player-minmax" },
        /* @__PURE__ */
...
Plugin_default.on(Events.START, async () => {
  ...
  const unpatch = Patcher.after(fluxContainer.type.prototype, "render", (_, __, ret) => {
    return [
      ...
      React.createElement(
        ErrorBoundary,
        { key: "SpotifyPlayer", id: "SpotifyPlayer" },
        /* @__PURE__ */
...
Plugin_default.on(Events.START, async () => {
  ...
  const unpatch = Patcher.after(
    socket.prototype,
    "handleEvent",
    function onSocketEvent(socket2, [socketEvent]) {
      if (Store.state.account?.accountId && socket2.accountId !== Store.state.account?.accountId)
        return;
...
var Heading_default = getModule(
  (a) => a?.render?.toString().includes("data-excessive-heading-level"),
  { searchExports: true },
);
...
function Collapsible({ title, children }) {
  ...
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className: c("container", { open }) },
    /* @__PURE__ */ React_default.createElement(
      "div",
      { className: c("header"), onClick: () => setOpen(!open) },
      /* @__PURE__ */
      React_default.createElement(Heading_default, { className: c("title"), tag: "h5" }, title),
      /* @__PURE__ */
      React_default.createElement(
        "div",
        { className: c("icon") },
        /* @__PURE__ */ React_default.createElement(ArrowIcon, null),
      ),
    ),
...
Gap.direction = { HORIZONTAL: "horizontal", VERTICAL: "vertical" };
...
function FieldSet({ label, description, children, contentGap = 16 }) {
  return /* @__PURE__ */ React_default.createElement(
    ...
    label &&
      /* @__PURE__ */ React_default.createElement(
        Heading_default,
        { className: c3("label"), tag: "legend", variant: "text-lg/medium" },
        label,
      ),
    description &&
      /* @__PURE__ */ React_default.createElement(
        Heading_default,
        { className: c3("description"), variant: "text-sm/normal", color: "text-secondary" },
        description,
      ),
    /* @__PURE__ */ React_default.createElement(
      "div",
      { className: c3("content"), style: { gap: contentGap } },
      children,
    ),
  );
}
...
var FormSwitch_default = getModule(Filters.byStrings("note", "tooltipNote"), {
  searchExports: true,
});

// common/Components/Switch/index.jsx
var Switch_default =
  getModule(Filters.byStrings('"data-toggleable-component":"switch"', 'layout:"horizontal"'), {
    searchExports: true,
  }) ||
...
Divider.direction = { HORIZONTAL: "horizontal", VERTICAL: "vertical" };
...
function SettingSwtich({ settingKey, note, border = false, onChange = nop, description, ...rest }) {
  ...
  return /* @__PURE__ */ React.createElement(
    React.Fragment,
    null,
    /* @__PURE__ */ React.createElement(Switch_default, {
      ...rest,
      checked: val,
...
function SpotifyEmbedOptions() {
  ...
  return /* @__PURE__ */ React_default.createElement(RadioGroup, {
    options: [
      { value: EmbedStyleEnum.KEEP, name: "Keep: Use original Spotify Embed" },
      { value: EmbedStyleEnum.REPLACE, name: "Replace: A less laggy Spotify Embed" },
      { value: EmbedStyleEnum.HIDE, name: "Hide: Completely remove spotify embed" },
    ],
...
function SettingComponent() {
  return /* @__PURE__ */ React_default.createElement(
    "div",
    { className: `${Config_default.info.name}-settings` },
    /* @__PURE__ */ React_default.createElement(
      Collapsible,
      { title: "miscellaneous" },
      /* @__PURE__ */ React_default.createElement(
        ...
        { contentGap: 8 },
        [
          { settingKey: "player", description: "Enable/Disable player." },
          {
            settingKey: "enableListenAlong",
            description: "Enables/Disable listen along without premium.",
          },
          { settingKey: "activity", description: "Modify Spotify activity." },
          { settingKey: "activityIndicator", description: "Show user's Spotify activity in chat." },
          { settingKey: "playerCompactMode", description: "Player compact mode" },
          {
    ...
    /* @__PURE__ */ React_default.createElement(Gap, { gap: 15 }),
    /* @__PURE__ */ React_default.createElement(
      Collapsible,
    ...
    /* @__PURE__ */ React_default.createElement(
      Collapsible,
      { title: "Spotify embed style" },
      /* @__PURE__ */ React_default.createElement(SpotifyEmbedOptions, null),
    ),
    /* @__PURE__ */ React_default.createElement(Gap, { gap: 15 }),
  );
}
...
Plugin_default.getSettingsPanel = () =>
  /* @__PURE__ */ React_default.createElement(SettingComponent, null);
