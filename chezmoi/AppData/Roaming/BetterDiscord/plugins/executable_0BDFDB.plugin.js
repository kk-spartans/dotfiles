...
module.exports = ((_) => {
  if (
    window.BDFDB_Global &&
    window.BDFDB_Global.PluginUtils &&
    typeof window.BDFDB_Global.PluginUtils.cleanUp == "function"
  )
  ...
  return class BDFDB_Frame {
    ...
    load() {
      ...
      Internal = Object.assign({}, BDFDB, {
        ...
        defaults: {
          general: {
            shareData: { value: true, onChange: (_) => (Cache.data = {}) },
            showToasts: {
              value: true,
              isDisabled: (data) => data.nativeValue,
              hasNote: (data) => data.disabled && data.value,
            },
            checkForUpdates: { value: false, hasNote: true },
            showSupportBadges: { value: false },
            useChromium: {
              value: false,
              isHidden: (data) =>
                !Internal.LibraryRequires.electron || !Internal.LibraryRequires.electron.remote,
              getValue: (data) => !data.disabled,
            },
          },
          choices: { toastPosition: { value: "right", items: "ToastPositions" } },
        },
      });
      ...
      PluginStores = {
        loaded: {},
        delayed: { loads: [], starts: [] },
        updateData: { plugins: {}, timeouts: [], downloaded: [], interval: null },
        modulePatches: {},
      };
      ...
      const Plugin = function (changeLog) {
        return class Plugin {
          constructor(meta) {
            for (let key in meta) if (!this[key]) this[key] = meta[key];
          }
          getName() {
            return this.name;
          }
          ...
          start() {
            ...
            if (window.BDFDB_Global.loading) {
              if (!PluginStores.delayed.starts.includes(this))
                PluginStores.delayed.starts.push(this);
            } else {
          ...
          stop() {
            if (window.BDFDB_Global.loading) {
              if (PluginStores.delayed.starts.includes(this))
                PluginStores.delayed.starts.splice(PluginStores.delayed.starts.indexOf(this), 1);
            } else {
      ...
      const requestFunction = function (...args) {
        ...
        if (!url) return;
        let callback =
          (typeof args[1] == "function" && args[1]) || (typeof args[2] == "function" && args[2]);
        ...
        if (url.indexOf("data:") == 0)
          ...
          if (config.form && typeof config.form == "object") {
            let query = Object.entries(config.form)
              .map((n) => n[0] + "=" + n[1])
              .join("&");
            if (query) url += `?${query}`;
          }
          ...
          let killed = false,
            timeoutObj = BDFDB.TimeUtils.timeout((_) => {
              killed = true;
          ...
          let response = null,
            isFallback = false;
          return (
            config.bdVersion && BdApi && BdApi.Net && BdApi.Net.fetch ? BdApi.Net.fetch : fetch
          )(url, config)
            .catch((error) => {
              BDFDB.TimeUtils.clear(timeoutObj);
              if (!config.bdVersion)
                return requestFunction(
                  url,
                  Object.assign({}, config, { bdVersion: true }),
                  callback,
                );
              else
                callback(
            ...
            .then((result) => {
              if (!killed && response) {
                if (!config.toBase64 || response.status != 200)
                  callback(
                    response.status != 200
                      ? new Error(response.statusText || "Fetch Failed")
                      : null,
                    response,
                    result,
                  );
      ...
      Internal.console = function (type, config = {}) {
        ...
        console[type](
          ...[
            [name && `%c[${name}]`, version && `%c(v${version})`].filter((n) => n).join(" "),
            name && "color: #3a71c1; font-weight: 700;",
            version && "color: #666; font-weight: 600; font-size: 11px;",
            [config.strings]
              .flat(10)
              .filter((n) => n)
              .join(" ")
      ...
      BDFDB.TimeUtils.timeout = function (callback, delay, ...args) {
        ...
        if (isNaN(delay) || typeof delay != "number" || delay < 1) {
          let immediate = setImmediate((_) =>
            BDFDB.TimeUtils.suppress(callback, "Immediate")(...[immediate, args].flat()),
          );
          return immediate;
        } else {
          let start,
            ...
            timeout = {
              ...
              resume: (_) => {
                ...
                timeout.timer = setTimeout(
                  (_) => BDFDB.TimeUtils.suppress(callback, "Timeout")(...[timeout, args].flat()),
                  delay,
                );
              },
            };
      ...
      BDFDB.TimeUtils.suppress = function (callback, strings, config) {
        return function (...args) {
          try {
            return callback(...args);
          } catch (err) {
            (!config || !config.ignoreErrors) && BDFDB.LogUtils.error([strings, err], config);
          }
        };
      };
      ...
      BDFDB.sameProto = function (a, b) {
        if (a != null && typeof a == "object")
          return (
            ...
            typeof a.constructor.prototype.isPrototypeOf == "function" &&
            a.constructor.prototype.isPrototypeOf(b)
          );
          ...
          if (i > 1000) result = null;
            ...
            if (typeof a !== typeof b) result = false;
              ...
              if (keysA.length !== keysB.length) result = false;
                ...
                for (let j = 0; result === true && j < keysA.length; j++) {
      ...
      BDFDB.ObjectUtils.get = function (nodeOrObj, valuePath) {
        ...
        let obj = Node.prototype.isPrototypeOf(nodeOrObj)
          ? BDFDB.ReactUtils.getInstance(nodeOrObj)
          : nodeOrObj;
      ...
      BDFDB.ObjectUtils.invert = function (obj) {
        ...
        if (BDFDB.ObjectUtils.is(obj))
          for (let entry of Object.entries(obj)) newObj[entry[1]] = entry[0];
        return newObj;
      };
      BDFDB.ObjectUtils.extract = function (obj, ...keys) {
        let newObj = {};
        if (BDFDB.ObjectUtils.is(obj))
          for (let key of keys.flat(10).filter((n) => n))
      ...
      BDFDB.ObjectUtils.delete = function (obj, ...keys) {
        if (BDFDB.ObjectUtils.is(obj))
          for (let key of keys.flat(10).filter((n) => n)) delete obj[key];
      };
      BDFDB.ObjectUtils.sort = function (obj, sort, except) {
        if (!BDFDB.ObjectUtils.is(obj)) return {};
        let newObj = {};
        if (sort === undefined || !sort)
          for (let key of Object.keys(obj).sort()) newObj[key] = obj[key];
        else {
          let values = [];
      ...
      BDFDB.ObjectUtils.filter = function (obj, filter, byKey = false) {
        ...
        return Object.keys(obj)
          .filter((key) => filter(byKey ? key : obj[key]))
      ...
      BDFDB.ObjectUtils.map = function (obj, mapFunc) {
        ...
        let newObj = {};
        for (let key in obj)
          if (BDFDB.ObjectUtils.is(obj[key]))
            newObj[key] = typeof mapFunc == "string" ? obj[key][mapFunc] : mapFunc(obj[key], key);
        return newObj;
      };
      ...
      BDFDB.ObjectUtils.copy = function (obj) {
        ...
        for (let key of Reflect.ownKeys(obj.constructor.prototype))
          if (!copy[key] && obj[key] !== undefined) copy[key] = obj[key];
        return copy;
      };
      ...
      BDFDB.BDUtils.getPluginsFolder = function () {
        ...
        else if (Internal.LibraryRequires.process.env.BETTERDISCORD_DATA_PATH)
          return Internal.LibraryRequires.path.resolve(
            Internal.LibraryRequires.process.env.BETTERDISCORD_DATA_PATH,
            "plugins/",
          );
        else if (Internal.LibraryRequires.process.env.injDir)
          return Internal.LibraryRequires.path.resolve(
            Internal.LibraryRequires.process.env.injDir,
            "plugins/",
          );
        else
          switch (Internal.LibraryRequires.process.platform) {
            case "win32":
              return Internal.LibraryRequires.path.resolve(
                Internal.LibraryRequires.process.env.appdata,
                "BetterDiscord/plugins/",
              );
            case "darwin":
              return Internal.LibraryRequires.path.resolve(
                Internal.LibraryRequires.process.env.HOME,
                "Library/Preferences/BetterDiscord/plugins/",
              );
            default:
              if (Internal.LibraryRequires.process.env.XDG_CONFIG_HOME)
                return Internal.LibraryRequires.path.resolve(
                  Internal.LibraryRequires.process.env.XDG_CONFIG_HOME,
                  "BetterDiscord/plugins/",
                );
              else if (Internal.LibraryRequires.process.env.HOME)
                return Internal.LibraryRequires.path.resolve(
                  Internal.LibraryRequires.process.env.HOME,
      ...
      BDFDB.BDUtils.getThemesFolder = function () {
        if (BdApi && BdApi.Themes && BdApi.Themes.folder && typeof BdApi.Themes.folder == "string")
          ...
          switch (Internal.LibraryRequires.process.platform) {
            ...
            case "darwin":
              return Internal.LibraryRequires.path.resolve(
                Internal.LibraryRequires.process.env.HOME,
                "Library/Preferences/BetterDiscord/themes/",
              );
            default:
              if (Internal.LibraryRequires.process.env.XDG_CONFIG_HOME)
                return Internal.LibraryRequires.path.resolve(
      ...
      BDFDB.BDUtils.isPluginEnabled = function (pluginName) {
        if (BdApi && BdApi.Plugins && typeof BdApi.Plugins.isEnabled == "function")
      ...
      BDFDB.BDUtils.enablePlugin = function (pluginName) {
        if (BdApi && BdApi.Plugins && typeof BdApi.Plugins.enable == "function")
          BdApi.Plugins.enable(pluginName);
      };
      BDFDB.BDUtils.disablePlugin = function (pluginName) {
        if (BdApi && BdApi.Plugins && typeof BdApi.Plugins.disable == "function")
          BdApi.Plugins.disable(pluginName);
      };
      BDFDB.BDUtils.getPlugin = function (pluginName, hasToBeEnabled = false, overHead = false) {
        if (
          (BdApi && !hasToBeEnabled) ||
          (BDFDB.BDUtils.isPluginEnabled(pluginName) &&
            BdApi.Plugins &&
            typeof BdApi.Plugins.get == "function")
        ) {
          let plugin = BdApi.Plugins.get(pluginName);
          if (!plugin) return null;
          if (overHead)
            return plugin.filename && plugin.exports && plugin.instance
              ? plugin
              : {
                  filename: Internal.LibraryRequires.fs.existsSync(
                    Internal.LibraryRequires.path.join(
                      BDFDB.BDUtils.getPluginsFolder(),
            ...
            return plugin.filename && plugin.exports && plugin.instance ? plugin.instance : plugin;
        }
        return null;
      };
      BDFDB.BDUtils.isThemeEnabled = function (themeName) {
        if (BdApi && BdApi.Themes && typeof BdApi.Themes.isEnabled == "function")
          return BdApi.Themes.isEnabled(themeName);
      };
      BDFDB.BDUtils.enableTheme = function (themeName) {
        if (BdApi && BdApi.Themes && typeof BdApi.Themes.enable == "function")
          BdApi.Themes.enable(themeName);
      };
      BDFDB.BDUtils.disableTheme = function (themeName) {
        if (BdApi && BdApi.Themes && typeof BdApi.Themes.disable == "function")
          BdApi.Themes.disable(themeName);
      };
      BDFDB.BDUtils.getTheme = function (themeName, hasToBeEnabled = false) {
        if (
          (BdApi && !hasToBeEnabled) ||
          (BDFDB.BDUtils.isThemeEnabled(themeName) &&
            BdApi.Themes &&
            typeof BdApi.Themes.get == "function")
        )
          return BdApi.Themes.get(themeName);
        return null;
      };
      ...
      BDFDB.BDUtils.toggleSettings = function (key, state) {
        if (BdApi && typeof key == "string") {
          ...
          if (state === true) {
            if (currentState === false && typeof BdApi.enableSetting == "function")
              BdApi.enableSetting(...path);
          } else if (state === false) {
            if (currentState === true && typeof BdApi.disableSetting == "function")
              BdApi.disableSetting(...path);
          } else if (currentState === true || currentState === false)
            BDFDB.BDUtils.toggleSettings(key, !currentState);
        }
      };
      BDFDB.BDUtils.getSettings = function (key) {
        ...
        if (typeof key == "string")
          return (
            typeof BdApi.isSettingEnabled == "function" && BdApi.isSettingEnabled(...key.split("."))
          );
        else
          return BDFDB.ArrayUtils.is(BdApi.settings)
            ? BdApi.settings
                .map((n) =>
                  n.settings.map((m) =>
                    m.settings.map((l) => ({ id: [n.id, m.id, l.id].join("."), value: l.value })),
                  ),
                )
                .flat(10)
                .reduce((newObj, setting) => ((newObj[setting.id] = setting.value), newObj), {})
            : {};
      };
      BDFDB.BDUtils.getSettingsProperty = function (property, key) {
        if (!BdApi || !BDFDB.ArrayUtils.is(BdApi.settings)) return key ? "" : {};
        else {
          let settingsMap = BdApi.settings
            .map((n) =>
              n.settings.map((m) =>
                m.settings.map((l) => ({ id: [n.id, m.id, l.id].join("."), value: l[property] })),
              ),
            )
            .flat(10)
            .reduce((newObj, setting) => ((newObj[setting.id] = setting.value), newObj), {});
      ...
      const cssFileName = "0BDFDB.raw.css",
        dataFileName = "0BDFDB.data.json";
      const cssFilePath = path.join(BDFDB.BDUtils.getPluginsFolder(), cssFileName),
        dataFilePath = path.join(BDFDB.BDUtils.getPluginsFolder(), dataFileName);
      BDFDB.PluginUtils = {};
      ...
      BDFDB.PluginUtils.load = function (plugin) {
        if (!PluginStores.updateData.timeouts.includes(plugin.name)) {
          ...
          PluginStores.updateData.plugins[url] = {
            name: plugin.name,
            raw: url,
          ...
          if (
            plugin.changeLog &&
            !BDFDB.ObjectUtils.isEmpty(plugin.changeLog) &&
            typeof plugin.getSettingsPanel != "function"
          )
            plugin.getSettingsPanel = (_) =>
              BDFDB.PluginUtils.createSettingsPanel(plugin, {
                children: BDFDB.ReactUtils.createElement(
                  Internal.LibraryComponents.MessagesPopoutComponents.EmptyState,
                  {
                    msg: "No Settings available for this Plugin",
                    image:
                      BDFDB.DiscordUtils.getTheme() == BDFDB.disCN.themelight
                        ? "/assets/9b0d90147f7fab54f00dd193fe7f85cd.svg"
                        : "/assets/308e587f3a68412f137f7317206e92c2.svg",
                  },
                ),
              });

          if (!PluginStores.updateData.interval)
            PluginStores.updateData.interval = BDFDB.TimeUtils.interval(
              (_) => {
          ...
          BDFDB.TimeUtils.timeout(
            (_) => BDFDB.ArrayUtils.remove(PluginStores.updateData.timeouts, plugin.name, true),
            30000,
          );
        }
      };
      BDFDB.PluginUtils.init = function (plugin) {
        ...
        BDFDB.LogUtils.log(
          BDFDB.LanguageUtils.LibraryStringsFormat("toast_plugin_started", ""),
          plugin,
        );
        if (
          Internal.settings.general.showToasts &&
          !BDFDB.BDUtils.getSettings(BDFDB.BDUtils.settingsIds.showToasts)
        )
          BDFDB.NotificationUtils.toast(
            BDFDB.LanguageUtils.LibraryStringsFormat(
      ...
      BDFDB.PluginUtils.clear = function (plugin) {
        BDFDB.LogUtils.log(
          BDFDB.LanguageUtils.LibraryStringsFormat("toast_plugin_stopped", ""),
          plugin,
        );
        if (
          Internal.settings.general.showToasts &&
          ...
          BDFDB.NotificationUtils.toast(
            ...
            { disableInteractions: true, barColor: BDFDB.DiscordConstants.ColorsCSS.STATUS_DANGER },
          );
        ...
        for (const modal of document.querySelectorAll(
          `.${plugin.name}-modal, .${plugin.name.toLowerCase()}-modal, .${plugin.name}-settingsmodal, .${plugin.name.toLowerCase()}-settingsmodal`,
        )) {
      ...
      BDFDB.PluginUtils.translate = function (plugin) {
        if (
          typeof plugin.setLabelsByLanguage == "function" ||
          typeof plugin.changeLanguageStrings == "function"
        ) {
          const translate = (_) => {
            if (typeof plugin.setLabelsByLanguage == "function")
        ...
        BDFDB.TimeUtils.suppress(
          (_) => {
            ...
            if (plugin == window.BDFDB_Global) {
              ...
              BDFDB.TimeUtils.clear(
                PluginStores && PluginStores.updateData && PluginStores.updateData.interval,
              );
              ...
              if (PluginStores)
                BDFDB.TimeUtils.interval((interval, count) => {
                  ...
                  if (window.BDFDB_Global.loaded)
                    for (let pluginName in BDFDB.ObjectUtils.sort(PluginStores.loaded))
                      BDFDB.TimeUtils.timeout((_) => {
                        if (PluginStores.loaded[pluginName].started)
                          BDFDB.BDUtils.reloadPlugin(pluginName);
                      });
                }, 1000);
            }
            if (BDFDB.DOMUtils && BDFDB.DOMUtils.removeLocalStyle)
              BDFDB.DOMUtils.removeLocalStyle(plugin.name);
            if (BDFDB.ListenerUtils && BDFDB.ListenerUtils.remove)
              BDFDB.ListenerUtils.remove(plugin);
            if (BDFDB.ListenerUtils && BDFDB.ListenerUtils.removeGlobal)
              BDFDB.ListenerUtils.removeGlobal(plugin);
            if (BDFDB.StoreChangeUtils && BDFDB.StoreChangeUtils.remove)
            ...
            for (const patchType in PluginStores.modulePatches) {
              for (const type in PluginStores.modulePatches[patchType]) {
                for (const priority in PluginStores.modulePatches[patchType][type])
                  BDFDB.ArrayUtils.remove(
                    PluginStores.modulePatches[patchType][type][priority],
                    plugin,
                    true,
                  );
                if (!PluginStores.modulePatches[patchType][type].flat(10).length)
                  delete PluginStores.modulePatches[patchType][type];
              }
              if (BDFDB.ObjectUtils.isEmpty(PluginStores.modulePatches[patchType]))
                delete PluginStores.modulePatches[patchType];
            }
          },
      ...
      BDFDB.PluginUtils.checkUpdate = function (pluginName, url) {
        if (
          pluginName &&
          url &&
          Internal.settings.general.checkForUpdates &&
          PluginStores.updateData.plugins[url]
        )
          return new Promise((callback) => {
            requestFunction(url, { timeout: 60000 }, (error, response, body) => {
              ...
              let newVersion = (
                body.match(/@version ([0-9]+\.[0-9]+\.[0-9]+)|['"]([0-9]+\.[0-9]+\.[0-9]+)['"]/i) ||
                []
              ).filter((n) => n)[1];
              if (!newVersion) return callback(null);
              if (
                BDFDB.NumberUtils.compareVersions(
                  newVersion,
                  PluginStores.updateData.plugins[url].version,
                )
              ) {
                if (PluginStores.updateData.plugins[url])
                  PluginStores.updateData.plugins[url].outdated = true;
      ...
      BDFDB.PluginUtils.checkAllUpdates = function () {
        return new Promise((callback) => {
          ...
          for (let url in PluginStores.updateData.plugins) {
            ...
            if (plugin)
              BDFDB.PluginUtils.checkUpdate(plugin.name, plugin.raw).then((state) => {
                finished++;
                if (state == 1) amount++;
                if (finished >= Object.keys(PluginStores.updateData.plugins).length)
                  callback(amount);
              });
          }
        });
      ...
      BDFDB.PluginUtils.hasUpdateCheck = function (url) {
        ...
        let updateStore = Object.assign(
          {},
          window.PluginUpdates && window.PluginUpdates.plugins,
          PluginStores.updateData.plugins,
        );
          ...
          let vanishObserver = new MutationObserver((changes) => {
            if (!document.contains(updateNotice)) {
              if (updateNotice && updateNotice.querySelector(BDFDB.dotCN.noticeupdateentry)) {
                let layers =
                  document.querySelector(BDFDB.dotCN.layers) ||
                  document.querySelector(BDFDB.dotCN.appmount);
                if (layers) layers.parentElement.insertBefore(updateNotice, layers);
              } else vanishObserver.disconnect();
            } else if (
              document.contains(updateNotice) &&
              !updateNotice.querySelector(BDFDB.dotCNC.noticeupdateentry + BDFDB.dotCN.noticebutton)
            )
              vanishObserver.disconnect();
          });
          vanishObserver.observe(document.body, { childList: true, subtree: true });
          updateNotice = BDFDB.NotificationUtils.notice(
            `${BDFDB.LanguageUtils.LibraryStrings.update_notice_update}&nbsp;&nbsp;&nbsp;&nbsp;<div class="${BDFDB.disCN.noticeupdateentries}"></div>`,
            {
              type: "info",
              ...
              buttons: [
                {
                  ...
                  onClick: (_) => {
                    for (let notice of updateNotice.querySelectorAll(BDFDB.dotCN.noticeupdateentry))
                      notice.click();
                  },
                },
              ],
        ...
        if (updateNotice) {
          ...
          if (updateNoticeList && !updateNoticeList.querySelector(`#${pluginName}-notice`)) {
            if (updateNoticeList.childElementCount)
              updateNoticeList.appendChild(
                BDFDB.DOMUtils.create(`<div class="${BDFDB.disCN.noticeupdateseparator}">, </div>`),
              );
            let updateEntry = BDFDB.DOMUtils.create(
              `<div class="${BDFDB.disCN.noticeupdateentry}" id="${pluginName}-notice">${pluginName}</div>`,
            );
            ...
            if (!updateNoticeList.hasTooltip) {
              ...
              updateNotice.tooltip = BDFDB.TooltipUtils.create(
                updateNoticeList,
                BDFDB.LanguageUtils.LibraryStrings.update_notice_click,
                {
                  type: "bottom",
                  zIndex: 100001,
                  delay: 500,
                  onHide: (_) => {
                    updateNoticeList.hasTooltip = false;
                  },
                },
              );
            }
      ...
      BDFDB.PluginUtils.removeUpdateNotice = function (
        pluginName,
        updateNotice = document.querySelector(BDFDB.dotCN.noticeupdate),
      ) {
        ...
        if (updateNoticeList) {
          ...
          if (noticeEntry) {
            ...
            if (
              nextSibling &&
              BDFDB.DOMUtils.containsClass(nextSibling, BDFDB.disCN.noticeupdateseparator)
            )
              nextSibling.remove();
            else if (
              prevSibling &&
              BDFDB.DOMUtils.containsClass(prevSibling, BDFDB.disCN.noticeupdateseparator)
            )
              prevSibling.remove();
            noticeEntry.remove();
          }
          if (!updateNoticeList.childElementCount) {
            let reloadButton = updateNotice.querySelector(BDFDB.dotCN.noticeupdatebuttonreload);
            if (reloadButton) {
              updateNotice.querySelector(BDFDB.dotCN.noticetext).innerText =
                BDFDB.LanguageUtils.LibraryStrings.update_notice_reload;
        ...
        if (pluginName && url)
          requestFunction(url, { timeout: 60000 }, (error, response, body) => {
            if (error) {
              BDFDB.PluginUtils.removeUpdateNotice(pluginName);
              BDFDB.NotificationUtils.toast(
                BDFDB.LanguageUtils.LibraryStringsFormat("toast_plugin_update_failed", pluginName),
                { type: "danger", disableInteractions: true },
              );
            } else {
              ...
              Internal.LibraryRequires.fs.writeFile(
                Internal.LibraryRequires.path.join(
                  BDFDB.BDUtils.getPluginsFolder(),
                  newFileName + ".plugin.js",
                ),
                body,
                (_) => {
                  if (PluginStores.updateData.plugins[url])
                    PluginStores.updateData.plugins[url].version = newVersion;
                  if (fileName != newFileName) {
                    Internal.LibraryRequires.fs.unlink(
                      Internal.LibraryRequires.path.join(
                        BDFDB.BDUtils.getPluginsFolder(),
                        fileName + ".plugin.js",
                      ),
                    ...
                    let configPath = Internal.LibraryRequires.path.join(
                      BDFDB.BDUtils.getPluginsFolder(),
                      fileName + ".config.json",
                    );
                    Internal.LibraryRequires.fs.exists(configPath, (exists) => {
                      if (exists)
                        Internal.LibraryRequires.fs.rename(
                          configPath,
                          Internal.LibraryRequires.path.join(
                            BDFDB.BDUtils.getPluginsFolder(),
                    ...
                    BDFDB.TimeUtils.timeout((_) => {
                      if (wasEnabled && !BDFDB.BDUtils.isPluginEnabled(newName))
                        BDFDB.BDUtils.enablePlugin(newName);
                    }, 3000);
                  }
                  BDFDB.NotificationUtils.toast(
                    BDFDB.LanguageUtils.LibraryStringsFormat(
                  ...
                  if (updateNotice) {
                    if (
                      updateNotice.querySelector(BDFDB.dotCN.noticebutton) &&
                      !PluginStores.updateData.downloaded.includes(pluginName)
                    ) {
      ...
      BDFDB.PluginUtils.checkChangeLog = function (plugin) {
        if (
          !BDFDB.ObjectUtils.is(plugin) ||
          !BDFDB.ObjectUtils.is(plugin.changeLog) ||
          plugin.changeLog.info
        )
          return;
        if (
          !changeLogs[plugin.name] ||
          BDFDB.NumberUtils.compareVersions(plugin.version, changeLogs[plugin.name])
        ) {
      ...
      BDFDB.PluginUtils.openChangeLog = function (plugin) {
        ...
        for (let type in plugin.changeLog) {
          ...
          if (InternalData.DiscordClasses["changelog" + type])
            changeLogEntries.push([
              BDFDB.ReactUtils.createElement("h1", {
                ...
                children:
                  (BDFDB.LanguageUtils &&
                    BDFDB.LanguageUtils.LibraryStrings &&
                    BDFDB.LanguageUtils.LibraryStrings["changelog_" + type]) ||
                  headers[type],
              }),
              BDFDB.ReactUtils.createElement("ul", {
                children: Object.keys(plugin.changeLog[type]).map((key) =>
                  BDFDB.ReactUtils.createElement("li", {
                    children: [
        ...
        if (changeLogEntries.length)
          BDFDB.ModalUtils.open(plugin, {
            ...
            footerChildren:
              (plugin == BDFDB ||
                plugin == this ||
                (PluginStores.loaded[plugin.name] &&
                  PluginStores.loaded[plugin.name] == plugin &&
                  plugin.author == "DevilBro")) &&
                ...
                children: [
                  ...
                  .map((data) =>
                    BDFDB.ReactUtils.createElement(
                      data.href
                        ? Internal.LibraryComponents.Anchor
                        : Internal.LibraryComponents.Clickable,
                      {
                        className: BDFDB.disCN.changelogsociallink,
                        href: data.href || "",
                        onClick: !data.onClick ? (_) => {} : data.onClick,
                        children: BDFDB.ReactUtils.createElement(
                          Internal.LibraryComponents.TooltipContainer,
                          {
                            text: data.name,
                            children: BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.SvgIcon,
                              {
      ...
      BDFDB.PluginUtils.addLoadingIcon = function (icon) {
        ...
        let loadingIconWrapper = document.querySelector(
          BDFDB.dotCN.app + ">" + BDFDB.dotCN.loadingiconwrapper,
        );
        if (!loadingIconWrapper) {
          loadingIconWrapper = BDFDB.DOMUtils.create(
            `<div class="${BDFDB.disCN.loadingiconwrapper}"></div>`,
          );
      ...
      BDFDB.PluginUtils.createSettingsPanel = function (addon, props) {
        if (!window.BDFDB_Global.loaded)
          return BdApi.React.createElement("div", {
            style: {
              color: BDFDB.DiscordConstants.ColorsCSS.HEADER_SECONDARY,
        ...
        if (
          settingsProps &&
          !BDFDB.ObjectUtils.is(settingsProps) &&
          (BDFDB.ReactUtils.isValidElement(settingsProps) || BDFDB.ArrayUtils.is(settingsProps))
        )
          settingsProps = { children: settingsProps };
        return BDFDB.ReactUtils.createElement(
          Internal.LibraryComponents.SettingsPanel,
          Object.assign(
            { addon: addon, collapseStates: settingsProps && settingsProps.collapseStates },
            settingsProps,
          ),
        );
      };
      BDFDB.PluginUtils.refreshSettingsPanel = function (plugin, settingsPanel, ...args) {
        if (BDFDB.ObjectUtils.is(plugin)) {
          if (
            settingsPanel &&
            settingsPanel.props &&
            BDFDB.ObjectUtils.is(settingsPanel.props._instance)
          ) {
            settingsPanel.props._instance.props = Object.assign(
              {},
              settingsPanel.props._instance.props,
              ...args,
            );
            BDFDB.ReactUtils.forceUpdate(settingsPanel.props._instance);
          } else if (
            typeof plugin.getSettingsPanel == "function" &&
            Node.prototype.isPrototypeOf(settingsPanel) &&
            settingsPanel.parentElement
          ) {
      ...
      Internal.writeConfig = function (plugin, path, config) {
        let allData = {};
        try {
          allData = JSON.parse(fs.readFileSync(path));
        } catch (err) {
          allData = {};
        }
        try {
          fs.writeFileSync(
            ...
            JSON.stringify(
              Object.assign({}, allData, {
                [Internal.shouldSyncConfig(plugin) ? "all" : BDFDB.UserUtils.me.id]: config,
              }),
              null,
              "	",
            ),
      ...
      Internal.shouldSyncConfig = function (plugin) {
        return plugin.neverSyncData !== undefined
          ? !plugin.neverSyncData
          : plugin.forceSyncData || Internal.settings.general.shareData;
      };
      ...
      BDFDB.DataUtils.save = function (data, plugin, key, id) {
        ...
        let config =
          Cache.data[pluginName] !== undefined
            ? Cache.data[pluginName]
            : Internal.readConfig(plugin, configPath) || {};

        if (key === undefined)
          config = BDFDB.ObjectUtils.is(data) ? BDFDB.ObjectUtils.sort(data) : data;
        else {
          if (id === undefined)
        ...
        if (
          key !== undefined &&
          configIsObject &&
          BDFDB.ObjectUtils.is(config[key]) &&
          BDFDB.ObjectUtils.isEmpty(config[key])
        )
          delete config[key];
        if (BDFDB.ObjectUtils.isEmpty(config)) {
          ...
          Cache.data[pluginName] = configIsObject
            ? BDFDB.ObjectUtils.deepAssign({}, config)
            : config;
      ...
      BDFDB.DataUtils.load = function (plugin, key, id) {
        ...
        if (key === undefined) return config;
          ...
          let keyData = configIsObject
            ? BDFDB.ObjectUtils.is(config[key]) || config[key] === undefined
              ? BDFDB.ObjectUtils.deepAssign({}, config[key])
              : config[key]
            : null;
          if (id === undefined) return keyData;
          else
            return !BDFDB.ObjectUtils.is(keyData) || keyData[id] === undefined ? null : keyData[id];
        }
      };
      BDFDB.DataUtils.remove = function (plugin, key, id) {
        ...
        if (BDFDB.ObjectUtils.is(config[key]) && BDFDB.ObjectUtils.isEmpty(config[key]))
          delete config[key];
        if (BDFDB.ObjectUtils.isEmpty(config)) {
          delete Cache.data[pluginName];
          if (fs.existsSync(configPath)) fs.unlinkSync(configPath);
      ...
      BDFDB.DataUtils.get = function (plugin, key, id) {
        ...
        if (!BDFDB.ObjectUtils.is(defaults) || (key && !BDFDB.ObjectUtils.is(defaults[key])))
        ...
        const checkLayer = (i, j) => {
          ...
          if (
            ...
            oldC[i][j] == null ||
            (isObj &&
              (!BDFDB.ObjectUtils.is(oldC[i][j]) ||
                Object.keys(defaults[i][j].value).some(
                  (n) =>
                    defaults[i][j].value[n] != null &&
                    !BDFDB.sameProto(defaults[i][j].value[n], oldC[i][j][n]),
                )))
          ) {
            newC[i][j] = isObj
              ? BDFDB.ObjectUtils.deepAssign({}, defaults[i][j].value)
              : defaults[i][j].value;
            update = true;
          } else newC[i][j] = oldC[i][j];
        };
        if (key) {
          for (let j in defaults[key]) checkLayer(key, j);
        } else {
          for (let i in defaults)
            if (BDFDB.ObjectUtils.is(defaults[i])) for (let j in defaults[i]) checkLayer(i, j);
        }
      ...
      let InternalData,
        libHashes = {},
        oldLibHashes = BDFDB.DataUtils.load(BDFDB, "hashes"),
        libraryCSS;

      const getBackup = (fileName, path) => {
        return {
          backup: fs.existsSync(path) && (fs.readFileSync(path) || "").toString(),
          hashIsSame:
            libHashes[fileName] &&
            oldLibHashes[fileName] &&
            libHashes[fileName] == oldLibHashes[fileName],
        };
      };
      const requestLibraryHashes = (tryAgain) => {
        requestFunction(
          "https://api.github.com/repos/mwittrien/BetterDiscordAddons/contents/Library/_res/",
          { timeout: 60000 },
          (e, r, b) => {
            if ((e || !b || r.statusCode != 200) && tryAgain)
              return BDFDB.TimeUtils.timeout((_) => requestLibraryHashes(), 10000);
            else {
              try {
                b = JSON.parse(b);
      ...
      const requestLibraryData = (tryAgain) => {
        const parseCSS = (css) => {
          ...
          const UserStore =
            ...
            BdApi.Webpack.getModule(BdApi.Webpack.Filters.byKeys("getCurrentUser"));
          if (
            (backupObj.backup && backupObj.hashIsSame) ||
            (UserStore && UserStore.getCurrentUser().id == "278543574059057154")
          )
            parseData(backupObj.backup);
          else
            requestFunction(
              `https://mwittrien.github.io/BetterDiscordAddons/Library/_res/${dataFileName}`,
              { timeout: 60000 },
              (e, r, b) => {
                if ((e || !b || r.statusCode != 200) && tryAgain)
                  return BDFDB.TimeUtils.timeout((_) => requestLibraryData(), 10000);
                if (!e && b && r.statusCode == 200) {
                  if (
                    backupObj.backup &&
                    backupObj.backup.replace(/\s/g, "") == b.replace(/\s/g, "")
                  ) {
                  ...
                  parseData(
                    fs.existsSync(dataFilePath) && (fs.readFileSync(dataFilePath) || "").toString(),
                  );
              },
            );
        };
        const parseData = (dataString, fetched) => {
          try {
            ...
            if (fetched) {
              try {
                ...
                BDFDB.LogUtils.error([
                  "Failed to initiate Library!",
                  "Failed Fetch!",
                  dataString ? "Corrupt Backup." : "No Backup.",
                  ,
                  err2,
                ]);
              }
          ...
          if (InternalData) loadLibrary();
            ...
            BdApi.UI.alert(
              "Error",
              "Could not initiate BDFDB Library Plugin. Check your Internet Connection and make sure GitHub isn't blocked by your Network or try disabling your VPN/Proxy.",
            );
        };
        ...
        if (backupObj.backup && backupObj.hashIsSame) parseCSS(backupObj.backup);
          ...
          requestFunction(
            `https://mwittrien.github.io/BetterDiscordAddons/Library/_res/${cssFileName}`,
            { timeout: 60000 },
            (e, r, b) => {
              if ((e || !b || r.statusCode != 200) && tryAgain)
                return BDFDB.TimeUtils.timeout((_) => requestLibraryData(), 10000);
              if (!e && b && r.statusCode == 200) {
                ...
                parseCSS(
                  fs.existsSync(cssFilePath) && (fs.readFileSync(cssFilePath) || "").toString(),
                );
            },
          );
      };
      const loadLibrary = (_) => {
        Internal.getPluginURL = function (plugin) {
          plugin = (plugin == BDFDB && Internal) || plugin;
          if (BDFDB.ObjectUtils.is(plugin)) {
            if (InternalData.PluginUrlMap && InternalData.PluginUrlMap[plugin.name])
              return InternalData.PluginUrlMap[plugin.name];
            else if (plugin.updateUrl) return plugin.updateUrl;
            else {
              let name =
                (InternalData.PluginNameMap && InternalData.PluginNameMap[plugin.name]) ||
                plugin.name;
        ...
        Internal.findModule = function (type, cacheString, filter, config = {}) {
          if (!BDFDB.ObjectUtils.is(Cache.modules[type]))
            Cache.modules[type] = { module: {}, export: {} };
          let defaultExport =
            typeof config.defaultExport != "boolean" ? true : config.defaultExport;
          if (!config.all && defaultExport && Cache.modules[type].export[cacheString])
            ...
            if (m) {
              ...
              BDFDB.LogUtils.warn(`${cacheString} [${type}] not found in WebModules`);
          }
        };
        Internal.checkModuleStrings = function (module, strings, config = {}) {
          ...
          return [strings]
            ...
            .map(config.ignoreCase ? (n) => n.toLowerCase() : (n) => n)
            .every(
              (string) =>
                module &&
                (((typeof module == "function" || typeof module == "string") &&
                  (check(module, string) ||
                    (typeof module.__originalFunction == "function" &&
                      check(module.__originalFunction, string)))) ||
                  (typeof module.type == "function" && check(module.type, string)) ||
                  ((typeof module == "function" || typeof module == "object") &&
                    module.prototype &&
                    Object.keys(module.prototype)
                      .filter((n) => n.indexOf("render") == 0)
                      .some((n) => check(module.prototype[n], string)))),
            );
        };
        ...
        Internal.getModuleString = function (module) {
          const id = (
            BDFDB.ModuleUtils.find((m) => m == module && m, { defaultExport: false }) || {}
          ).id;
          if (!id) return "";
        ...
        BDFDB.ModuleUtils.lazyLoadModuleImports = function (moduleString) {
          return new Promise((callback) => {
            if (typeof moduleString !== "string")
              moduleString = Internal.getModuleString(moduleString);
            ...
            let run = true,
              ...
              menuIndexes = [];
            while (run) {
              const [matchString, promiseMatch, menuRequest] =
                moduleString.match(/await .+?(\(.+?\))\.then\(.{1}\.bind\((.+?)\)\)/) ?? [];
            ...
            const req = Internal.getWebModuleReq();
            Promise.all(BDFDB.ArrayUtils.removeCopies(imports).map((i) => req.e(i)))
              .then((_) =>
                Promise.all(BDFDB.ArrayUtils.removeCopies(menuIndexes).map((i) => req(i))),
              )
              .then(callback);
          });
        };
        ...
        BDFDB.ModuleUtils.find = function (filter, config = {}) {
          let defaultExport =
            typeof config.defaultExport != "boolean" ? true : config.defaultExport;
          let onlySearchUnloaded =
            typeof config.onlySearchUnloaded != "boolean" ? false : config.onlySearchUnloaded;
          let all = typeof config.all != "boolean" ? false : config.all;
          const req = Internal.getWebModuleReq();
          const found = [];
          const isSearchable = (m, checkObject) =>
            m &&
            ((checkObject && typeof m == "object") || typeof m == "function") &&
            !(m instanceof DOMTokenList) &&
            m[Symbol.toStringTag] !== "IntlMessagesProxy";
          if (!onlySearchUnloaded) {
              ...
              for (let i in req.c) {
                ...
                if (keys.length < 400) {
                  ...
                  for (let ki = 0; ki < keys.length; ki++)
                    try {
                      let sub = m[keys[ki]];
                      if (sub && isSearchable(sub, true)) subs.push(sub);
                    } catch (e) {}
                ...
                _moduleIndex.push({
                  ...
                  subs,
                  def,
                  defType: def && isSearchable(m.default.type, true) ? m.default.type : null,
                });
              }
            }
            const _stale = () => {
              let c = 0;
              for (let i in req.c) c++;
              if (c !== _moduleIndexCount) {
                _moduleIndex = null;
                return true;
              }
              return false;
            ...
            for (let j = 0; j < _moduleIndex.length; j++) {
              let e = _moduleIndex[j],
                r = null;
              if (!!(r = filter(e.m))) {
                if (all) found.push(defaultExport ? r : e.mod);
                else return defaultExport ? r : e.mod;
              }
              if (e.subs)
                for (let k = 0; k < e.subs.length; k++)
                  if (!!(r = filter(e.subs[k]))) {
                    if (all) found.push(defaultExport ? r : e.mod);
                    else return defaultExport ? r : e.mod;
                  }
              if (config.moduleName && isSearchable(e.m[config.moduleName], true)) {
                if (!!(r = filter(e.m[config.moduleName]))) {
                  if (all) found.push(defaultExport ? r : e.mod);
                  else return defaultExport ? r : e.mod;
                } else if (
                  e.m[config.moduleName].type &&
                  isSearchable(e.m[config.moduleName].type, true) &&
                  !!(r = filter(e.m[config.moduleName].type))
                ) {
                  if (all) found.push(defaultExport ? r : e.mod);
                  else return defaultExport ? r : e.mod;
                }
              ...
              if (e.def) {
                ...
                } else if (e.defType && !!(r = filter(e.defType))) {
                  if (all) found.push(defaultExport ? r : e.mod);
                  else return defaultExport ? r : e.mod;
                }
              }
          ...
          for (let i in req.m)
            if (req.m.hasOwnProperty(i)) {
              ...
              if (m && isSearchable(m)) {
                if (
                  req.c[i] &&
                  isSearchable(req.c[i].exports, true) &&
                  (!config.exportsFilter || config.exportsFilter(req.c[i].exports)) &&
                  !onlySearchUnloaded &&
                  filter(m)
                ) {
                ...
                if (!req.c[i] && onlySearchUnloaded && filter(m)) {
                  ...
                  const trueResolved =
                    resolved2 && BDFDB.ObjectUtils.isEmpty(resolved2) ? resolved : resolved2;
        ...
        BDFDB.ModuleUtils.findByProperties = function (...properties) {
          ...
          return Internal.findModule(
            "props",
            JSON.stringify(properties),
            (m) => Internal.checkModuleProps(m, properties) && m,
            config,
          );
        };
        BDFDB.ModuleUtils.findByName = function (name, config = {}) {
          return Internal.findModule(
            ...
            JSON.stringify(name),
            (m) =>
              (m.displayName === name && m) ||
              (m.render && m.render.displayName === name && m) ||
              (m.constructor && m.constructor.displayName === name && m) ||
              (m[name] && m[name].displayName === name && m[name]) ||
              (typeof m.getName == "function" && m.getName() == name && m),
            config,
          );
        };
        BDFDB.ModuleUtils.findByString = function (...strings) {
          ...
          return Internal.findModule(
            ...
            (m) => Internal.checkModuleStrings(m, strings) && m,
            config,
          );
        };
        BDFDB.ModuleUtils.findByPrototypes = function (...protoProps) {
          ...
          return Internal.findModule(
            "proto",
            JSON.stringify(protoProps),
            (m) => Internal.checkModuleProtos(m, protoProps) && m,
            config,
          );
        };
        BDFDB.ModuleUtils.findStringObject = function (props, config = {}) {
          let nonProps = [config.nonProps].flat(10).filter((n) => n);
          let firstReturn = BDFDB.ModuleUtils.find(
            (m) => {
              let amount = Object.keys(m).length;
              return (
                (!config.length ||
                  (config.smaller ? amount < config.length : amount == config.length)) &&
                [props].flat(10).every((prop) => typeof m[prop] == "string") &&
                (!nonProps.length || nonProps.every((prop) => typeof m[prop] == "undefined")) &&
                m
              );
            },
          ...
          let secondReturn = BDFDB.ModuleUtils.find(
            (m) => {
              if (typeof m != "function") return false;
              let stringified = m.toString().replace(/\s/g, "");
              if (
                stringified.indexOf(".exports={") == -1 ||
                !(
                  /function\([A-z],[A-z],[A-z]\)\{[A-z]\.[A-z]\([A-z]\.exports=\{/.test(
                    stringified,
                  ) ||
                  /function\([A-z],[A-z],[A-z]\)\{[A-z]\.exports=\{/.test(stringified) ||
                  /function\([A-z]\)\{[A-z]\.exports=\{/.test(stringified)
                )
              )
                return false;
              ...
              return (
                (!config.length ||
                  (config.smaller ? amount < config.length : amount == config.length)) &&
                [props].flat(10).every((string) => stringified.indexOf(`${string}:`) > -1) &&
                (!nonProps.length ||
                  nonProps.every((string) => stringified.indexOf(`${string}:`) == -1)) &&
                m
              );
            },
        ...
        const DiscordConstantsObject = BDFDB.ModuleUtils.find((m) => {
          try {
            if (!m || !Object.keys(m)) return;
            if (
              [
                ["CUSTOM_STATUS", "STREAMING", "WATCHING"],
                ["GUILD_TEXT", "GUILD_FORUM", "GUILD_STORE"],
                ["THREAD_CREATED", "CHAT_INPUT_COMMAND"],
              ].every((array) =>
        ...
        if (InternalData.CustomDiscordConstants)
          DiscordConstants = Object.assign(DiscordConstants, InternalData.CustomDiscordConstants);
        if (DiscordConstantsObject)
          DiscordConstants = Object.assign(DiscordConstants, DiscordConstantsObject);
        Internal.DiscordConstants = new Proxy(DiscordConstants, {
          get: function (_, item) {
            ...
            if (InternalData.DiscordConstants[item].strings)
              DiscordConstants[item] = BDFDB.ModuleUtils.findByString(
                InternalData.DiscordConstants[item].strings,
              );
            else if (InternalData.DiscordConstants[item].props)
              DiscordConstants[item] = BDFDB.ModuleUtils.findByProperties(
                InternalData.DiscordConstants[item].props,
              );
            else {
              if (DiscordConstantsObject)
                DiscordConstants[item] =
                  DiscordConstantsObject[
                    Object.keys(DiscordConstantsObject).find((n) => {
                      let keys = Object.keys(DiscordConstantsObject[n]);
                      if (
                        keys &&
                        keys.length < 10000 &&
                        InternalData.DiscordConstants[item].every &&
                        InternalData.DiscordConstants[item].every((k) => keys.indexOf(k) > -1)
                      )
                        return true;
                    })
                  ];
              if (!DiscordConstants[item])
                DiscordConstants[item] = BDFDB.ModuleUtils.findByProperties(
                  InternalData.DiscordConstants[item],
                );
            }
            if (DiscordConstants[item] && InternalData.DiscordConstants[item].value)
              DiscordConstants[item] =
                DiscordConstants[item][InternalData.DiscordConstants[item].value] ||
                DiscordConstants[item];
        ...
        Internal.DiscordConstants.ColorsCSS = new Proxy(ColorsCSS, {
          get: function (_, item) {
            const color =
              ColorsCSS[item] || ColorsCSS[item.toLowerCase()] || ColorsCSS[item.toUpperCase()];
        ...
        Internal.DiscordConstants.Colors = new Proxy(DiscordColors, {
          get: function (_, item) {
            const color =
              DiscordColors[item] ||
              DiscordColors[item.toLowerCase()] ||
              DiscordColors[item.toUpperCase()];
            if (color) return (color && color.hex) || color || "";
            else {
              const item2 = item + "_500";
              const color2 =
                DiscordColors[item2] ||
                DiscordColors[item2.toLowerCase()] ||
                DiscordColors[item2.toUpperCase()];
              if (color2) return (color2 && color2.hex) || color2 || "";
              else {
                const item3 = item.replace(/-/g, "_");
                const color3 =
                  DiscordColors[item3] ||
                  DiscordColors[item3.toLowerCase()] ||
                  DiscordColors[item3.toUpperCase()];
                if (color3) return (color3 && color3.hex) || color3 || "";
                else {
                  const item4 = item.replace(/_/g, "-");
                  const color4 =
                    DiscordColors[item4] ||
                    DiscordColors[item4.toLowerCase()] ||
                    DiscordColors[item4.toUpperCase()];
        ...
        Internal.DiscordObjects = new Proxy(DiscordObjects, {
          get: function (_, item) {
            ...
            let defaultExport =
              InternalData.DiscordObjects[item].exported == undefined
                ? true
                : InternalData.DiscordObjects[item].exported;
            if (InternalData.DiscordObjects[item].props)
              DiscordObjects[item] = BDFDB.ModuleUtils.findByPrototypes(
                InternalData.DiscordObjects[item].props,
                { defaultExport },
              );
            else if (InternalData.DiscordObjects[item].strings)
              DiscordObjects[item] = BDFDB.ModuleUtils.findByString(
                InternalData.DiscordObjects[item].strings,
                { defaultExport },
              );
        ...
        Internal.LibraryRequires = new Proxy(LibraryRequires, {
          get: function (_, item) {
            ...
            try {
              LibraryRequires[item] = require(item);
            } catch (err) {}
        ...
        Internal.LibraryStores = new Proxy(LibraryStores, {
          get: function (_, item) {
            ...
            LibraryStores[item] = BDFDB.ModuleUtils.find(
              (m) =>
                m &&
                m.constructor &&
                typeof m.constructor.displayName == "string" &&
                m.constructor.displayName == item &&
                m,
            );
            if (!LibraryStores[item])
              BDFDB.LogUtils.warn([item, "could not be found in Webmodule Stores"]);
            ...
            if (!callback) {
              for (let listener of plugin.changeListeners) {
                ...
                if (removedListeners.length)
                  plugin.changeListeners = plugin.changeListeners.filter(
                    (listener) => !removedListeners.includes(listener),
                  );
              }
            } else if (typeof callback == "function") {
              store.removeChangeListener(callback);
              plugin.changeListeners = plugin.changeListeners.filter(
                (listener) => listener.store == store && listener.callback == callback,
              );
            }
          }
        };
        ...
        BDFDB.ListenerUtils.add = function (
          plugin,
          ele,
          actions,
          selectorOrCallback,
          ...
          if (
            !BDFDB.ObjectUtils.is(plugin) ||
            (!Node.prototype.isPrototypeOf(ele) && ele !== window) ||
            !actions
          )
            return;
          ...
          for (let action of actions.split(" ")) {
            ...
            eventName =
              eventName == "mouseenter" || eventName == "mouseleave" ? "mouseover" : eventName;
            ...
            if (selector) {
              if (origEventName == "mouseenter" || origEventName == "mouseleave")
                eventCallback = (e) => {
                  if (e.composedPath)
                    for (let child of e.composedPath())
                      if (
                        typeof child.matches == "function" &&
                        child.matches(selector) &&
                        !child[namespace + "BDFDB" + origEventName]
                      ) {
                        child[namespace + "BDFDB" + origEventName] = true;
                        if (origEventName == "mouseenter")
                          callback(BDFDB.ListenerUtils.copyEvent(e, child));
                        let mouseOut = (e2) => {
                          if (
                            e2.target.contains(child) ||
                            e2.target == child ||
                            !child.contains(e2.target)
                          ) {
                            if (origEventName == "mouseleave")
                              callback(BDFDB.ListenerUtils.copyEvent(e, child));
            ...
            if (Node.prototype.isPrototypeOf(ele)) {
              observer = new MutationObserver((changes) =>
                changes.forEach((change) => {
                  ...
                  if (nodes.indexOf(ele) > -1 || nodes.some((n) => n.contains(ele)))
                    BDFDB.ListenerUtils.remove(plugin, ele, actions, selector);
                }),
              );
          ...
          if (!ele) {
            ...
            for (let action of actions.split(" ")) {
              ...
              for (let listener of plugin.eventListeners) {
                ...
                if (
                  listener.ele == ele &&
                  (!eventName || listener.origEventName == eventName) &&
                  listener.namespace == namespace &&
                  (selector === undefined || listener.selector == selector)
                ) {
                ...
                if (removedListeners.length)
                  plugin.eventListeners = plugin.eventListeners.filter(
                    (listener) => !removedListeners.includes(listener),
                  );
              }
            }
        ...
        const leftSideMap = { 16: 160, 17: 170, 18: 164 };
        BDFDB.ListenerUtils.addGlobal = function (plugin, id, keybind, action) {
          plugin = (plugin == BDFDB && Internal) || plugin;
          if (
            !BDFDB.ObjectUtils.is(plugin) ||
            !id ||
            !BDFDB.ArrayUtils.is(keybind) ||
            typeof action != "function"
          )
            return;
          if (!BDFDB.ObjectUtils.is(plugin.globalKeybinds)) plugin.globalKeybinds = {};
          BDFDB.ListenerUtils.removeGlobal(plugin, id);
          plugin.globalKeybinds[id] = BDFDB.NumberUtils.generateId(
            Object.entries(plugin.globalKeybinds).map((n) => n[1]),
          );
          BDFDB.LibraryModules.WindowUtils.inputEventRegister(
            plugin.globalKeybinds[id],
            keybind.map((n) => [0, n]),
            action,
            { blurred: true, focused: true, keydown: false, keyup: true },
          );
          if (Object.keys(leftSideMap).some((key) => keybind.indexOf(parseInt(key)) > -1)) {
            const alternativeId = id + "___ALTERNATIVE";
            plugin.globalKeybinds[alternativeId] = BDFDB.NumberUtils.generateId(
              Object.entries(plugin.globalKeybinds).map((n) => n[1]),
            );
            BDFDB.LibraryModules.WindowUtils.inputEventRegister(
              plugin.globalKeybinds[alternativeId],
              keybind.map((n) => [0, leftSideMap[n] ?? n]),
              action,
              { blurred: true, focused: true, keydown: false, keyup: true },
            );
          }
        ...
        BDFDB.ListenerUtils.removeGlobal = function (plugin, id) {
          ...
          if (!id) {
            for (let cachedId in plugin.globalKeybinds)
              BDFDB.LibraryModules.WindowUtils.inputEventUnregister(
                plugin.globalKeybinds[cachedId],
              );
            plugin.globalKeybinds = {};
          } else {
            if (plugin.globalKeybinds[id])
              BDFDB.LibraryModules.WindowUtils.inputEventUnregister(plugin.globalKeybinds[id]);
            delete plugin.globalKeybinds[id];
            const alternativeId = id + "___ALTERNATIVE";
            if (plugin.globalKeybinds[alternativeId])
              BDFDB.LibraryModules.WindowUtils.inputEventUnregister(
                plugin.globalKeybinds[alternativeId],
              );
        ...
        BDFDB.ListenerUtils.multiAdd = function (node, actions, callback) {
          if (!Node.prototype.isPrototypeOf(node) || !actions || typeof callback != "function")
            return;
          for (let action of actions
            .trim()
            .split(" ")
            .filter((n) => n))
            node.addEventListener(action, callback, true);
        };
        BDFDB.ListenerUtils.multiRemove = function (node, actions, callback) {
          ...
          for (let action of actions
            ...
            .split(" ")
            .filter((n) => n))
            node.removeEventListener(action, callback, true);
        };
        BDFDB.ListenerUtils.addToChildren = function (node, actions, selector, callback) {
          if (
            !Node.prototype.isPrototypeOf(node) ||
            !actions ||
            !selector ||
            !selector.trim() ||
            typeof callback != "function"
          ...
          for (let action of actions
            ...
            if (action == "mouseenter" || action == "mouseleave")
              eventCallback = (e) => {
                if (e.target.matches(selector)) callback(e);
              };
            node
              .querySelectorAll(selector.trim())
              .forEach((child) => child.addEventListener(action, eventCallback, true));
          }
        };
        ...
        BDFDB.ListenerUtils.stopEvent = function (e) {
          if (BDFDB.ObjectUtils.is(e)) {
            ...
            if (BDFDB.ObjectUtils.is(e.originalEvent)) {
              ...
              if (typeof e.originalEvent.stopImmediatePropagation == "function")
        ...
        var Toasts = [],
          NotificationBars = [];
        var ToastQueues = {},
          DesktopNotificationQueue = { queue: [], running: false };
        for (let key in Internal.DiscordConstants.ToastPositions)
          ToastQueues[Internal.DiscordConstants.ToastPositions[key]] = { queue: [], full: false };
        ...
        BDFDB.NotificationUtils.toast = function (children, config = {}) {
          ...
          if (!app) return;
          let position =
            (config.position && Internal.DiscordConstants.ToastPositions[config.position]) ||
            (Internal.settings.choices.toastPosition &&
              Internal.DiscordConstants.ToastPositions[Internal.settings.choices.toastPosition]) ||
            Internal.DiscordConstants.ToastPositions.right;
          ...
          const runQueue = (_) => {
            ...
            if (!toasts) {
              toasts = BDFDB.DOMUtils.create(
                `<div class="${BDFDB.DOMUtils.formatClassName(BDFDB.disCN.toasts, BDFDB.disCN[position])}"></div>`,
              );
            ...
            if (data.config.css)
              BDFDB.DOMUtils.appendLocalStyle("BDFDBcustomToast" + id, data.config.css);
            if (data.config.style)
              data.toast.style = Object.assign({}, data.toast.style, data.config.style);

            let backgroundColor, fontColor, barColor;

            let type = data.config.type && BDFDB.disCN["toast" + data.config.type];
            if (!type) {
              barColor = BDFDB.ColorUtils.getType(data.config.barColor)
                ? BDFDB.ColorUtils.convert(data.config.barColor, "HEX")
            ...
            let disableInteractions =
              data.config.disableInteractions && typeof data.config.onClick != "function";
            let timeout =
              typeof data.config.timeout == "number" && !disableInteractions
                ? data.config.timeout
                : 3000;
            timeout = (timeout > 0 ? timeout : 600000) + 300;
            if (data.config.ellipsis && typeof data.children == "string")
              loadingInterval = BDFDB.TimeUtils.interval(
                (_) =>
                  data.toast.update(
                    data.children.endsWith(".....")
            ...
            if (disableInteractions)
              ...
              data.toast.addEventListener("click", (event) => {
                if (
                  typeof data.config.onClick == "function" &&
                  !BDFDB.DOMUtils.getParent(BDFDB.dotCN.toastcloseicon, event.target)
                )
                  data.config.onClick();
                data.toast.close();
              });
            ...
            BDFDB.TimeUtils.timeout((_) =>
              BDFDB.DOMUtils.removeClass(data.toast, BDFDB.disCN.toastopening),
            );

            let icon = data.config.avatar
              ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Avatars.Avatar, {
                  src: data.config.avatar,
                  size: Internal.LibraryComponents.AvatarConstants.AvatarSizes.SIZE_24,
                })
              : data.config.icon ||
                  (data.config.type && Internal.DiscordConstants.ToastIcons[data.config.type])
                ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                    name:
                      data.config.type &&
                      Internal.DiscordConstants.ToastIcons[data.config.type] &&
                      Internal.LibraryComponents.SvgIcon.Names[
                        Internal.DiscordConstants.ToastIcons[data.config.type]
                      ],
            ...
            BDFDB.ReactUtils.render(
              BDFDB.ReactUtils.createElement(
                class BDFDB_Toast extends Internal.LibraryModules.React.Component {
                  componentDidMount() {
                  ...
                  render() {
                    return BDFDB.ReactUtils.createElement(Internal.LibraryModules.React.Fragment, {
                      children: [
                            ...
                            icon &&
                              BDFDB.ReactUtils.createElement("div", {
                                className: BDFDB.DOMUtils.formatClassName(
                                  data.config.avatar && BDFDB.disCN.toastavatar,
                                  BDFDB.disCN.toasticon,
                                  data.config.iconClassName,
                                ),
                                children: icon,
                              }),
                            BDFDB.ReactUtils.createElement("div", {
                              className: BDFDB.DOMUtils.formatClassName(
                                BDFDB.disCN.toasttext,
                                data.config.textClassName,
                              ),
                        ...
                        BDFDB.ReactUtils.createElement("div", {
                          className: BDFDB.DOMUtils.formatClassName(
                            BDFDB.disCN.toastbar,
                            barColor && BDFDB.disCN.toastcustombar,
                          ),
                          style: {
        ...
        BDFDB.NotificationUtils.notice = function (text, config = {}) {
          ...
          let notice = BDFDB.DOMUtils.create(
            `<div class="${BDFDB.disCNS.notice + BDFDB.disCN.noticewrapper}"><div class="${BDFDB.disCN.noticedismiss}"><svg class="${BDFDB.disCN.noticedismissicon}" aria-hidden="false" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M18.4 4L12 10.4L5.6 4L4 5.6L10.4 12L4 18.4L5.6 20L12 13.6L18.4 20L20 18.4L13.6 12L20 5.6L18.4 4Z"></path></svg></div><div class="${BDFDB.disCN.noticetext}"></div></div>`,
          );
          layers.parentElement.insertBefore(notice, layers);
          let noticeText = notice.querySelector(BDFDB.dotCN.noticetext);
          if (config.platform)
            for (let platform of config.platform.split(" "))
              if (InternalData.DiscordClasses["noticeicon" + platform]) {
                let icon = BDFDB.DOMUtils.create(
                  `<i class="${BDFDB.disCN["noticeicon" + platform]}"></i>`,
                );
          ...
          if (config.customIcon) {
            let icon = document.createElement("i"),
              iconInner = BDFDB.DOMUtils.create(config.customIcon);
            if (iconInner.nodeType == Node.TEXT_NODE)
              icon.style.setProperty(
                "background",
                `url(${config.customIcon}) center/cover no-repeat`,
              );
            else {
              icon = iconInner;
          ...
          if (BDFDB.ArrayUtils.is(config.buttons))
            for (let data of config.buttons) {
              let contents = typeof data.contents == "string" && data.contents;
              if (contents) {
                let button = BDFDB.DOMUtils.create(
                  `<button class="${BDFDB.DOMUtils.formatClassName(BDFDB.disCN.noticebutton, data.className)}">${contents}</button>`,
                );
                button.addEventListener("click", (event) => {
                  if (data.close) notice.close();
                  if (typeof data.onClick == "function") data.onClick(event, notice);
                });
                if (typeof data.onMouseEnter == "function")
                  button.addEventListener("mouseenter", (event) =>
                    data.onMouseEnter(event, notice),
                  );
                if (typeof data.onMouseLeave == "function")
                  button.addEventListener("mouseleave", (event) =>
                    data.onMouseLeave(event, notice),
                  );
          ...
          if (config.css)
            BDFDB.DOMUtils.appendLocalStyle("BDFDBcustomNotificationBar" + id, config.css);
            ...
            for (let word of text.split(" ")) {
              ...
              newText.push(
                link.host && link.host !== window.location.host
                  ? `<label class="${BDFDB.disCN.noticetextlink}">${encodedWord}</label>`
                  : encodedWord,
              );
            }
          ...
          if (
            config.type &&
            !document.querySelector(BDFDB.dotCNS.chatbase + BDFDB.dotCN.noticestreamer)
          ) {
            ...
            if (config.type == "premium") {
              ...
              if (noticeButton)
                BDFDB.DOMUtils.addClass(noticeButton, BDFDB.disCN.noticepremiumaction);
              BDFDB.DOMUtils.addClass(noticeText, BDFDB.disCN.noticepremiumtext);
              notice.insertBefore(
                BDFDB.DOMUtils.create(`<i class="${BDFDB.disCN.noticepremiumlogo}"></i>`),
                noticeText,
              );
            }
          }
          if (!type) {
            ...
            if (comp) {
              ...
              notice.style.setProperty("--custom-notice-background", backgroundColor);
              notice.style.setProperty("--custom-notice-text", fontColor);
              notice.style.setProperty("--custom-notice-button-hover", fontColor);
          ...
          notice.close = (_) => {
            ...
            if (notice.tooltip && typeof notice.tooltip.removeTooltip == "function")
              notice.tooltip.removeTooltip();
            BDFDB.TimeUtils.timeout((_) => {
        ...
        BDFDB.NotificationUtils.alert = function (header, body) {
          if (
            ...
            typeof header == "string" &&
            BdApi &&
            typeof BdApi.UI.alert == "function"
          )
            BdApi.UI.alert(header, body);
        };
        ...
        BDFDB.TooltipUtils.create = function (anker, text, config = {}) {
          ...
          const itemLayerContainer =
            document.querySelector(
              `${BDFDB.dotCN.app} ~ ${BDFDB.dotCN.itemlayercontainer}:has(${BDFDB.notCN.pictureinpicture})`,
            ) ||
            document.querySelector(
              `${BDFDB.dotCN.app} ~ ${BDFDB.dotCN.itemlayercontainer}:empty`,
            ) ||
            document.querySelector(`${BDFDB.dotCN.app} ~ ${BDFDB.dotCN.itemlayercontainer}`) ||
            document.querySelector(BDFDB.dotCN.itemlayercontainer);
          if (
            !itemLayerContainer ||
            !Node.prototype.isPrototypeOf(anker) ||
            !document.contains(anker)
          )
            return null;
          const id = BDFDB.NumberUtils.generateId(Tooltips);
          const wrapper = BDFDB.DOMUtils.create(
            `<div class="${BDFDB.disCN.itemlayercontainerclicktrap}"><div class="${BDFDB.disCNS.itemlayer + BDFDB.disCN.itemlayerdisabledpointerevents}"><div class="${BDFDB.disCN.tooltip}" tooltip-id="${id}"><div class="${BDFDB.disCN.tooltipcontent}"></div><div class="${BDFDB.disCNS.tooltippointer + BDFDB.disCN.tooltippointerbg}"></div><div class="${BDFDB.disCN.tooltippointer}"></div></div></div></div>`,
          );
          ...
          if (typeof config.type != "string" || !BDFDB.disCN["tooltip" + config.type.toLowerCase()])
            config.type = "top";
          ...
          let fontColorIsGradient = false,
            customBackgroundColor = false,
            style = "";
          if (config.style) style += config.style;
          if (config.fontColor) {
            fontColorIsGradient = BDFDB.ObjectUtils.is(config.fontColor);
            if (!fontColorIsGradient)
              style =
                (style ? style + " " : "") +
                `color: ${BDFDB.ColorUtils.convert(config.fontColor, "RGBA")} !important;`;
          }
          if (config.backgroundColor) {
            customBackgroundColor = true;
            let backgroundColorIsGradient = BDFDB.ObjectUtils.is(config.backgroundColor);
            let backgroundColor = !backgroundColorIsGradient
              ? BDFDB.ColorUtils.convert(config.backgroundColor, "RGBA")
              : BDFDB.ColorUtils.createGradient(config.backgroundColor);
            let borderColor = backgroundColorIsGradient
              ? BDFDB.ColorUtils.convert(config.backgroundColor[type == "left" ? 100 : 0], "RGBA")
              : backgroundColor;
            style =
              (style ? style + " " : "") +
              `background: ${backgroundColor} !important; --tooltip-pointer-bg: ${borderColor} !important;`;
          }
          ...
          if (zIndexed) {
            ...
            BDFDB.DOMUtils.addClass(
              itemLayerContainer,
              BDFDB.disCN.itemlayercontainerzindexdisabled,
            );
          }
          ...
          else if (config.color && BDFDB.disCN["tooltip" + config.color.toLowerCase()])
            BDFDB.DOMUtils.addClass(tooltip, BDFDB.disCN["tooltip" + config.color.toLowerCase()]);
          else BDFDB.DOMUtils.addClass(tooltip, BDFDB.disCN.tooltipprimary);

          if (config.list || BDFDB.ObjectUtils.is(config.guild))
            BDFDB.DOMUtils.addClass(tooltipContent, BDFDB.disCN.guildlistitemtooltip);
          ...
          const setText = (newText) => {
            if (BDFDB.ObjectUtils.is(config.guild)) {
              ...
              let muteConfig = Internal.LibraryStores.UserGuildSettingsStore.getMuteConfig(
                config.guild.id,
              );

              let children = [typeof newText == "function" ? newText() : newText]
                .flat(10)
                .filter((n) => typeof n == "string" || BDFDB.ReactUtils.isValidElement(n));
              BDFDB.ReactUtils.render(
                BDFDB.ReactUtils.createElement(Internal.LibraryModules.React.Fragment, {
                  children: [
                    BDFDB.ReactUtils.createElement("div", {
                      className: BDFDB.DOMUtils.formatClassName(
                        BDFDB.disCN.tooltiprow,
                        BDFDB.disCN.tooltiprowguildname,
                      ),
                      children: [
                        ...
                        BDFDB.ReactUtils.createElement("span", {
                          ...
                          children: fontColorIsGradient
                            ? BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.TextGradientElement,
                                {
                    ...
                    config.note &&
                      BDFDB.ReactUtils.createElement("div", {
                        className: BDFDB.DOMUtils.formatClassName(
                          BDFDB.disCN.tooltiprow,
                          BDFDB.disCN.tooltiprowextra,
                          BDFDB.disCN.tooltipnote,
                        ),
                        children: config.note,
                      }),
                    BDFDB.ReactUtils.createElement(Internal.LibraryComponents.GuildVoiceList, {
                      guild: config.guild,
                    }),
                    isMuted &&
                      muteConfig &&
                      (muteConfig.end_time == null
                        ...
                        : BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.GuildTooltipMutedText,
                            {
                              className: BDFDB.DOMUtils.formatClassName(
          ...
          const update = (newText) => {
            ...
            if (type == "top" || type == "bottom") {
              if (left < 0) {
                ...
                for (let pointer of tooltipPointers)
                  pointer.style.setProperty("margin-left", `${left - 10}px`, "important");
              } else {
                const rightMargin = aRects.width - (left + iRects.width);
                if (rightMargin < 0) {
                  itemLayer.style.setProperty(
                    "left",
                    `${aRects.width - iRects.width - 5}px`,
                    "important",
                  );
                  for (let pointer of tooltipPointers)
                    pointer.style.setProperty("margin-left", `${-1 * rightMargin}px`, "important");
                }
              }
            } else if (type == "left" || type == "right") {
              if (top < 0) {
                const bRects = BDFDB.DOMUtils.getRects(
                  document.querySelector(BDFDB.dotCN.titlebar),
                );
                const barCorrection =
                  (bRects.width || 0) >= Math.round((75 * window.outerWidth) / aRects.width)
                    ? bRects.height + 5
                    : 0;
                ...
                if (bottomMargin < 0) {
                  itemLayer.style.setProperty(
                    "top",
                    `${aRects.height - iRects.height - 5}px`,
                    "important",
                  );
                  for (let pointer of tooltipPointers)
                    pointer.style.setProperty("margin-top", `${-1 * bottomMargin}px`, "important");
                }
              }
            }
          };
          ...
          const observer = new MutationObserver((changes) =>
            changes.forEach((change) => {
              const nodes = Array.from(change.removedNodes);
              if (
                nodes.indexOf(wrapper) > -1 ||
                nodes.indexOf(anker) > -1 ||
                nodes.some((n) => n.contains(anker))
              )
                removeTooltip();
            }),
          );
        ...
        Internal.addModulePatches = function (plugin) {
          ...
          for (let patchType in plugin.modulePatches) {
            ...
            for (let type of plugin.modulePatches[patchType]) {
              if (!InternalData.PatchModules[type]) {
                BDFDB.LogUtils.warn(`[${type}] not found in PatchModules InternalData`, plugin);
                continue;
              }
              if (
                !InternalData.PatchModules[type].noSearch &&
                (patchType == "before" || patchType == "after")
              )
                searchEntries.push({ patchType, type });
                  ...
                  if (isSearchable(m, true)) {
                    for (let key of Object.keys(m)) {
                      try {
                        if (
                          m[key] &&
                          isSearchable(m[key], true) &&
                          Internal.isCorrectModule(m[key], type)
                        ) {
                          valid = true;
                          break;
                        }
                    ...
                    if (!valid && typeof m == "function" && Internal.isCorrectModule(m, type))
                      valid = true;
                    if (
                      !valid &&
                      req.m[modId] &&
                      isSearchable(req.m[modId]) &&
                      Internal.isCorrectModule(req.m[modId], type)
                    )
                      valid = true;
                    if (valid) {
                      foundExports[type] = m;
              ...
              for (let i in req.c) {
                ...
                let searchableKeys =
                  mKeys.length < 400
                    ? mKeys.filter((key) => {
                        try {
                          return m[key] && isSearchable(m[key], true);
                        } catch (e) {
                          return false;
                        }
                      })
                    : null;
                for (let type of remaining) {
                  if (typeof m == "function" && Internal.isCorrectModule(m, type)) {
                    foundExports[type] = m;
                    moduleCache[type] = i;
                    cacheChanged = true;
                    remaining.delete(type);
                  ...
                  if (
                    m[type] &&
                    isSearchable(m[type], true) &&
                    Internal.isCorrectModule(m[type], type)
                  ) {
                    foundExports[type] = m;
                    moduleCache[type] = i;
                    cacheChanged = true;
                    ...
                    m.__esModule &&
                    m.default &&
                    isSearchable(m.default, true) &&
                    Internal.isCorrectModule(m.default, type)
                  ) {
                    foundExports[type] = m;
                    moduleCache[type] = i;
                    cacheChanged = true;
                ...
                if (remaining.size && searchableKeys)
                  for (let ki = 0; ki < searchableKeys.length && remaining.size; ki++) {
                    let fn = m[searchableKeys[ki]];
                    for (let type of remaining) {
                      if (Internal.isCorrectModule(fn, type)) {
                        foundExports[type] = m;
                        moduleCache[type] = i;
                        cacheChanged = true;
                        remaining.delete(type);
                ...
                for (let i in req.m)
                  if (req.m.hasOwnProperty(i)) {
                    ...
                    if (m && isSearchable(m) && req.c[i] && isSearchable(req.c[i].exports, true)) {
                      for (let type of remaining)
                        if (Internal.isCorrectModule(m, type)) {
                          foundExports[type] = req.c[i].exports;
                          moduleCache[type] = i;
                          cacheChanged = true;
                          remaining.delete(type);
                        }
          ...
          const registerPatch = (patchType, type) => {
            ...
            if (store[type][patchPriority].length > 1)
              store[type][patchPriority] = BDFDB.ArrayUtils.keySort(
                store[type][patchPriority],
                "name",
              );
          };
          for (let { patchType, type } of searchEntries) {
            let found = false,
              exports = foundExports[type];
            if (exports && !exports.default)
              for (let key of Object.keys(exports)) {
                ...
                if (
                  ...
                  Internal.isCorrectModule(fn, type, false) &&
                  fn.toString().length < 50000
                ) {
                  found = true;
        ...
        Internal.addContextPatches = function (plugin) {
          if (
            !InternalData.ContextMenuTypes ||
            !BdApi ||
            !BdApi.ContextMenu ||
            typeof BdApi.ContextMenu.patch != "function"
          )
            return;
          plugin = (plugin == BDFDB && Internal) || plugin;
          if (!plugin) return;
          for (let type in InternalData.ContextMenuTypes)
            if (typeof plugin[`on${type}`] == "function") {
              if (!BDFDB.ArrayUtils.is(plugin.patchCancels)) plugin.patchCancels = [];
              plugin.patchCancels.push(
                BdApi.ContextMenu.patch(
                  InternalData.ContextMenuTypes[type],
                  (returnValue, props) =>
                    typeof plugin[`on${type}`] == "function" &&
                    plugin[`on${type}`]({ returnvalue: returnValue, instance: { props } }),
                ),
              );
            }
        };
          ...
          if (BDFDB.ObjectUtils.is(plugin) && !plugin.stopping && e.instance) {
            ...
            if (typeof plugin[`process${type}`] == "function") {
              if (
                ...
                ["componentDidMount", "componentDidUpdate", "componentWillUnmount"].indexOf(
                  e.methodname,
                ) > -1
              ) {
        ...
        BDFDB.PatchUtils.isPatched = function (plugin, module, methodName) {
          ...
          if (
            !plugin ||
            !methodName ||
            (!BDFDB.ObjectUtils.is(module) && !BDFDB.ArrayUtils.is(module)) ||
            !module[methodName] ||
            !module[methodName].BDFDB_Patches
          )
            return false;
          const pluginId = (typeof plugin === "string" ? plugin : plugin.name).toLowerCase();
          return (
            pluginId &&
            BDFDB.ObjectUtils.toArray(module[methodName].BDFDB_Patches).some((patchObj) =>
              BDFDB.ObjectUtils.toArray(patchObj.plugins).some((priorityObj) =>
                Object.keys(priorityObj).includes(pluginId),
              ),
            )
          );
        };
        BDFDB.PatchUtils.patch = function (plugin, module, methodNames, patchMethods, config = {}) {
          ...
          if (
            ...
            (!BDFDB.ObjectUtils.is(module) && !BDFDB.ArrayUtils.is(module)) ||
            !methodNames ||
            !BDFDB.ObjectUtils.is(patchMethods)
          )
            return null;
          patchMethods = BDFDB.ObjectUtils.filter(
            patchMethods,
            (type) =>
              PatchTypes.includes(type) &&
              typeof BdApi.Patcher[type] == "function" &&
              typeof patchMethods[type] == "function",
            true,
          );
          ...
          let patchPriority = !isNaN(config.priority)
            ? config.priority
            : BDFDB.ObjectUtils.is(plugin) && !isNaN(plugin.patchPriority)
          ...
          for (let methodName of methodNames)
            if (module[methodName] == null || typeof module[methodName] == "function") {
              ...
              for (let type in patchMethods) {
                const internalData = (Object.entries(InternalData.LibraryModules).find(
                  (n) =>
                    n &&
                    n[0] &&
                    LibraryModules[n[0]] == module &&
                    n[1] &&
                    n[1]._originalModule &&
                    n[1]._mappedItems[methodName],
                ) || [])[1];
                const name =
                ...
                try {
                  if (!patches[type]) {
                    const originalMethod =
                      module[methodName].__originalFunction || module[methodName];
                    const mainCancel = BdApi.Patcher[type](
                      Internal.name,
                      (internalData && internalData._originalModule) || module,
                      (internalData && internalData._mappedItems[methodName]) || methodName,
                      function (...args) {
                        let callInsteadAfterwards = false,
                          stopInsteadCall = false;
                        const data = {
                          component: module,
                        ...
                        if (type == "instead") {
                          data.callOriginalMethod = (_) =>
                            (data.returnValue = data.originalMethod.apply(
                              this && this !== window ? this : {},
                              data.methodArguments,
                            ));
                          data.callOriginalMethodAfterwards = (_) => (
                            (callInsteadAfterwards = true), data.returnValue
                          );
                          data.stopOriginalMethodCall = (_) => (stopInsteadCall = true);
                        }
                        if (args[0] != module) data.instance = args[0] || { props: args[1][0] };
                        if (module == LibraryModules.MessageToolbarUtils)
                          data.instance = { props: args[1][0] };
                        for (let priority in patches[type].plugins)
                          for (let id in BDFDB.ObjectUtils.sort(patches[type].plugins[priority])) {
                            let tempReturn = BDFDB.TimeUtils.suppress(
                              patches[type].plugins[priority][id],
                              `"${type}" callback of ${methodName} in ${name}`,
                              {
                                name: patches[type].plugins[priority][id].pluginName,
                                version: patches[type].plugins[priority][id].pluginVersion,
                                ignoreErrors: config.ignoreErrors,
                              },
                            )(data);
                            if (type != "before" && tempReturn !== undefined)
                              data.returnValue = tempReturn;
                          }
                        if (type == "instead" && callInsteadAfterwards && !stopInsteadCall)
                          BDFDB.TimeUtils.suppress(
                            data.callOriginalMethod,
                            `originalMethod of ${methodName} in ${name}`,
                            { name: "Discord", ignoreErrors: config.ignoreErrors },
                          )();

                        if (type != "before")
                          return (methodName == "render" || methodName == "type") &&
                    ...
                    patches[type] = {
                      ...
                      cancel: (_) => {
                        if (!config.noCache)
                          BDFDB.ArrayUtils.remove(
                            Internal.patchCancels,
                            patches[type].cancel,
                            true,
                          );
                        delete patches[type];
                        if (!config.noCache && BDFDB.ObjectUtils.isEmpty(patches))
                          delete module[methodName].BDFDB_Patches;
                        mainCancel();
                      },
                    };
                    if (!config.noCache) {
                  ...
                  if (!patches[type].plugins[patchPriority])
                    patches[type].plugins[patchPriority] = {};
                  ...
                  BDFDB.LogUtils.error(
                    [
                      "Could not patch Component!",
                      `"${type}" Patch of ${methodName} in ${name}`,
                      err,
                    ],
                    plugin,
                  );
        ...
        BDFDB.PatchUtils.unpatch = function (plugin, module, methodNames) {
          ...
          if (!module || !methodNames) {
            ...
            const pluginId = !plugin
              ? null
              : (typeof plugin === "string" ? plugin : plugin.name).toLowerCase();
            for (let methodName of [methodNames].flat(10).filter((n) => n))
              if (module[methodName] && module[methodName].BDFDB_Patches) {
                let patches = module[methodName].BDFDB_Patches;
                for (let type in patches) {
                  if (pluginId)
                    for (let priority in patches[type].plugins) {
                      delete patches[type].plugins[priority][pluginId];
                      if (BDFDB.ObjectUtils.isEmpty(patches[type].plugins[priority]))
                        delete patches[type].plugins[priority];
                    }
        ...
        Internal.forceUpdate = function (pluginDataObjs, instance, type) {
          ...
          if (pluginDataObjs.length && instance && type) {
            ...
            for (let pluginData of pluginDataObjs) {
              let plugin = (pluginData.plugin == BDFDB && Internal) || pluginData.plugin,
                methodNames = [];
              for (let patchType in plugin.modulePatches) {
                if (plugin.modulePatches[patchType].indexOf(type) > -1) methodNames.push(patchType);
              }
              methodNames = BDFDB.ArrayUtils.removeCopies(methodNames)
                .flat(10)
                .filter((n) => n);
              if (methodNames.indexOf("componentDidMount") > -1)
                Internal.initiatePatch(plugin, type, {
                  arguments: [],
                  instance: instance,
              ...
              if (methodNames.indexOf("before") > -1 || methodNames.indexOf("after") > -1)
                forceRender = true;
              else if (!forceRender && methodNames.includes("componentDidUpdate") > -1)
                Internal.initiatePatch(plugin, type, {
                  arguments: [],
                  instance: instance,
                  returnvalue: undefined,
        ...
        BDFDB.PatchUtils.forceAllUpdates = function (plugins, selectedTypes) {
          plugins = [plugins]
            .flat(10)
            .map((n) => (n == BDFDB && Internal) || n)
            .filter((n) => BDFDB.ObjectUtils.is(n.modulePatches));
          ...
          let toBeUpdatedModules = BDFDB.ArrayUtils.removeCopies(
            BDFDB.ObjectUtils.toArray(toBeUpdatedModulesObj).flat(10),
          );
          if (!BDFDB.ArrayUtils.sum(toBeUpdatedModules.map((n) => n.length))) return;
          try {
            const appInsDown = BDFDB.ReactUtils.findOwner(app, {
              name: toBeUpdatedModules,
              all: true,
              unlimited: true,
              group: true,
            });
            ...
            for (let type in appInsDown) {
              let filteredPlugins = plugins
                .filter((n) => toBeUpdatedModulesObj[n.name].includes(type))
                .map((n) => ({ plugin: n, patchTypes: patchTypes[n.name] }));
        ...
        Internal.isCorrectModule = function (module, type, useCache = false) {
          if (!InternalData.PatchModules || !InternalData.PatchModules[type]) return false;
            ...
            Cache.modules &&
            Cache.modules.patch &&
            Cache.modules.patch[type] == module
          )
            return true;
          else {
            let foundModule = null;
            if (InternalData.PatchModules[type].strings)
              foundModule = Internal.checkModuleStrings(
                module._originalFunction || module,
                InternalData.PatchModules[type].strings,
              )
                ? module
                : null;
            if (InternalData.PatchModules[type].props)
              foundModule = Internal.checkModuleProps(module, InternalData.PatchModules[type].props)
                ? module
                : null;
            if (InternalData.PatchModules[type].protos)
              foundModule = Internal.checkModuleProtos(
                module,
                InternalData.PatchModules[type].protos,
              )
                ? module
                : null;
            if (foundModule) {
              if (InternalData.PatchModules[type].nonStrings)
                foundModule = Internal.checkModuleStrings(
                  module._originalFunction || module,
                  InternalData.PatchModules[type].nonStrings,
                  { hasNot: true },
                )
                  ? module
                  : null;
              if (InternalData.PatchModules[type].nonProps)
                foundModule = Internal.checkModuleProps(
                  module,
                  InternalData.PatchModules[type].nonProps,
                  { hasNot: true },
                )
                  ? module
                  : null;
              if (InternalData.PatchModules[type].nonProtos)
                foundModule = Internal.checkModuleProtos(
                  module,
                  InternalData.PatchModules[type].nonProtos,
                  { hasNot: true },
                )
                  ? module
                  : null;
        ...
        Internal.isCorrectModuleButDontPatch = function (type) {
          if (type == "MessageToolbar" && document.querySelector(BDFDB.dotCN.emojipicker))
            return true;
          return false;
        };
        Internal.findModuleViaData = (moduleStorage, dataStorage, item) => {
          ...
          let defaultExport =
            typeof dataStorage[item].exported != "boolean" ? true : dataStorage[item].exported;
          if (dataStorage[item].props)
            moduleStorage[item] = BDFDB.ModuleUtils.findByProperties(dataStorage[item].props, {
              defaultExport: defaultExport,
              moduleName: item,
            });
          else if (dataStorage[item].protos)
            moduleStorage[item] = BDFDB.ModuleUtils.findByPrototypes(dataStorage[item].protos, {
              defaultExport: defaultExport,
              moduleName: item,
            });
          else if (dataStorage[item].name)
            moduleStorage[item] = BDFDB.ModuleUtils.findByName(dataStorage[item].name, {
              defaultExport: defaultExport,
              moduleName: item,
            });
          else if (dataStorage[item].strings) {
            if (dataStorage[item].nonStrings) {
              moduleStorage[item] = Internal.findModule(
                "strings + nonStrings",
                JSON.stringify([dataStorage[item].strings, dataStorage[item].nonStrings].flat(10)),
                (m) =>
                  Internal.checkModuleStrings(m, dataStorage[item].strings) &&
                  Internal.checkModuleStrings(m, dataStorage[item].nonStrings, { hasNot: true }) &&
                  m,
                { defaultExport: defaultExport, moduleName: item },
              );
            } else
              moduleStorage[item] = BDFDB.ModuleUtils.findByString(dataStorage[item].strings, {
                defaultExport: defaultExport,
                moduleName: item,
              });
          }
          if (dataStorage[item].value)
            moduleStorage[item] = (moduleStorage[item] || {})[dataStorage[item].value];
          if (dataStorage[item].assign)
            moduleStorage[item] = Object.assign({}, moduleStorage[item]);
          if (!moduleStorage[item]) return;
          if (moduleStorage[item][item]) moduleStorage[item] = moduleStorage[item][item];
          if (dataStorage[item].funcStrings)
            moduleStorage[item] = (Object.entries(moduleStorage[item]).find((n) => {
              if (!n || !n[1]) return;
              let funcString =
                typeof n[1] == "function"
                  ? n[1].toString()
              ...
              let renderFuncString =
                (typeof n[1].render == "function" && n[1].render.toString()) || "";
              return (
                (funcString || renderFuncString) &&
                [dataStorage[item].funcStrings]
                  .flat(10)
                  .filter((s) => s && typeof s == "string")
                  .every(
                    (string) =>
                      (funcString && funcString.indexOf(string) > -1) ||
                      (renderFuncString && renderFuncString.indexOf(string) > -1),
                  )
              );
            }) || [])[1];
          if (dataStorage[item].map) {
            ...
            moduleStorage[item] = new Proxy(
              Object.assign({}, dataStorage[item]._originalModule, dataStorage[item].map),
              {
                get: function (_, item2) {
                  if (dataStorage[item]._originalModule[item2])
                    return dataStorage[item]._originalModule[item2];
                  if (dataStorage[item]._mappedItems[item2])
                    return dataStorage[item]._originalModule[dataStorage[item]._mappedItems[item2]];
                  if (!dataStorage[item].map[item2])
                    return dataStorage[item]._originalModule[item2];
                  let foundFunc =
                    dataStorage[item].map[item2] &&
                    dataStorage[item].map[item2].length == 1 &&
                    dataStorage[item]._originalModule[dataStorage[item].map[item2][0]]
                      ? [
                          dataStorage[item].map[item2][0],
                          dataStorage[item]._originalModule[dataStorage[item].map[item2][0]],
                        ]
                      : Object.entries(dataStorage[item]._originalModule).find((n) => {
                          if (!n || !n[1]) return;
                          let funcString =
                            typeof n[1] == "function"
                              ? n[1].toString()
                          ...
                          let renderFuncString =
                            (typeof n[1].render == "function" && n[1].render.toString()) || "";
                          return [dataStorage[item].map[item2]]
                            .flat(10)
                            .filter((s) => s && typeof s == "string")
                            .every(
                              (string) =>
        ...
        const hasDiscordStringHash = (m) => {
          return (
            m &&
            [InternalData.LanguageStringHashes.DISCORD]
              .flat(10)
              .some((hash) => m[hash] && typeof m[hash] != "function")
          );
        };
        const LanguageStores = BDFDB.ModuleUtils.find(
          (m) => (hasDiscordStringHash(m) || hasDiscordStringHash(m.default)) && m,
          { all: true, defaultExport: false },
        );

        LibraryModules.EngLanguageStore =
          (
            LanguageStores.find((n) => n && n.exports && hasDiscordStringHash(n.exports.default)) ||
            {}
          ).exports || {};
        LibraryModules.EngLanguageStore =
          LibraryModules.EngLanguageStore.default || LibraryModules.EngLanguageStore;

        LibraryModules.LanguageStore =
          (
            LanguageStores.find((n) => n && hasDiscordStringHash(n.exports)) ||
            LanguageStores.find((n) => n && n.exports && hasDiscordStringHash(n.exports.default)) ||
            {}
          ).exports || {};
        LibraryModules.LanguageStore =
          LibraryModules.LanguageStore.default || LibraryModules.LanguageStore;

        LibraryModules.React = BDFDB.ModuleUtils.findByProperties("createElement", "cloneElement");
        LibraryModules.ReactDOM =
          BDFDB.ModuleUtils.findByProperties("render", "findDOMNode", { noWarnings: true }) ||
          BDFDB.ModuleUtils.findByProperties("createRoot");
        LibraryModules.ReactPortal = BDFDB.ModuleUtils.findByProperties(
          "flushSync",
          "createPortal",
        );
        ...
        Internal.LibraryModules = new Proxy(LibraryModules, {
          get: function (_, item) {
            ...
            if (item == "HTTPUtils") {
              ...
              if (APIUtils && InternalData.LibraryModules.HTTPUtils.props)
                LibraryModules[item] = (Object.entries(APIUtils).find((entry) =>
                  InternalData.LibraryModules.HTTPUtils.props.every(
                    (prop) => entry[1][prop] !== undefined,
                  ),
                ) || [])[1];
        ...
        LibraryModules.MessageToolbarUtils.useMessageMenu = (props, returnValue) => {
          return returnValue;
        };
        ...
        if (Internal.LibraryModules.KeyCodeUtils && Internal.LibraryModules.PlatformUtils) {
          ...
          let codeMap = BDFDB.ObjectUtils.invert(
            Internal.LibraryModules.PlatformUtils.isLinux &&
              Internal.LibraryModules.PlatformUtils.isLinux()
              ? Internal.DiscordConstants.LinuxKeyToCode
              : Internal.LibraryModules.PlatformUtils.isMac &&
                  Internal.LibraryModules.PlatformUtils.isMac()
                ? Internal.DiscordConstants.MacosKeyToCode
                : Internal.LibraryModules.PlatformUtils &&
                    Internal.LibraryModules.PlatformUtils.isWindows &&
                    Internal.LibraryModules.PlatformUtils.isWindows()
                  ? Internal.DiscordConstants.WindowsKeyToCode
                  : {},
          );
          let keyMap = [
            ["META", "⌘"],
          ...
          const getString = function (keyArray, upperCase = true) {
            let strings = [keyArray]
              .flat(10)
              .filter((n) => n)
              .map((keyCode) => {
                let code =
                  Internal.LibraryModules.KeyCodeUtils.keyToCode(
                    (Object.entries(Internal.LibraryModules.KeyEvents.codes).find(
                      (n) =>
                        n[1] == keyCode &&
                        Internal.LibraryModules.KeyCodeUtils.keyToCode(n[0], null),
                    ) || [])[0],
                    null,
                  ) || keyCode;
            ...
            if (!upperCase) return strings.join("+");
              ...
              strings =
                window.navigator.appVersion.indexOf("Mac OS X") != -1
                  ? strings.map(mapKeys)
                  : strings;
        ...
        MyReact.childrenToArray = function (parent) {
          if (
            ...
            !BDFDB.ArrayUtils.is(parent.props.children)
          ) {
        ...
        MyReact.createElement = function (
          ...
          if (component && component.defaultProps)
            for (let key in component.defaultProps)
              if (props[key] == null) props[key] = component.defaultProps[key];
          try {
            let child =
              Internal.LibraryModules.React.createElement(component || "div", props) || null;
            if (errorWrap)
              return (
                Internal.LibraryModules.React.createElement(
            ...
            !ignoreErrors && BDFDB.LogUtils.error(["Could not create React Element!", err]);
          }
          return null;
        };
        MyReact.objectToReact = function (obj) {
          if (!obj) return null;
            ...
            return BDFDB.ReactUtils.createElement(
              (obj.type && typeof obj.type == "function") || typeof obj.type == "string"
                ? obj.type
                : (obj.props && obj.props.href && "a") || "div",
              !obj.props
                ? {}
        ...
        MyReact.markdownParse = function (str) {
          ...
          if (!MyReact.markdownParse.parser || !MyReact.markdownParse.render) {
            MyReact.markdownParse.parser = Internal.LibraryModules.SimpleMarkdownParser.parserFor(
              Internal.LibraryModules.SimpleMarkdownParser.defaultRules,
            );
            MyReact.markdownParse.render = Internal.LibraryModules.SimpleMarkdownParser.reactFor(
              Internal.LibraryModules.SimpleMarkdownParser.ruleOutput(
                Internal.LibraryModules.SimpleMarkdownParser.defaultRules,
                "react",
              ),
            );
          }
          return MyReact.render && MyReact.parser
            ? MyReact.render(MyReact.parser(str, { inline: true }))
            : null;
        };
        MyReact.elementToReact = function (node, ref) {
          ...
          if (node.attributes) {
            ...
            if (node.attributes.style)
              attributes.style = BDFDB.ObjectUtils.filter(
                node.style,
                (n) => node.style[n] && isNaN(parseInt(n)),
                true,
              );
          }
          attributes.children = [];
          if (node.style && node.style.cssText)
            for (let propStr of node.style.cssText.split(";"))
              if (propStr.endsWith("!important")) {
          ...
          for (let child of node.childNodes)
            attributes.children.push(MyReact.elementToReact(child));
          attributes.className = BDFDB.DOMUtils.formatClassName(
            attributes.className,
            attributes.class,
          );
          delete attributes.class;
          return BDFDB.ReactUtils.forceStyle(
            BDFDB.ReactUtils.createElement(node.tagName, attributes),
            importantStyles,
          );
        };
        MyReact.forceStyle = function (reactEle, styles) {
          ...
          if (
            !BDFDB.ObjectUtils.is(reactEle.props.style) ||
            !BDFDB.ArrayUtils.is(styles) ||
            !styles.length
          )
            return reactEle;
          ...
          reactEle.ref = (instance) => {
            ...
            if (Node.prototype.isPrototypeOf(node))
              for (let key of styles) {
                let propValue =
                  reactEle.props.style[key.replace(/-([a-z]?)/g, (m, g) => g.toUpperCase())];
        ...
        MyReact.findDOMNode = function (instance, onlyChildren) {
          ...
          let node =
            Internal.LibraryModules.ReactDOM.findDOMNode &&
            Internal.LibraryModules.ReactDOM.findDOMNode(instance);
          for (let path of [
            "child.stateNode",
            "child.ref.current",
            !onlyChildren && "return.stateNode",
            !onlyChildren && "return.return.stateNode",
          ])
            if (!node && path) {
              node = BDFDB.ObjectUtils.get(
                instance[BDFDB.ReactUtils.instanceKey] || instance,
                path,
              );
          ...
          if (!node) {
            if (!onlyChildren)
              node = BDFDB.ReactUtils.findValue(
                instance[BDFDB.ReactUtils.instanceKey] || instance,
                "containerInfo",
                { up: true },
              );
        ...
        MyReact.findParent = function (nodeOrInstance, config) {
          if (
            !nodeOrInstance ||
            !BDFDB.ObjectUtils.is(config) ||
            (!config.name && !config.type && !config.key && !config.props && !config.filter)
          )
            return [null, -1];
          let instance = Node.prototype.isPrototypeOf(nodeOrInstance)
            ? BDFDB.ReactUtils.getInstance(nodeOrInstance)
            : nodeOrInstance;
          if (
            (!BDFDB.ObjectUtils.is(instance) && !BDFDB.ArrayUtils.is(instance)) ||
            (instance.props && typeof instance.props.children == "function")
          )
            return [null, -1];
          ...
          while (!BDFDB.ArrayUtils.is(firstArray) && firstArray.props && firstArray.props.children)
            firstArray = firstArray.props.children;
          ...
          function getParent(children) {
            ...
            if (!BDFDB.ArrayUtils.is(children)) {
              ...
              for (let i = 0; result[1] == -1 && i < children.length; i++)
                if (children[i]) {
          ...
          function check(instance) {
            ...
            if (
              ...
              config.name.some(
                (name) =>
                  instance.type.displayName == name ||
                  instance.type.name == name ||
                  Internal.isCorrectModule(
                    instance.type.render || instance.type.type || instance.type,
                    name,
                  ),
              )
            )
            ...
            if (
              config.props &&
              props &&
              config.props[config.someProps ? "some" : "every"]((prop) =>
                BDFDB.ArrayUtils.is(prop)
                  ? BDFDB.ArrayUtils.is(prop[1])
                    ? prop[1].some((checkValue) => propCheck(props, prop[0], checkValue))
                    : propCheck(props, prop[0], prop[1])
          ...
          function propCheck(props, key, value) {
            return (
              ...
              value != null &&
              (key == "className"
                ? (" " + props[key] + " ").indexOf(" " + value + " ") > -1
                : BDFDB.equals(props[key], value))
            );
          }
        };
        MyReact.findChild = function (nodeOrInstance, config) {
          ...
          let maxDepth = config.unlimited
            ? 999999999
            : config.depth === undefined
              ? 30
              : config.depth;
          let maxTime = config.unlimited
            ? 999999999
          ...
          function getChild(children) {
            let result = null;
            if (!children || depth >= maxDepth || performance.now() - start >= maxTime)
              return result;
            if (!BDFDB.ArrayUtils.is(children)) {
              if (check(children)) {
        ...
        MyReact.findOwner = function (nodeOrInstance, config) {
          ...
          if (
            ...
            return config.all ? (config.group ? {} : []) : null;
          let instance = Node.prototype.isPrototypeOf(nodeOrInstance)
            ? BDFDB.ReactUtils.getInstance(nodeOrInstance)
            : nodeOrInstance;
          ...
          let whitelist = config.up
            ? { return: true, sibling: true, default: true }
            : { child: true, sibling: true, default: true };
          ...
          if (config.all) {
            for (let i in foundInstances) {
              if (config.group)
                for (let j in foundInstances[i]) delete foundInstances[i][j].BDFDBreactSearch;
          ...
          function getOwner(instance) {
            ...
            if (
              instance &&
              !Node.prototype.isPrototypeOf(instance) &&
              !BDFDB.ReactUtils.getInstance(instance) &&
              depth < maxDepth &&
              performance.now() - start < maxTime
            ) {
              let props = instance.stateNode ? instance.stateNode.props : instance.props;
              let foundName = "";
              if (
                instance.stateNode &&
                !Node.prototype.isPrototypeOf(instance.stateNode) &&
                ((config.name &&
                  instance.type &&
                  ...
                  (config.type &&
                    instance.type &&
                    config.type.some((type) => instance.type == type)) ||
                  (config.key && instance.key && config.key.some((key) => instance.key == key)) ||
                  (config.props &&
                    props &&
                    config.props.every((prop) =>
                      BDFDB.ArrayUtils.is(prop)
                        ? BDFDB.ArrayUtils.is(prop[1])
                          ? prop[1].some((checkValue) => BDFDB.equals(props[prop[0]], checkValue))
                          : BDFDB.equals(props[prop[0]], prop[1])
                        : props[prop] !== undefined,
                    )) ||
                ...
                if (config.all === undefined || !config.all) result = instance.stateNode;
                  ...
                  if (!instance.stateNode.BDFDBreactSearch) {
                    ...
                    if (config.group) {
                      if (foundName) {
                        if (!BDFDB.ArrayUtils.is(foundInstances[foundName]))
                          foundInstances[foundName] = [];
              ...
              if (result === undefined) {
                ...
                for (let i = 0; result === undefined && i < keys.length; i++) {
                  ...
                  if (
                    key &&
                    whitelist[key] &&
                    (typeof instance[key] === "object" || typeof instance[key] == "function")
                  )
                    result = getOwner(instance[key]);
                }
              }
            }
            depth--;
        ...
        MyReact.findValue = function (nodeOrInstance, searchKey, config = {}) {
          ...
          let blacklist = { contextSection: true };
          ...
          function getKey(instance) {
            ...
            if (
              ...
              let keys = Object.keys(instance).sort((x, y) =>
                whitelistKeys.indexOf(x) < whitelistKeys.indexOf(y) ? -1 : 1,
              );
              for (let i = 0; result === undefined && i < keys.length; i++) {
                let key = keys[i];
                if (key && !blacklist[key]) {
                  ...
                  if (
                    searchKey === key &&
                    (config.notNull === undefined || value !== null) &&
                    (config.value === undefined || BDFDB.equals(config.value, value))
                  )
                    result = value;
                  else if (
        ...
        MyReact.render = function (component, node, ignoreErrors = false) {
          if (!BDFDB.ReactUtils.isValidElement(component) || !Node.prototype.isPrototypeOf(node))
            return;
          try {
            let root;
            if (Internal.LibraryModules.ReactDOM.render)
        ...
        MyReact.unmountComponentAtNode = function (node) {
          node && node.root && node.root.unmount
            ? node.root.unmount()
            : Internal.LibraryModules.ReactDOM.unmountComponentAtNode &&
              Internal.LibraryModules.ReactDOM.unmountComponentAtNode(node);
        };
        ...
        BDFDB.MessageUtils.isSystemMessage = function (message) {
          return (
            message &&
            !Internal.DiscordConstants.MessageTypeGroups.USER_MESSAGE.has(message.type) &&
            (message.type !== Internal.DiscordConstants.MessageTypes.CHAT_INPUT_COMMAND ||
        ...
        BDFDB.MessageUtils.rerenderAll = function (instant) {
          ...
          BDFDB.MessageUtils.rerenderAll.timeout = BDFDB.TimeUtils.timeout(
            (_) => {
              ...
              let LayerProviderIns = BDFDB.ReactUtils.findOwner(
                document.querySelector(BDFDB.dotCN.chatcontent),
                { name: "LayerProvider", unlimited: true, up: true },
              );
              let LayerProviderPrototype = BDFDB.ObjectUtils.get(
                LayerProviderIns,
                `${BDFDB.ReactUtils.instanceKey}.type.prototype`,
              );
              if (LayerProviderIns && LayerProviderPrototype) {
                BDFDB.PatchUtils.patch(
                  { name: "BDFDB MessageUtils" },
                  LayerProviderPrototype,
                  "render",
                  {
                    after: (e) => {
                      e.returnValue.props.children =
                        typeof e.returnValue.props.children == "function"
                          ? (_) => {
                              return null;
                            }
                          : [];
                      ...
                      if (scrollTop)
                        requestAnimationFrame(() => messagesScroller.scrollTo({ top: scrollTop }));
                    },
                  },
                  { once: true },
                );
        ...
        BDFDB.MessageUtils.openMenu = function (message, e = mousePosition, slim = false) {
          ...
          let moduleFindData =
            InternalData.PatchModules[type] && InternalData.PatchModules[type].strings;
          if (!moduleFindData) return;
          let menu = BDFDB.ModuleUtils.findByString(moduleFindData, { noWarnings: true });
          if (menu)
            Internal.LibraryModules.ContextMenuUtils.openContextMenu(e, (e2) =>
              BDFDB.ReactUtils.createElement(
                menu.default || menu,
                Object.assign({}, e2, { message, channel }),
              ),
            );
          else
            BDFDB.ModuleUtils.lazyLoadModuleImports(
              BDFDB.ModuleUtils.findByString(
                slim
                  ? InternalData.PatchModules.SearchResult &&
                      InternalData.PatchModules.SearchResult.strings
                  : InternalData.LibraryModules.MessageComponentUtils &&
                      InternalData.LibraryModules.MessageComponentUtils.strings,
              ),
            ).then((_) => {
        ...
        const myDataUser =
          Internal.LibraryStores.UserStore &&
          Internal.LibraryStores.UserStore.getCurrentUser &&
          Internal.LibraryStores.UserStore.getCurrentUser();
        ...
        BDFDB.UserUtils.getStatus = function (id = BDFDB.UserUtils.me.id) {
          ...
          return activity && activity.type == Internal.DiscordConstants.ActivityTypes.STREAMING
            ? "streaming"
            : Internal.LibraryStores.PresenceStore.getStatus(id);
        };
        BDFDB.UserUtils.getStatusColor = function (status, useColor) {
          if (!Internal.DiscordConstants.Colors || !Internal.DiscordConstants.ColorsCSS)
            return null;
          ...
          switch (status) {
            case "online":
              color = useColor
                ? Internal.DiscordConstants.Colors.GREEN_360
                : BDFDB.DiscordConstants.ColorsCSS.STATUS_POSITIVE;
              break;
            case "idle":
              color = useColor
                ? Internal.DiscordConstants.Colors.YELLOW_300
                : BDFDB.DiscordConstants.ColorsCSS.STATUS_WARNING;
              break;
            case "dnd":
              color = useColor
                ? Internal.DiscordConstants.Colors.RED_400
                : BDFDB.DiscordConstants.ColorsCSS.STATUS_DANGER;
              break;
            case "playing":
              color = useColor ? Internal.DiscordConstants.Colors.BRAND : "var(--bdfdb-blurple)";
              break;
            case "listening":
              color = Internal.DiscordConstants.Colors.SPOTIFY;
              break;
            case "streaming":
              color = Internal.DiscordConstants.Colors.TWITCH;
              break;
            default:
              color = Internal.DiscordConstants.Colors.PRIMARY_400;
              break;
          }
          return (color || Internal.DiscordConstants.Colors.GREEN_360).replace(
            /calc\(.+\s*\*\s*([0-9\.\%]+)\)/g,
            "$1",
          );
        };
        BDFDB.UserUtils.getActivity = function (id = BDFDB.UserUtils.me.id) {
          for (let activity of Internal.LibraryStores.PresenceStore.getActivities(id))
            if (activity.type != Internal.DiscordConstants.ActivityTypes.CUSTOM_STATUS)
              return activity;
          return null;
        };
        ...
        BDFDB.UserUtils.getAvatar = function (id = BDFDB.UserUtils.me.id) {
          ...
          if (!user) return window.location.origin + "/assets/1f0bfc0865d324c2587920a7d80c609b.png";
            ...
            return (
              (user.avatar ? "" : window.location.origin) +
              Internal.LibraryModules.IconUtils.getUserAvatarURL(user)
            ).split("?")[0];
        };
        BDFDB.UserUtils.getBanner = function (
          id = BDFDB.UserUtils.me.id,
          guildId = Internal.LibraryStores.SelectedGuildStore.getGuildId(),
          canAnimate = false,
        ) {
          let displayProfile = Internal.LibraryModules.MemberDisplayUtils.getDisplayProfile(
            id,
            guildId,
          );
          return (
            (!guildId
              ? Internal.LibraryModules.IconUtils.getUserBannerURL({
                  banner: displayProfile && displayProfile.banner,
                  id,
                  canAnimate,
                })
              : Internal.LibraryModules.IconUtils.getGuildMemberBannerURL({
                  banner: displayProfile && displayProfile.banner,
                  id,
                  guildId,
                  canAnimate,
        ...
        BDFDB.UserUtils.can = function (
          permission,
          id = BDFDB.UserUtils.me.id,
          channelId = Internal.LibraryStores.SelectedChannelStore.getChannelId(),
        ) {
          if (!Internal.DiscordConstants.Permissions[permission])
            BDFDB.LogUtils.warn([permission, "not found in Permissions"]);
          else {
            let channel = Internal.LibraryStores.ChannelStore.getChannel(channelId);
            if (channel)
              return Internal.LibraryModules.PermissionRoleUtils.can({
                permission: Internal.DiscordConstants.Permissions[permission],
                user: id,
                context: channel,
              });
          }
        ...
        BDFDB.UserUtils.openMenu = function (user, guildId, channelId, e = mousePosition) {
          ...
          if (guildId && !channelId)
            channelId = (BDFDB.LibraryStores.GuildChannelStore.getDefaultChannel(guildId) || {}).id;
          e = BDFDB.ListenerUtils.copyEvent(e.nativeEvent || e, (e.nativeEvent || e).currentTarget);
          ...
          if (menu)
            Internal.LibraryModules.ContextMenuUtils.openContextMenu(e, (e2) =>
              BDFDB.ReactUtils.createElement(
                menu.default || menu,
                Object.assign({}, e2, { user, guildId, channelId }),
              ),
            );
          else
            BDFDB.ModuleUtils.lazyLoadModuleImports(
              BDFDB.ModuleUtils.findByString(
                channelId
                  ? InternalData.LibraryModules.UserPopoutUtils &&
                      InternalData.LibraryModules.UserPopoutUtils.strings
                  : InternalData.PatchModules.ParticipantsForSelectedParticipant &&
                      InternalData.PatchModules.ParticipantsForSelectedParticipant.strings,
              ),
            ).then((_) => {
              menu = BDFDB.ModuleUtils.findByString(moduleFindData);
              if (menu)
        ...
        BDFDB.GuildUtils.is = function (guild) {
          ...
          return (
            guild instanceof Internal.DiscordObjects.Guild ||
            Object.keys(new Internal.DiscordObjects.Guild({})).every(
        ...
        BDFDB.GuildUtils.getIcon = function (id, size, canAnimate) {
          ...
          return size
            ? Internal.LibraryModules.IconUtils.getGuildIconURL({
                id,
                size,
                canAnimate,
                icon: guild.icon,
        ...
        BDFDB.GuildUtils.getFolder = function (id) {
          return Internal.LibraryStores.SortedGuildStore.getGuildFolders()
            .filter((n) => n.folderId)
            .find((n) => n.guildIds.includes(id));
        };
        BDFDB.GuildUtils.openMenu = function (guild, e = mousePosition) {
          if (!guild) return;
          e = BDFDB.ListenerUtils.copyEvent(e.nativeEvent || e, (e.nativeEvent || e).currentTarget);
          let moduleFindData =
            InternalData.PatchModules.GuildContextMenu &&
            InternalData.PatchModules.GuildContextMenu.strings;
          if (!moduleFindData) return;
          let menu = BDFDB.ModuleUtils.findByString(moduleFindData, { noWarnings: true });
          if (menu)
            Internal.LibraryModules.ContextMenuUtils.openContextMenu(e, (e2) =>
              BDFDB.ReactUtils.createElement(
                menu.default || menu,
                Object.assign({}, e2, { guild }),
              ),
            );
          else
            BDFDB.ModuleUtils.lazyLoadModuleImports(
              BDFDB.ModuleUtils.findByString(
                InternalData.PatchModules.GuildSidebar &&
                  InternalData.PatchModules.GuildSidebar.strings,
              ),
            ).then((_) => {
              menu = BDFDB.ModuleUtils.findByString(moduleFindData);
              if (menu)
        ...
        BDFDB.GuildUtils.markAsRead = function (guildIds) {
          guildIds = [guildIds]
            .flat(10)
            .filter(
              (id) => id && typeof id == "string" && Internal.LibraryStores.GuildStore.getGuild(id),
            );
          if (!guildIds) return;
          let channels = guildIds
            .map((id) => [
              BDFDB.ObjectUtils.toArray(Internal.LibraryStores.GuildChannelStore.getChannels(id)),
              BDFDB.ObjectUtils.toArray(
                Internal.LibraryStores.ActiveThreadsStore.getThreadsForGuild(id),
              )
                .map(BDFDB.ObjectUtils.toArray)
                .flat(),
              Internal.LibraryStores.GuildScheduledEventStore.getGuildScheduledEventsForGuild(id),
            ])
            .flat(10)
            .map((n) => n && ((n.channel && n.channel.id) || n.id))
            .flat()
            .filter((n) => n);
          if (channels.length) BDFDB.ChannelUtils.markAsRead(channels);
          let eventChannels = guildIds
            .map((id) => ({
              channelId: id,
              readStateType: Internal.DiscordConstants.ReadStateTypes.GUILD_EVENT,
              messageId: Internal.LibraryStores.ReadStateStore.lastMessageId(
                id,
                Internal.DiscordConstants.ReadStateTypes.GUILD_EVENT,
              ),
            }))
        ...
        BDFDB.FolderUtils.getDefaultName = function (folderId) {
          ...
          let names = [],
            allNames = folder.guildIds
              .map((guildId) => (Internal.LibraryStores.GuildStore.getGuild(guildId) || {}).name)
              .filter((n) => n);
          for (let name of allNames)
            if (name.length < rest || names.length === 0) {
              names.push(name);
              rest -= name.length;
            }
        ...
        BDFDB.ChannelUtils.is = function (channel) {
          ...
          return (
            channel instanceof Internal.DiscordObjects.Channel ||
            Object.keys(new Internal.DiscordObjects.Channel({})).every(
              (key) => keys.indexOf(key) > -1,
            )
          );
        };
        BDFDB.ChannelUtils.isTextChannel = function (channelOrId) {
          let channel =
            typeof channelOrId == "string"
              ? Internal.LibraryStores.ChannelStore.getChannel(channelOrId)
              : channelOrId;
          return (
            BDFDB.ObjectUtils.is(channel) &&
            (channel.type == Internal.DiscordConstants.ChannelTypes.GUILD_TEXT ||
              channel.type == Internal.DiscordConstants.ChannelTypes.GUILD_STORE ||
              channel.type == Internal.DiscordConstants.ChannelTypes.GUILD_ANNOUNCEMENT)
          );
        };
        ...
        BDFDB.ChannelUtils.isForumPost = function (channelOrId) {
          ...
          return (
            ...
            channel.parentChannelThreadType == Internal.DiscordConstants.ChannelTypes.GUILD_FORUM
          );
        };
        BDFDB.ChannelUtils.isEvent = function (channelOrId) {
          let channel =
            typeof channelOrId == "string"
              ? Internal.LibraryStores.GuildScheduledEventStore.getGuildScheduledEvent(channelOrId)
              : channelOrId;
          return (
            channel &&
            Internal.LibraryStores.GuildScheduledEventStore.getGuildScheduledEvent(channel.id) &&
            true
          );
        };
        BDFDB.ChannelUtils.markAsRead = function (channelIds) {
          let unreadChannels = [channelIds]
            .flat(10)
            .filter(
              (id) =>
                id &&
                typeof id == "string" &&
                (BDFDB.LibraryStores.ChannelStore.getChannel(id) || {}).type !=
                  Internal.DiscordConstants.ChannelTypes.GUILD_CATEGORY &&
                (Internal.LibraryStores.ReadStateStore.hasUnread(id) ||
                  Internal.LibraryStores.ReadStateStore.getMentionCount(id) > 0),
            )
            .map((id) => ({
              channelId: id,
        ...
        BDFDB.ChannelUtils.rerenderAll = function (instant) {
          ...
          BDFDB.ChannelUtils.rerenderAll.timeout = BDFDB.TimeUtils.timeout(
            (_) => {
              let ChannelsIns = BDFDB.ReactUtils.findOwner(
                document.querySelector(BDFDB.dotCN.guildchannels),
                { name: "ChannelsList", unlimited: true },
              );
              if (!ChannelsIns) return;
                ...
                if (
                  ChannelsIns &&
                  ChannelsIns.props &&
                  ChannelsIns.props.guildChannels.categories &&
                  Object.keys(ChannelsIns.props.guildChannels.categories).length
                ) {
                  let category =
                    ChannelsIns.props.guildChannels.categories[
                      Object.keys(ChannelsIns.props.guildChannels.categories)[0]
                    ];
                  category.isCollapsed
                    ? BDFDB.LibraryModules.CategoryCollapseUtils.categoryCollapse(category.id)
                    : BDFDB.LibraryModules.CategoryCollapseUtils.categoryExpand(category.id);
                } else {
                  let ChannelsPrototype = BDFDB.ObjectUtils.get(
                    ChannelsIns,
                    `${BDFDB.ReactUtils.instanceKey}.type.prototype`,
                  );
        ...
        BDFDB.DMUtils.getIcon = function (id) {
          ...
          if (!channel.icon)
            return channel.isDM()
              ? BDFDB.UserUtils.getAvatar(channel.recipients[0])
              : channel.isGroupDM()
                ? window.location.origin +
                  Internal.LibraryModules.IconUtils.getChannelIconURL(channel).split("?")[0]
                : null;
          ...
          let unreadDMs = [dmIds]
            ...
            .filter(
              (id) =>
                id &&
                typeof id == "string" &&
                BDFDB.DMUtils.isDMChannel(id) &&
                (Internal.LibraryStores.ReadStateStore.hasUnread(id) ||
                  Internal.LibraryStores.ReadStateStore.getMentionCount(id) > 0),
            );
          if (unreadDMs.length)
            for (let i in unreadDMs)
              BDFDB.TimeUtils.timeout(
                (_) => Internal.LibraryModules.AckUtils.ack(unreadDMs[i]),
                i * 1000,
              );
        };
          ...
          if (BDFDB.ObjectUtils.is(color)) {
            ...
            type =
              ...
              ![
                ...
                ? BDFDB.ColorUtils.getType(color)
                : type.toUpperCase();
            if (conv == "RGBCOMP") {
              switch (type) {
                case "RGBCOMP":
                ...
                case "HSLCOMP":
                  ...
                  if (hslComp.length == 3)
                    return BDFDB.ColorUtils.convert(
                      `hsl(${processHSX(hslComp).join(",")})`,
                      "RGBCOMP",
                    );
                ...
                case "HSL":
                  ...
                  var r, g, b, m, c, x, p, q;
                  var h = hslComp[0],
                    s = processPercentage(hslComp[1]),
                    l = processPercentage(hslComp[2]);
                  var a = s * Math.min(l, 1 - l);
                  var f = (n, k = (n + h / 30) % 12) =>
                    parseInt((l - a * Math.max(Math.min(k - 3, 9 - k, 1), -1)) * 255);
                  return [f(0), f(8), f(4)];
                case "HSLA":
                  var hslComp = color.replace(/\s/g, "").slice(5, -1).split(",");
                  return BDFDB.ColorUtils.convert(
                    `hsl(${hslComp.slice(0, 3).join(",")})`,
                    "RGBCOMP",
                  ).concat(processA(hslComp.pop()));
                case "HSVCOMP":
                  var hsvComp = [].concat(color);
                  if (hsvComp.length == 3)
                    return BDFDB.ColorUtils.convert(
                      `hsv(${processHSX(hsvComp).join(",")})`,
                      "RGBCOMP",
                    );
                ...
                case "HSV":
                  ...
                  var r, g, b, i, f, p, q, t;
                  var h = hsvComp[0] / 360,
                    s = processPercentage(hsvComp[1]),
                    v = processPercentage(hsvComp[2]);
                  ((i = Math.floor(h * 6)),
                    (f = h * 6 - i),
                    (p = v * (1 - s)),
                    (q = v * (1 - f * s)),
                ...
                case "HSVA":
                  ...
                  return BDFDB.ColorUtils.convert(
                    `hsv(${hsvComp.slice(0, 3).join(",")})`,
                    "RGBCOMP",
                  ).concat(processA(hsvComp.pop()));
                case "HEX":
                  var hex =
                    /^#([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})$|^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(
                      color,
                    );
                  return [
                    parseInt(hex[1] + hex[1] || hex[4], 16),
                    parseInt(hex[2] + hex[2] || hex[5], 16),
                    parseInt(hex[3] + hex[3] || hex[6], 16),
                  ];
                case "HEXA":
                  var hex =
                    /^#([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})$|^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(
                      color,
                    );
                  return [
                    parseInt(hex[1] + hex[1] || hex[5], 16),
                    parseInt(hex[2] + hex[2] || hex[6], 16),
                    parseInt(hex[3] + hex[3] || hex[7], 16),
                    Math.floor(
                      BDFDB.NumberUtils.mapRange(
                        [0, 255],
                        [0, 100],
                        parseInt(hex[4] + hex[4] || hex[8], 16),
                      ),
                    ) / 100,
                  ];
                case "INT":
                  ...
                  return [
                    parseInt((color >> 16) & 255),
                    parseInt((color >> 8) & 255),
                    parseInt(color & 255),
                  ];
                default:
                  return null;
              ...
              if (
                conv &&
                type &&
                ((conv.indexOf("HSL") == 0 && type.indexOf("HSL") == 0) ||
                  (conv.indexOf("HSV") == 0 && type.indexOf("HSV") == 0))
              ) {
                ...
                return BDFDB.ColorUtils.convert(
                  color
                    .replace(/\s/g, "")
                    .slice(color.toUpperCase().indexOf("A") == 3 ? 5 : 4, -1)
                    .split(","),
                  conv,
                  `${name}COMP`,
                );
                ...
                let rgbComp =
                  type == "RGBCOMP"
                    ? [].concat(color)
                    : BDFDB.ColorUtils.convert(color, "RGBCOMP", type);
                if (rgbComp)
                  switch (conv) {
                    case "RGB":
                    ...
                    case "HSLCOMP":
                      ...
                      var hslComp = processHSX(
                        BDFDB.ColorUtils.convert(rgbComp, "HSL").replace(/\s/g, "").split(","),
                      );
                      return a != null ? hslComp.concat(a) : hslComp;
                    case "HSL":
                      var r = processC(rgbComp[0]) / 255,
                        g = processC(rgbComp[1]) / 255,
                        b = processC(rgbComp[2]) / 255;
                    ...
                    case "HSVCOMP":
                      var a = rgbComp.length == 4 ? processA(rgbComp.pop()) : null;
                      var hsvComp = processHSX(
                        BDFDB.ColorUtils.convert(rgbComp, "HSV").replace(/\s/g, "").split(","),
                      );
                      return a != null ? hsvComp.concat(a) : hsvComp;
                    case "HSV":
                      var r = processC(rgbComp[0]),
                        g = processC(rgbComp[1]),
                        b = processC(rgbComp[2]);
                      var max = Math.max(r, g, b),
                        min = Math.min(r, g, b),
                        dif = max - min,
                        h,
                    ...
                    case "HEX":
                      return (
                        "#" +
                        (0x1000000 + (rgbComp[2] | (rgbComp[1] << 8) | (rgbComp[0] << 16)))
                          .toString(16)
                          .slice(1)
                      ).toUpperCase();
                    case "HEXA":
        ...
        BDFDB.ColorUtils.change = function (color, value, conv) {
          ...
          if (color != null && typeof value == "number" && !isNaN(value)) {
            if (BDFDB.ObjectUtils.is(color)) {
              let newColor = {};
              for (let pos in color)
                newColor[pos] = BDFDB.ColorUtils.change(color[pos], value, conv);
              return newColor;
            } else {
              let rgbComp = BDFDB.ColorUtils.convert(color, "RGBCOMP");
              if (rgbComp) {
        ...
        BDFDB.ColorUtils.invert = function (color, conv) {
          ...
          if (BDFDB.ObjectUtils.is(color)) {
            ...
            if (comp)
              return BDFDB.ColorUtils.convert(
                [255 - comp[0], 255 - comp[1], 255 - comp[2]],
                conv || BDFDB.ColorUtils.getType(color),
              );
          }
          return null;
        ...
        BDFDB.ColorUtils.isBright = function (color, compare = 160) {
          ...
          return (
            parseInt(compare) <
            Math.sqrt(0.299 * color[0] ** 2 + 0.587 * color[1] ** 2 + 0.144 * color[2] ** 2)
          );
        };
        BDFDB.ColorUtils.getType = function (color) {
          if (color !== null) {
            if (typeof color === "object" && (color.length == 3 || color.length == 4)) {
              ...
              if (/^#[a-f\d]{3}$|^#[a-f\d]{6}$/i.test(color)) return "HEX";
                ...
                else if (color.indexOf("RGBA(") == 0 && comp.length == 4 && isRGB(comp))
                  return "RGBA";
                else if (color.indexOf("HSL(") == 0 && comp.length == 3 && isHSL(comp))
                  return "HSL";
                else if (color.indexOf("HSLA(") == 0 && comp.length == 4 && isHSL(comp))
                  return "HSLA";
                else if (color.indexOf("HSV(") == 0 && comp.length == 3 && isHSL(comp))
                  return "HSV";
                else if (color.indexOf("HSVA(") == 0 && comp.length == 4 && isHSL(comp))
                  return "HSVA";
              }
            } else if (
              typeof color === "number" &&
          ...
          function isRGB(comp) {
            return comp
              .slice(0, 3)
              .every(
                (rgb) => rgb.toString().indexOf("%") == -1 && parseFloat(rgb) == parseInt(rgb),
              );
          }
          function isHSL(comp) {
            return comp.slice(1, 3).every((hsl) => hsl.toString().indexOf("%") == hsl.length - 1);
          }
        };
        ...
        BDFDB.DOMUtils.addClass = function (eles, ...classes) {
          ...
          for (let ele of [eles]
            .map((n) => (NodeList.prototype.isPrototypeOf(n) ? Array.from(n) : n))
            .flat(10)
            .filter((n) => n)) {
            if (Node.prototype.isPrototypeOf(ele)) add(ele);
            else if (NodeList.prototype.isPrototypeOf(ele)) for (let e of ele) add(e);
            else if (typeof ele == "string")
              for (let e of ele.split(","))
                if (e && (e = e.trim())) for (let n of document.querySelectorAll(e)) add(n);
          }
          function add(node) {
            if (node && node.classList)
              for (let cla of classes)
                for (let cl of [cla].flat(10).filter((n) => n))
                  if (typeof cl == "string")
                    for (let c of cl.split(" ")) if (c) node.classList.add(c);
          }
        };
        BDFDB.DOMUtils.removeClass = function (eles, ...classes) {
          ...
          for (let ele of [eles]
            ...
            if (Node.prototype.isPrototypeOf(ele)) remove(ele);
              ...
              for (let e of ele.split(","))
                if (e && (e = e.trim())) for (let n of document.querySelectorAll(e)) remove(n);
          }
          function remove(node) {
            if (node && node.classList)
              for (let cla of classes)
                for (let cl of [cla].flat(10).filter((n) => n))
                  if (typeof cl == "string")
                    for (let c of cl.split(" ")) if (c) node.classList.remove(c);
          }
        };
        BDFDB.DOMUtils.toggleClass = function (eles, ...classes) {
          ...
          for (let ele of [eles]
            ...
            if (Node.prototype.isPrototypeOf(ele)) toggle(ele);
              ...
              for (let e of ele.split(","))
                if (e && (e = e.trim())) for (let n of document.querySelectorAll(e)) toggle(n);
          }
          function toggle(node) {
            if (node && node.classList)
              for (let cla of classes)
                for (let cl of [cla].flat(10).filter((n) => n))
                  if (typeof cl == "string")
                    for (let c of cl.split(" ")) if (c) node.classList.toggle(c, force);
          }
        };
        BDFDB.DOMUtils.containsClass = function (eles, ...classes) {
          ...
          for (let ele of [eles]
            ...
            if (Node.prototype.isPrototypeOf(ele)) contains(ele);
              ...
              for (let c of ele.split(","))
                if (c && (c = c.trim())) for (let n of document.querySelectorAll(c)) contains(n);
          }
          return contained;
        ...
        BDFDB.DOMUtils.formatClassName = function (...classes) {
          return BDFDB.ArrayUtils.removeCopies(
            classes
              .flat(10)
              .filter((n) => n)
              .join(" ")
        ...
        BDFDB.DOMUtils.removeClassFromDOM = function (...classes) {
          for (let c of classes.flat(10).filter((n) => n))
            if (typeof c == "string")
              for (let a of c.split(","))
                if (a && (a = a.replace(/\.|\s/g, "")))
                  BDFDB.DOMUtils.removeClass(document.querySelectorAll("." + a), a);
        };
        ...
        BDFDB.DOMUtils.toggle = function (...eles) {
          ...
          for (let ele of eles.flat(10).filter((n) => n)) {
            if (Node.prototype.isPrototypeOf(ele)) toggle(ele);
              ...
              for (let c of ele.split(","))
                if (c && (c = c.trim()))
                  for (let node of document.querySelectorAll(c)) toggle(node);
          }
          function toggle(node) {
            ...
            if (hide) {
              ...
              if (display && display != "none")
                node.BDFDBhideDisplayState = {
                  display: display,
                  important:
                    (` ${node.style.cssText} `.split(` display: ${display}`)[1] || "")
                      .trim()
                      .indexOf("!important") == 0,
                };
              ...
              if (node.BDFDBhideDisplayState) {
                node.style.setProperty(
                  "display",
                  node.BDFDBhideDisplayState.display,
                  node.BDFDBhideDisplayState.important ? "important" : "",
                );
        ...
        BDFDB.DOMUtils.isHidden = function (node) {
          if (Node.prototype.isPrototypeOf(node) && node.nodeType != Node.TEXT_NODE)
            return getComputedStyle(node, null).getPropertyValue("display") == "none";
        };
        ...
        BDFDB.DOMUtils.getParent = function (listOrSelector, node) {
          ...
          if (Node.prototype.isPrototypeOf(node) && listOrSelector) {
            let list = NodeList.prototype.isPrototypeOf(listOrSelector)
              ? listOrSelector
              : typeof listOrSelector == "string"
                ? document.querySelectorAll(listOrSelector)
                : null;
            if (list)
        ...
        BDFDB.DOMUtils.setText = function (node, stringOrNode) {
          ...
          if (!textnode)
            for (let child of node.childNodes)
              if (
                ...
                BDFDB.DOMUtils.containsClass(child, "BDFDB-textnode")
              ) {
                textnode = child;
                break;
              }
          if (textnode) {
            if (
              Node.prototype.isPrototypeOf(stringOrNode) &&
              stringOrNode.nodeType != Node.TEXT_NODE
            ) {
              BDFDB.DOMUtils.addClass(stringOrNode, "BDFDB-textnode");
              node.replaceChild(stringOrNode, textnode);
            } else if (
              Node.prototype.isPrototypeOf(textnode) &&
              textnode.nodeType != Node.TEXT_NODE
            )
              node.replaceChild(document.createTextNode(stringOrNode), textnode);
            else textnode.textContent = stringOrNode;
          } else
            node.appendChild(
              Node.prototype.isPrototypeOf(stringOrNode)
                ? stringOrNode
                : document.createTextNode(stringOrNode),
            );
        };
        ...
        BDFDB.DOMUtils.appendWebScript = function (url, container) {
          ...
          if (!container && !document.body.querySelector("bd-head bd-scripts"))
            document.body.appendChild(
              BDFDB.DOMUtils.create(`<bd-head><bd-scripts></bd-scripts></bd-head>`),
            );
          container =
            container || document.body.querySelector("bd-head bd-scripts") || document.body;
        ...
        BDFDB.DOMUtils.appendWebStyle = function (url, container) {
          if (typeof url != "string") return;
          if (!container && !document.body.querySelector("bd-head bd-styles"))
            document.body.appendChild(
              BDFDB.DOMUtils.create(`<bd-head><bd-styles></bd-styles></bd-head>`),
            );
          container =
            container || document.body.querySelector("bd-head bd-styles") || document.body;
          container = Node.prototype.isPrototypeOf(container) ? container : document.body;
          BDFDB.DOMUtils.removeWebStyle(url, container);
          container.appendChild(
            BDFDB.DOMUtils.create(`<link type="text/css" rel="stylesheet" href="${url}"></link>`),
          );
        };
        ...
        BDFDB.DOMUtils.appendLocalStyle = function (id, css, container) {
          ...
          container.appendChild(
            BDFDB.DOMUtils.create(`<style id="${id}CSS">${css.replace(/\t|\r|\n/g, "")}</style>`),
          );
        };
        ...
        BDFDB.ModalUtils.open = function (plugin, config) {
          ...
          let modalInstance,
            ...
            cancels = [],
            closeModal = (_) => {
              if (BDFDB.ObjectUtils.is(modalProps) && typeof modalProps.onClose == "function")
                modalProps.onClose();
            };
          ...
          if (config.children) {
            let tabBarItems = [],
              tabIns = {};
            for (let child of [config.children].flat(10).filter((n) => n))
              if (Internal.LibraryModules.React.isValidElement(child)) {
                ...
                BDFDB.ReactUtils.createElement("div", {
                  ...
                  children: [
                    BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TabBar, {
                      ...
                      items: tabBarItems,
                      onItemSelect: (value) => {
                        for (let key in tabIns) tabIns[key].setState({ open: key == value });
                      },
                    }),
            ...
            for (let button of config.buttons) {
              ...
              if (contents) {
                let color =
                  ...
                  Internal.LibraryComponents.Button.Colors[button.color.toUpperCase()];
                let look =
                  typeof button.look == "string" &&
                  Internal.LibraryComponents.Button.Looks[button.look.toUpperCase()];
                let click =
                ...
                footerChildren.push(
                  BDFDB.ReactUtils.createElement(
                    Internal.LibraryComponents.Button,
                    BDFDB.ObjectUtils.exclude(
                      Object.assign({}, button, {
                        look:
                          look ||
                          (color
                            ? Internal.LibraryComponents.Button.Looks.FILLED
                            : Internal.LibraryComponents.Button.Looks.LINK),
          ...
          contentChildren = contentChildren
            .concat(config.contentChildren)
            .filter((n) => n && (typeof n == "string" || BDFDB.ReactUtils.isValidElement(n)));
          titleChildren = titleChildren
            .concat(config.titleChildren)
            .filter((n) => n && (typeof n == "string" || BDFDB.ReactUtils.isValidElement(n)));
          headerChildren = headerChildren
            .concat(config.headerChildren)
            .filter((n) => n && (typeof n == "string" || BDFDB.ReactUtils.isValidElement(n)));
          footerChildren = footerChildren
            .concat(config.footerChildren)
            .filter((n) => n && (typeof n == "string" || BDFDB.ReactUtils.isValidElement(n)));
          ...
          if (contentChildren.length) {
            ...
            let name =
              plugin.name || (typeof plugin.getName == "function" ? plugin.getName() : null);
            name = typeof name == "string" ? name : null;
            let oldTransitionState = 0;
            !Internal.LibraryModules.ModalUtils
              ? BdApi.UI.alert(
                  BDFDB.ReactUtils.createElement("div", {
                    style: { display: "flex", "flex-direction": "column" },
                    children: [
                      config.header,
                      typeof config.subHeader == "string" ||
                      BDFDB.ReactUtils.isValidElement(config.subHeader)
                        ? config.subHeader
                        : name || "",
                    ]
                      .filter((n) => n)
                      .map((n) => BDFDB.ReactUtils.createElement("span", { children: n })),
                  }),
                  config.content || config.children,
                )
              : Internal.LibraryModules.ModalUtils.openModal(
                  (props) => {
                    modalProps = props;
                    return BDFDB.ReactUtils.createElement(
                      class BDFDB_Modal extends Internal.LibraryModules.React.Component {
                        render() {
                          return BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.ModalComponents.ModalRoot,
                            {
                              className: BDFDB.DOMUtils.formatClassName(
                                name && `${name}-modal`,
                                BDFDB.disCN.modalwrapper,
                                config.className,
                              ),
                              size:
                                (typeof config.size == "string" &&
                                  Internal.LibraryComponents.ModalComponents.ModalSize[
                                    config.size.toUpperCase()
                                  ]) ||
                                Internal.LibraryComponents.ModalComponents.ModalSize.SMALL,
                              transitionState: props.transitionState,
                              children: [
                                BDFDB.ReactUtils.createElement(
                                  Internal.LibraryComponents.ModalComponents.ModalHeader,
                                  {
                                    className: BDFDB.DOMUtils.formatClassName(
                                      config.headerClassName,
                                      config.shade && BDFDB.disCN.modalheadershade,
                                      headerChildren.length && BDFDB.disCN.modalheaderhassibling,
                                    ),
                                    separator: config.headerSeparator || false,
                                    children: [
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.Flex.Child,
                                        {
                                          style: { flex: 1 },
                                          children: [
                                            BDFDB.ReactUtils.createElement(
                                              Internal.LibraryComponents.FormTitle.Title,
                                              {
                                                tag:
                                                  Internal.LibraryComponents.FormTitle.Tags &&
                                                  Internal.LibraryComponents.FormTitle.Tags.H4,
                                            ...
                                            BDFDB.ReactUtils.createElement(
                                              ...
                                              {
                                                size: Internal.LibraryComponents.TextElement.Sizes
                                                  .SIZE_12,
                                                children:
                                                  typeof config.subHeader == "string" ||
                                                  BDFDB.ReactUtils.isValidElement(config.subHeader)
                                      ...
                                      titleChildren,
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.ModalComponents.ModalCloseButton,
                                        { onClick: closeModal },
                                      ),
                                    ]
                                      .flat(10)
                                ...
                                headerChildren.length
                                  ? BDFDB.ReactUtils.createElement(
                                      Internal.LibraryComponents.Flex,
                                      { grow: 0, shrink: 0, children: headerChildren },
                                    )
                                  : null,
                                BDFDB.ReactUtils.createElement(
                                  Internal.LibraryComponents.ModalComponents.ModalContent,
                                  {
                                ...
                                footerChildren.length
                                  ? BDFDB.ReactUtils.createElement(
                                      Internal.LibraryComponents.ModalComponents.ModalFooter,
                                      {
                        ...
                        componentDidMount() {
                          modalInstance = this;
                          if (
                            props.transitionState == 1 &&
                            props.transitionState > oldTransitionState
                          )
        ...
        var MappedMenuItems = {},
          RealMenuItems = BDFDB.ModuleUtils.find((m) => {
            if (!m || typeof m != "function") return false;
            let string = m.toString();
            return (
              string.indexOf("(){return null}") > -1 &&
              string.indexOf("catch(") == -1 &&
              string.split("){return null}").length > 4
            );
          });
        if (!RealMenuItems) {
          RealMenuItems = {};
        ...
        BDFDB.ContextMenuUtils.open = function (plugin, e, children) {
          Internal.LibraryModules.ContextMenuUtils.openContextMenu(e || mousePosition, (_) =>
            BDFDB.ReactUtils.createElement(
              Internal.LibraryComponents.Menu,
              {
                navId: "bdfdb-context",
        ...
        BDFDB.ContextMenuUtils.close = function (nodeOrInstance) {
          ...
          let instance = BDFDB.ReactUtils.findOwner(nodeOrInstance, {
            props: "closeContextMenu",
            up: true,
          });
          if (
            BDFDB.ObjectUtils.is(instance) &&
            instance.props &&
            typeof instance.props.closeContextMenu == "function"
          )
            instance.props.closeContextMenu();
        ...
        BDFDB.ContextMenuUtils.createItem = function (component, props = {}) {
          if (!component) return null;
            ...
            if (
              props.render ||
              props.persisting ||
              BDFDB.ObjectUtils.is(props.popoutProps) ||
              (typeof props.color == "string" &&
                !InternalData.DiscordClasses[`menu${props.color.toLowerCase()}`])
            )
              component = Internal.MenuItem;
            if (BDFDB.ObjectUtils.toArray(RealMenuItems).some((c) => c == component))
              return BDFDB.ReactUtils.createElement(component, props);
            else
              return BDFDB.ReactUtils.createElement(LibraryComponents.MenuItems.MenuItem, {
                id: props.id,
                disabled: props.disabled,
                customItem: true,
                render: (menuItemProps) => {
                  if (!props.state)
                    props.state = BDFDB.ObjectUtils.extract(props, "checked", "value");
                  return BDFDB.ReactUtils.createElement(
                    Internal.CustomMenuItemWrapper,
                    {
                      disabled: props.disabled,
        ...
        BDFDB.ContextMenuUtils.findItem = function (returnvalue, config) {
          if (!returnvalue || !BDFDB.ObjectUtils.is(config) || (!config.label && !config.id))
            return [null, -1];
          config.label = config.label && [config.label].flat().filter((n) => n);
          config.id = config.id && [config.id].flat().filter((n) => n);
          let contextMenu = BDFDB.ArrayUtils.is(returnvalue)
            ? { props: { children: returnvalue } }
            : BDFDB.ReactUtils.findChild(returnvalue, { props: "navId" });
          if (contextMenu) {
            let children = BDFDB.ArrayUtils.is(contextMenu.props.children)
              ? contextMenu.props.children
              : [contextMenu.props.children];
              ...
              if (children[i]) {
                if (check(children[i])) return [children, parseInt(i)];
                  ...
                  for (let j in children[i])
                    if (check(children[i][j])) return [children[i], parseInt(j)];
                } else if (children[i].props && children[i].props.children) {
                  if (
                    BDFDB.ArrayUtils.is(children[i].props.children) &&
                    children[i].props.children.length
                  ) {
                    let [possibleChildren, possibleIndex] = BDFDB.ContextMenuUtils.findItem(
                    ...
                    children[i].props.children.props.children
                  ) {
                    if (
                      BDFDB.ArrayUtils.is(children[i].props.children.props.children) &&
                      children[i].props.children.props.children.length
                    ) {
                      let [possibleChildren, possibleIndex] = BDFDB.ContextMenuUtils.findItem(
                        children[i].props.children.props.children,
                        config,
          ...
          function check(child) {
            ...
            return (
              (config.id &&
                config.id.some((key) =>
                  key == "devmode-copy-id"
                    ? typeof props.id == "string" && props.id.startsWith(key)
                    : props.id == key,
                )) ||
              (config.label && config.label.some((key) => props.label == key))
            );
          }
        };
        ...
        BDFDB.StringUtils.extractSelection = function (original, selection) {
          ...
          for (let i of BDFDB.ArrayUtils.getAllIndexes(original, selection[0]))
            if (!done) {
        ...
        BDFDB.SlateUtils.isRichValue = function (richValue) {
          return (
            richValue &&
            typeof richValue == "object" &&
            BDFDB.SlateUtils.toRichValue("").constructor.prototype.isPrototypeOf(richValue)
          );
        };
        BDFDB.SlateUtils.toTextValue = function (richValue) {
          return BDFDB.SlateUtils.isRichValue(richValue)
            ? Internal.LibraryModules.SlateTextUtils.toTextValue(richValue)
            : "";
        };
        BDFDB.SlateUtils.toRichValue = function (string) {
          return typeof string == "string"
            ? Internal.LibraryModules.SlateRichUtils.toRichValue(string)
            : null;
        };
        ...
        BDFDB.NumberUtils.formatBytes = function (bytes, sigDigits) {
          ...
          return (
            parseFloat(
              (bytes / Math.pow(1024, size)).toFixed(
                sigDigits < 1 ? 0 : sigDigits > 20 ? 20 : sigDigits || 2,
              ),
            ) +
            " " +
            ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"][size]
          );
        };
        BDFDB.NumberUtils.mapRange = function (from, to, value) {
          if (parseFloat(value) < parseFloat(from[0])) return parseFloat(to[0]);
            ...
            return (
              parseFloat(to[0]) +
              ((parseFloat(value) - parseFloat(from[0])) *
                (parseFloat(to[1]) - parseFloat(to[0]))) /
                (parseFloat(from[1]) - parseFloat(from[0]))
            );
        };
        ...
        BDFDB.NumberUtils.compareVersions = function (newV, oldV) {
          ...
          for (let i = 0; i < length; i++)
            for (let iOutdated = false, j = 0; j <= i; j++) {
        ...
        BDFDB.DiscordUtils.getSetting = function (category, key) {
          ...
          return (
            BDFDB.LibraryStores.UserSettingsProtoStore &&
            BDFDB.LibraryStores.UserSettingsProtoStore.settings[category] &&
            BDFDB.LibraryStores.UserSettingsProtoStore.settings[category][key] &&
            BDFDB.LibraryStores.UserSettingsProtoStore.settings[category][key].value
          );
        };
        ...
        BDFDB.DiscordUtils.getSettingsStore = function () {
          return (
            BDFDB.LibraryModules.UserSettingsProtoUtils &&
            (Object.entries(BDFDB.LibraryModules.UserSettingsProtoUtils).find(
              (n) =>
                n &&
                n[1] &&
                n[1].updateAsync &&
        ...
        BDFDB.DiscordUtils.openLink = function (url, config = {}) {
          if (
            (config.inBuilt ||
              (config.inBuilt === undefined && Internal.settings.general.useChromium)) &&
            Internal.LibraryRequires.electron &&
            Internal.LibraryRequires.electron.remote
          ) {
            let browserWindow = new Internal.LibraryRequires.electron.remote.BrowserWindow({
              ...
              webPreferences: { nodeIntegration: false, nodeIntegrationInWorker: false },
            });
        ...
        window.DiscordNative &&
          window.DiscordNative.app &&
          window.DiscordNative.app.getPath("appData").then((path) => {
            BDFDB.DiscordUtils.getFolder.base = path;
          });
        BDFDB.DiscordUtils.isPlaformEmbedded = function () {
          return (
            (Internal.LibraryModules.PlatformUtils &&
              (Object.entries(Internal.LibraryModules.PlatformUtils).find(
                (n) => typeof n[1] == "boolean",
              ) || [])[1]) ||
            false
          );
        };
        BDFDB.DiscordUtils.getFolder = function () {
          if (!BDFDB.DiscordUtils.getFolder.base) return "";
            ...
            try {
              ...
              folder = Internal.LibraryRequires.path.resolve(
                BDFDB.DiscordUtils.getFolder.base,
                build,
                BDFDB.DiscordUtils.getVersion(),
              );
            } catch (err) {
              folder = BDFDB.DiscordUtils.getFolder.base;
            }
        ...
        BDFDB.DiscordUtils.getLanguage = function () {
          return (
            (Internal.LibraryModules.LanguageUtils &&
              (Internal.LibraryModules.LanguageUtils.chosenLocale ||
                Internal.LibraryModules.LanguageUtils._chosenLocale)) ||
            (Internal.LibraryModules.LanguageIntlUtils &&
              Internal.LibraryModules.LanguageIntlUtils.getSystemLocale &&
              Internal.LibraryModules.LanguageIntlUtils.getSystemLocale &&
              Internal.LibraryModules.LanguageIntlUtils.getSystemLocale()) ||
        ...
        BDFDB.DiscordUtils.getBuild = function () {
          if (BDFDB.DiscordUtils.getBuild.build) return BDFDB.DiscordUtils.getBuild.build;
            ...
            let build;
            try {
              build = window.DiscordNative.app.getReleaseChannel();
            } catch (err) {
              let version = BDFDB.DiscordUtils.getVersion();
              if (version) {
                version = version.split(".");
                if (version.length == 3 && !isNaN((version = parseInt(version[2]))))
                  build = version > 300 ? "stable" : version > 200 ? "canary" : "ptb";
                else build = "stable";
              } else build = "stable";
            }
        ...
        BDFDB.DiscordUtils.getVersion = function () {
          if (BDFDB.DiscordUtils.getVersion.version) return BDFDB.DiscordUtils.getVersion.version;
            ...
            let version;
            try {
              version = window.DiscordNative.app.getVersion();
            } catch (err) {
              version = "999.999.9999";
            }
            return (BDFDB.DiscordUtils.getVersion.version = version);
          }
        };
        BDFDB.DiscordUtils.getTheme = function () {
          return BDFDB.LibraryStores.ThemeStore.theme == "light"
            ? BDFDB.disCN.themelight
            : BDFDB.disCN.themedark;
        };
        ...
        BDFDB.DiscordUtils.shake = function () {
          BDFDB.ReactUtils.findOwner(document.querySelector(BDFDB.dotCN.appcontainer), {
            name: "Shakeable",
            unlimited: true,
            up: true,
          }).shake();
        ...
        BDFDB.DiscordUtils.rerenderAll = function (instant) {
          ...
          BDFDB.DiscordUtils.rerenderAll.timeout = BDFDB.TimeUtils.timeout(
            (_) => {
              let LayersProviderIns = BDFDB.ReactUtils.findOwner(
                document.querySelector(BDFDB.dotCN.layers),
                { name: "LayersProvider", unlimited: true, up: true },
              );
              let LayersProviderType =
                LayersProviderIns &&
                BDFDB.ObjectUtils.get(LayersProviderIns, `${BDFDB.ReactUtils.instanceKey}.type`);
              if (!LayersProviderType) return;
              let parentSelector = "",
                notices = document.querySelector("#bd-notices");
              if (notices) {
                let parentClasses = [];
                for (
                  let i = 0, parent = notices.parentElement;
                  i < 3;
                ...
                parentSelector = parentClasses
                  .reverse()
                  .map((n) => (!n ? "*" : `.${n.split(" ").join(".")}`))
                  .join(" > ");
              }
              BDFDB.PatchUtils.patch(
                { name: "BDFDB DiscordUtils" },
                LayersProviderType.prototype,
                "render",
                {
                  after: (e) => {
                    e.returnValue = BDFDB.ReactUtils.createElement(
                      LayersProviderType,
                      LayersProviderIns.props,
                    );
                      ...
                      BDFDB.TimeUtils.timeout((_) => {
                        if (!document.contains(notices)) {
                          let parent =
                            ...
                            document.querySelector(BDFDB.dotCN.app).parentElement;
                          if (parent) {
                            parent.insertBefore(notices, parent.firstElementChild);
                            let ZeresPluginLibrary =
                              notices.querySelector("#outdated-plugins") &&
                              BDFDB.BDUtils.getPlugin("ZeresPluginLibrary");
                            let updateChecker =
                              ZeresPluginLibrary &&
                              ZeresPluginLibrary?.Library?.PluginUpdater?.checkAllPlugins;
                            if (typeof updateChecker == "function")
                              updateChecker.apply(ZeresPluginLibrary.Library.PluginUpdater);
                          }
                        }
                      }, 1000);
                  },
        ...
        let classCache = BDFDB.DataUtils.load(BDFDB, "ClassModuleCache") || {},
          classSave = null;
          ...
          get: function (_, item) {
            ...
            if (item in classCache) {
              ...
              if (
                m &&
                m != window &&
                [InternalData.DiscordClassModules[item].props]
                  .flat(10)
                  .every((p) => typeof m[p] == "string")
              )
                return (DiscordClassModules[item] = m);
              delete classCache[item];
            }
            DiscordClassModules[item] = BDFDB.ModuleUtils.findStringObject(
              InternalData.DiscordClassModules[item].props,
              Object.assign({}, InternalData.DiscordClassModules[item]),
            );
            if (DiscordClassModules[item]) {
              let req = Internal.getWebModuleReq();
              for (let i in req.c)
                if (req.c[i].exports === DiscordClassModules[item]) {
                  classCache[item] = i;
                  break;
                }
            } else classCache[item] = false;
            if (!classSave)
              classSave = setTimeout(() => {
                classSave = null;
        ...
        for (let item in InternalData.DiscordClassModules)
          if (!DiscordClassModules[item]) DiscordClassModules[item] = undefined;
        ...
        Internal.getDiscordClass = function (item, selector) {
          let className,
            fallbackClassName,
            notFoundAndLazyloaded = false;
          className =
            fallbackClassName = `${Internal.DiscordClassModules.BDFDB.BDFDBundefined}_${Internal.generateClassId()}`;
          if (DiscordClasses[item] === undefined) {
            BDFDB.LogUtils.warn([item, "not found in DiscordClasses"]);
            return className;
          } else if (
            !BDFDB.ArrayUtils.is(DiscordClasses[item]) ||
            DiscordClasses[item].length != 2
          ) {
            ...
            if (
              !InternalData.LazyloadedClassModules ||
              !InternalData.LazyloadedClassModules[DiscordClasses[item][0]]
            ) {
              BDFDB.LogUtils.warn([DiscordClasses[item][0], "not found in DiscordClassModules"]);
              return className;
            } else notFoundAndLazyloaded = true;
          } else if (
            [DiscordClasses[item][1]].flat().every(
              (prop) =>
                Internal.DiscordClassModules[DiscordClasses[item][0]][prop] === undefined &&
                !(
                  JSON.stringify(Internal.DiscordClassModules[DiscordClasses[item][0]])
                    .split(" ")
                    .find((n) => n.startsWith(`${prop}_`) || n.endsWith(`-${prop}`)) || ""
                ).split('"')[0],
            )
          ) {
            if (
              ...
              BDFDB.LogUtils.warn([
                DiscordClasses[item][1],
                "not found in",
                DiscordClasses[item][0],
                "in DiscordClassModules",
              ]);
              return className;
            } else notFoundAndLazyloaded = true;
          }
          if (notFoundAndLazyloaded) {
            ...
            DiscordClassModules[DiscordClasses[item][0]] = Object.assign(
              {},
              DiscordClassModules[item],
              { [DiscordClasses[item][1]]: className },
            );
          } else
            for (let prop of [DiscordClasses[item][1]].flat()) {
              className =
                Internal.DiscordClassModules[DiscordClasses[item][0]][prop] ||
                (
                  JSON.stringify(Internal.DiscordClassModules[DiscordClasses[item][0]])
                    .split(" ")
                    .find((n) => n.startsWith(`${prop}_`) || n.endsWith(`-${prop}`)) || ""
        ...
        Internal.generateClassId = function () {
          let id = "";
          while (id.length < 6)
            id += generationChars[Math.floor(Math.random() * generationChars.length)];
          return id;
        };
        ...
        const LanguageStringsObj =
          (Internal.LibraryModules.LanguageStore &&
            Internal.LibraryModules.LanguageStore.Messages) ||
          Internal.LibraryModules.LanguageStore ||
          {};
        const EngLanguageStringsObj =
          (Internal.LibraryModules.EngLanguageStore &&
            Internal.LibraryModules.EngLanguageStore.Messages) ||
          Internal.LibraryModules.EngLanguageStore ||
          {};
        const LanguageStringFormatter =
          Internal.LibraryModules.LanguageIntlUtils &&
          Internal.LibraryModules.LanguageIntlUtils.formatToPlainString;
        var LanguageStringFormattersObj =
          BDFDB.ModuleUtils.findByString("use strict", "createLoader:", {
            exportsFilter: (m) => !m.messagesLoader,
            all: true,
          }).find(
            (n) =>
              n &&
              ...
              [InternalData.LanguageStringHashes.DISCORD]
                .flat(10)
                .some((hash) => LanguageStringFormatter((n.A && n.A[hash]) || (n.Z && n.Z[hash]))),
          ) || {};
        LanguageStringFormattersObj =
          LanguageStringFormattersObj.A || LanguageStringFormattersObj.Z;

        const LanguageHashes = {};
        for (let hash of Object.keys(EngLanguageStringsObj))
          LanguageHashes[
            (typeof EngLanguageStringsObj[hash] == "string"
              ? [EngLanguageStringsObj[hash]]
              : EngLanguageStringsObj[hash]
            )
              .map((n) => (typeof n == "string" ? n : "PLACEHOLDER"))
              .join(" ")
              .toUpperCase()
              .replace(/[^a-zA-Z0-9 ]/g, "")
              .split(" ")
        ...
        BDFDB.LanguageUtils.getLanguage = function () {
          ...
          lang =
            langId2 && langId.toUpperCase() !== langId2.toUpperCase()
              ? langId + "-" + langId2
              : langId;
          return (
            BDFDB.LanguageUtils.languages[lang] ||
            BDFDB.LanguageUtils.languages[langId] ||
            BDFDB.LanguageUtils.languages.en
          );
        };
        BDFDB.LanguageUtils.getName = function (language) {
          ...
          if (language.name.startsWith("Discord"))
            return (
              ...
              (language.ownlang &&
              (BDFDB.LanguageUtils.languages[language.id] || {}).name != language.ownlang
                ? ` / ${language.ownlang}`
                : "") +
              ")"
            );
        ...
        BDFDB.LanguageUtils.LanguageStrings = new Proxy(LanguageHashes, {
          get: function (list, item) {
            ...
            if (!stringObj)
              BDFDB.LogUtils.warn([item, "not found in BDFDB.LanguageUtils.LanguageStrings"]);
            else {
              if (
                (stringObj &&
                  typeof stringObj == "object" &&
                  typeof stringObj.format == "function") ||
                BDFDB.ArrayUtils.is(stringObj)
              )
                return BDFDB.LanguageUtils.LanguageStringsFormat(item);
        ...
        let parseLanguageStringObj = (obj) => {
          ...
          } else if (BDFDB.ArrayUtils.is(obj))
            for (let ele of obj) string += parseLanguageStringObj(ele);
          ...
          if (item) {
            ...
            if (
              ...
              while (!returnvalue && i < 10) {
                i++;
                try {
                  returnvalue = BDFDB.ArrayUtils.is(stringObj)
                    ? LanguageStringFormatter(
                        LanguageStringFormattersObj[LanguageHashes[item]],
                        formatVars,
                      )
                    : stringObj.format(formatVars, false);
                } catch (err) {
                  error += "Error 1 " + err + "\n";
                  returnvalue = null;
                  ...
                  if (stringObj.intMessage) {
                    try {
                      for (let hook of stringObj.intMessage
                        .format(formatVars)
                        .match(/\([^\(\)]+\)/gi))
                        formatVars[hook.replace(/[\(\)]/g, "")] = (n) => n;
                    } catch (err2) {
                      error += "Error 2 " + err2 + "\n";
                    }
                  }
                  if (stringObj.intlMessage) {
                    try {
                      for (let hook of stringObj.intlMessage
                        .format(formatVars)
                        .match(/\([^\(\)]+\)/gi))
                        formatVars[hook.replace(/[\(\)]/g, "")] = (n) => n;
                    } catch (err3) {
              ...
              if (returnvalue) return parseLanguageStringObj(returnvalue);
                ...
                BDFDB.LogUtils.warn([
                  item,
                  "failed to format string in BDFDB.LanguageUtils.LanguageStrings",
                  error,
                ]);
                return "";
              }
            ...
            BDFDB.LogUtils.warn([
              item,
              "enter a valid key to format the string in BDFDB.LanguageUtils.LanguageStrings",
            ]);
          return "";
        };
        BDFDB.LanguageUtils.LibraryStrings = new Proxy(LibraryStrings.default || {}, {
          get: function (list, item) {
            ...
            if (LibraryStrings[languageId] && LibraryStrings[languageId][item])
              return LibraryStrings[languageId][item];
        ...
        BDFDB.LanguageUtils.LibraryStringsFormat = function (item, ...values) {
          if (item) {
            let languageId = BDFDB.LanguageUtils.getLanguage().id,
              string = null;
            if (LibraryStrings[languageId] && LibraryStrings[languageId][item])
              string = LibraryStrings[languageId][item];
            ...
            if (string) {
              for (let i = 0; i < values.length; i++)
                if (typeof values[i] == "string" || typeof values[i] == "number")
                  string = string.replace(new RegExp(`{{var${i}}}`, "g"), values[i]);
              return string;
            } else BDFDB.LogUtils.warn([item, "not found in BDFDB.LanguageUtils.LibraryStrings"]);
          } else
            BDFDB.LogUtils.warn([
              item,
              "enter a valid key to format the string in BDFDB.LanguageUtils.LibraryStrings",
            ]);
          return "";
        };
        BDFDB.TimeUtils.interval((interval) => {
          if (BDFDB.DiscordUtils.getLanguage()) {
            ...
            if (language)
              BDFDB.LanguageUtils.languages.$discord = Object.assign({}, language, {
                name: `Discord (${language.name})`,
              });
          }
        }, 100);
        for (let key in BDFDB.LanguageUtils.languages)
          try {
            if (new Date(0).toLocaleString(key, { second: "numeric" }) != "0") {
              BDFDB.LanguageUtils.languages[key].numberMap = {};
              for (let i = 0; i < 10; i++)
                BDFDB.LanguageUtils.languages[key].numberMap[i] = new Date(i * 1000).toLocaleString(
                  key,
                  { second: "numeric" },
                );
            }
        ...
        const reactInitialized =
          Internal.LibraryModules.React && Internal.LibraryModules.React.Component;
        Internal.setDefaultProps = function (component, defaultProps) {
          if (BDFDB.ObjectUtils.is(component))
            component.defaultProps = Object.assign({}, component.defaultProps, defaultProps);
        };
        let openedItem;
        Internal.MenuItem =
          reactInitialized &&
          class BDFDB_MenuItem extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
            ...
            render() {
              let color = (
                typeof this.props.color == "string"
                  ? this.props.color
                  : Internal.DiscordConstants.MenuItemColors.DEFAULT
              ).toLowerCase();
              ...
              if (color) {
                if (InternalData.DiscordClasses[`menucolor${color}`]) color = color;
                  ...
                  color = (Internal.DiscordConstants.MenuItemColors.DEFAULT || "").toLowerCase();
              }
              let renderPopout,
                onClose,
                hasPopout = BDFDB.ObjectUtils.is(this.props.popoutProps);
              if (hasPopout) {
                renderPopout = (instance) => {
                  openedItem = this.props.id;
                  return (
                    typeof this.props.popoutProps.renderPopout == "function" &&
                    this.props.popoutProps.renderPopout(instance)
                  );
                };
                onClose = (instance) => {
                  openedItem = null;
                  typeof this.props.popoutProps.onClose == "function" &&
                    this.props.popoutProps.onClose(instance);
                };
              }
              ...
              let item = BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.Clickable,
                Object.assign(
                  {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.menuitem,
                      (this.props.label || this.props.subtext) && BDFDB.disCN.menulabelcontainer,
                      color &&
                        (isCustomColor
                          ? BDFDB.disCN.menucolorcustom
                          : BDFDB.disCN[`menucolor${color}`]),
                      this.props.disabled && BDFDB.disCN.menudisabled,
                      focused && BDFDB.disCN.menufocused,
                    ),
                    style: {
                      color: isCustomColor
                        ? focused || this.state.hovered
                          ? BDFDB.ColorUtils.isBright(color)
                            ? "#000000"
                            : "#ffffff"
                          : color
                        : this.state.hovered
                    ...
                    children: [
                      ...
                      typeof this.props.render == "function"
                        ...
                        : this.props.render,
                      (this.props.label || this.props.subtext) &&
                        BDFDB.ReactUtils.createElement("div", {
                          ...
                          children: [
                            ...
                            this.props.subtext &&
                              BDFDB.ReactUtils.createElement("div", {
                                ...
                                children:
                                  typeof this.props.subtext == "function"
                                    ? this.props.subtext(this)
                                    : this.props.subtext,
                              }),
                          ].filter((n) => n),
                        }),
                      this.props.hint &&
                        ...
                        BDFDB.ReactUtils.createElement("div", {
                          ...
                          children: BDFDB.ReactUtils.createElement(
                            BDFDB.LibraryComponents.MenuHint,
                            {
                              children:
                                typeof this.props.hint == "function"
                                  ? this.props.hint(this)
                                  : this.props.hint,
                            },
                          ),
                        }),
              ...
              return hasPopout
                ? BDFDB.ReactUtils.createElement(
                    Internal.LibraryComponents.PopoutContainer,
                    Object.assign({}, this.props.popoutProps, {
                      children: item,
        ...
        Internal.CustomMenuItemWrapper =
          reactInitialized &&
          class BDFDB_CustomMenuItemWrapper extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
              this.state = { hovered: false };
            }
            render() {
              ...
              let item = BDFDB.ReactUtils.createElement(
                this.props.children,
                Object.assign({}, this.props.childProps, {
                  onMouseEnter: isItem
                    ? (e) => {
                        if (
                          this.props.childProps &&
                          typeof this.props.childProps.onMouseEnter == "function"
                        )
                          this.props.childProps.onMouseEnter(e, this);
                        this.setState({ hovered: true });
                      }
                  ...
                  onMouseLeave: isItem
                    ? (e) => {
                        if (
                          this.props.childProps &&
                          typeof this.props.childProps.onMouseLeave == "function"
                        )
                          this.props.childProps.onMouseLeave(e, this);
                        this.setState({ hovered: false });
                      }
              ...
              return isItem
                ? item
                : BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
        ...
        Internal.ErrorBoundary =
          reactInitialized &&
          class BDFDB_ErrorBoundary extends Internal.LibraryModules.React.PureComponent {
            constructor(props) {
              super(props);
            ...
            render() {
              if (this.state.hasError)
                return Internal.LibraryModules.React.createElement("span", {
                  style: {
        ...
        CustomComponents.AutoFocusCatcher =
          reactInitialized &&
          class BDFDB_AutoFocusCatcher extends Internal.LibraryModules.React.Component {
            render() {
              const style = {
                padding: 0,
                margin: 0,
              ...
              return BDFDB.ReactUtils.forceStyle(
                BDFDB.ReactUtils.createElement("input", { style }),
                Object.keys(style),
              );
            }
          };

        CustomComponents.BadgeAnimationContainer =
          reactInitialized &&
          class BDFDB_BadgeAnimationContainer extends Internal.LibraryModules.React.Component {
            componentDidMount() {
              BDFDB.ReactUtils.forceUpdate(this);
            }
            ...
            componentWillLeave(e) {
              if (typeof e == "function") this.timeoutId = setTimeout(e, 300);
            }
            componentWillUnmount() {
              BDFDB.TimeUtils.clear(this.timeoutId);
            }
            render() {
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.Animations.animated.div,
                {
        ...
        CustomComponents.Badges.IconBadge =
          reactInitialized &&
          class BDFDB_IconBadge extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.badgeiconbadge,
                  (this.props.shape &&
                    Internal.LibraryComponents.Badges.BadgeShapes[this.props.shape]) ||
                    Internal.LibraryComponents.Badges.BadgeShapes.ROUND,
                ),
                style: Object.assign(
                  {
                    backgroundColor: this.props.disableColor
                      ? null
                      : this.props.color || BDFDB.DiscordConstants.ColorsCSS.STATUS_DANGER,
                  },
                  this.props.style,
                ),
        ...
        CustomComponents.Badges.NumberBadge =
          reactInitialized &&
          class BDFDB_NumberBadge extends Internal.LibraryModules.React.Component {
            handleClick(e) {
            ...
            getBadgeWidthForValue(e) {
              return e < 10 ? 16 : e < 100 ? 22 : 30;
            }
            getBadgeCountString(e) {
            ...
            render() {
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.badgenumberbadge,
                  Internal.LibraryComponents.Badges &&
                    Internal.LibraryComponents.Badges.BadgeShapes &&
                    ((this.props.shape &&
                      Internal.LibraryComponents.Badges.BadgeShapes[this.props.shape]) ||
                      Internal.LibraryComponents.Badges.BadgeShapes.ROUND),
                ),
        ...
        CustomComponents.BotTag =
          reactInitialized &&
          class BDFDB_BotTag extends Internal.LibraryModules.React.Component {
            handleClick(e) {
              if (typeof this.props.onClick == "function") this.props.onClick(e, this);
            }
            handleContextMenu(e) {
            ...
            render() {
              return BDFDB.ReactUtils.createElement("span", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  this.props.invertColor ? BDFDB.disCN.bottaginvert : BDFDB.disCN.bottagregular,
                  this.props.useRemSizes ? BDFDB.disCN.bottagrem : BDFDB.disCN.bottagpx,
                ),
        ...
        CustomComponents.Button =
          reactInitialized &&
          class BDFDB_Button extends Internal.LibraryModules.React.Component {
            handleClick(e) {
              if (typeof this.props.onClick == "function") this.props.onClick(e, this);
            }
            handleContextMenu(e) {
            ...
            render() {
              ...
              let processingAndListening =
                (this.state.disabled || this.props.submitting) &&
                (null != this.props.onMouseEnter || null != this.props.onMouseLeave);
              let props = BDFDB.ObjectUtils.exclude(
                this.props,
                "look",
                "color",
                "hover",
              ...
              let button = BDFDB.ReactUtils.createElement(
                "button",
                Object.assign({}, !this.state.disabled && !this.props.submitting && props, {
                  className: BDFDB.DOMUtils.formatClassName(
                    this.props.className,
                    BDFDB.disCN.button,
                    this.props.look != null
                      ? this.props.look
                      : Internal.LibraryComponents.Button.Looks.FILLED,
                    this.props.color != null
                      ? this.props.color
                      : Internal.LibraryComponents.Button.Colors.BRAND,
                    this.props.hover,
                    this.props.size != null
                      ? this.props.size
                      : Internal.LibraryComponents.Button.Sizes.MEDIUM,
                    processingAndListening && this.props.wrapperClassName,
                    this.props.fullWidth && BDFDB.disCN.buttonfullwidth,
                    (this.props.grow === undefined || this.props.grow) && BDFDB.disCN.buttongrow,
                    this.props.submitting && BDFDB.disCN.buttonsubmitting,
                  ),
                  onClick:
                    this.state.disabled || this.props.submitting
                      ? (e) => {
                  ...
                  rel: this.props.rel,
                  children: [
                    this.props.submitting && !this.state.disabled
                      ? BDFDB.ReactUtils.createElement(
                          Internal.LibraryComponents.SpinnerComponents.Spinner,
                          {
                            type: Internal.LibraryComponents.SpinnerComponents.Types
                              .PULSING_ELLIPSIS,
              ...
              return !processingAndListening
                ? button
                : BDFDB.ReactUtils.createElement("span", {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.buttondisabledwrapper,
        ...
        CustomComponents.Card =
          reactInitialized &&
          class BDFDB_Card extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                "div",
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.hovercardwrapper,
                      this.props.horizontal && BDFDB.disCN.hovercardhorizontal,
                      this.props.backdrop && BDFDB.disCN.hovercard,
                      this.props.className,
                    ),
                    onMouseEnter: (e) => {
                      if (typeof this.props.onMouseEnter == "function")
                    ...
                    onContextMenu: (e) => {
                      if (typeof this.props.onContextMenu == "function")
                        this.props.onContextMenu(e, this);
                    },
                    children: [
                      !this.props.noRemove
                        ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                            ...
                            className:
                              BDFDB.disCNS.hovercardbutton +
                              BDFDB.disCNS.hovercardremovebutton +
                              BDFDB.disCN.hovercardremovebuttondefault,
                            children: BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.SvgIcon,
                              {
                                nativeClass: true,
                      ...
                      typeof this.props.children == "string"
                        ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TextElement, {
                            ...
                            children: BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.TextScroller,
                              { children: this.props.children },
                            ),
                          })
                        : this.props.children,
        ...
        CustomComponents.ChannelTextAreaButton =
          reactInitialized &&
          class BDFDB_ChannelTextAreaButton extends Internal.LibraryModules.React.Component {
            render() {
              const inner = BDFDB.ReactUtils.createElement("div", {
                ...
                children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                  ...
                  className: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.textareaicon,
                    this.props.iconClassName,
                    this.props.pulse && BDFDB.disCN.textareaiconpulse,
                  ),
              ...
              const button = BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Button, {
                ...
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.isActive && BDFDB.disCN.textareabuttonactive,
                ),
                innerClassName: BDFDB.DOMUtils.formatClassName(
                  BDFDB.disCN.textareabutton,
                  this.props.className,
                  this.props.pulse && BDFDB.disCN.textareaattachbuttonplus,
                ),
                ...
                children:
                  this.props.tooltip && this.props.tooltip.text
                    ? BDFDB.ReactUtils.createElement(
                        Internal.LibraryComponents.TooltipContainer,
                        Object.assign({}, this.props.tooltip, { children: inner }),
                      )
                    : inner,
              });
              return (this.props.className || "").indexOf(BDFDB.disCN.textareapickerbutton) > -1
                ? BDFDB.ReactUtils.createElement("div", {
        ...
        CustomComponents.CharCounter =
          reactInitialized &&
          class BDFDB_CharCounter extends Internal.LibraryModules.React.Component {
            getCounterString() {
              let input = this.refElement || {},
                string = "";
              if (BDFDB.DOMUtils.containsClass(this.refElement, BDFDB.disCN.textarea)) {
                let instance = BDFDB.ReactUtils.findOwner(input, {
                  name: "ChannelTextAreaEditor",
                  up: true,
                });
              ...
              if (
                ...
                (string.length / this.props.max) * 100 < this.props.showPercentage
              )
                return "";
              let start = input.selectionStart || 0,
                end = input.selectionEnd || 0,
                selectlength = end - start,
                selection = BDFDB.DOMUtils.getSelection();
              let select = !selectlength && !selection ? 0 : selectlength || selection.length;
              select = !select
                ? 0
                : select > string.length
                  ? end || start
                    ? string.length - (string.length - end - start)
                    : string.length
                  : select;
              let children = [
                typeof this.props.renderPrefix == "function" &&
                  this.props.renderPrefix(string.length),
                `${string.length}${!this.props.max ? "" : "/" + this.props.max}${!select ? "" : " (" + select + ")"}`,
                typeof this.props.renderSuffix == "function" &&
                  this.props.renderSuffix(string.length),
              ].filter((n) => n);
            ...
            handleSelection() {
              ...
              let mouseUp = (_) => {
                ...
                if (this.refElement.selectionEnd - this.refElement.selectionStart)
                  BDFDB.TimeUtils.timeout((_) => {
                ...
                if (node && node.parentElement) {
                  ...
                  if (this.refElement) {
                    if (!this._updateCounter)
                      this._updateCounter = (_) => {
                        if (!document.contains(node))
                          BDFDB.ListenerUtils.multiRemove(
                            this.refElement,
                            "keydown click change",
                            this._updateCounter,
                          );
                        else this.updateCounter();
                      };
                    if (!this._handleSelection)
                      this._handleSelection = (_) => {
                        if (!document.contains(node))
                          BDFDB.ListenerUtils.multiRemove(
                            this.refElement,
                            "mousedown",
                    ...
                    BDFDB.ListenerUtils.multiAdd(
                      this.refElement,
                      "mousedown",
                      this._handleSelection,
                    );
                    if (
                      this.refElement.tagName == "INPUT" ||
                      this.refElement.tagName == "TEXTAREA"
                    ) {
                      BDFDB.ListenerUtils.multiRemove(
                        this.refElement,
                        "keydown click change",
                      ...
                      if (!this._mutationObserver)
                        this._mutationObserver = new MutationObserver((changes) => {
                          if (!document.contains(node)) this._mutationObserver.disconnect();
                          else this.updateCounter();
                        });
                      else this._mutationObserver.disconnect();
                      this._mutationObserver.observe(this.refElement, {
                        childList: true,
                        subtree: true,
                      });
                    }
                    ...
                    BDFDB.LogUtils.warn(["could not find referenceElement for BDFDB_CharCounter"]);
                }
            ...
            render() {
              ...
              BDFDB.TimeUtils.timeout(
                (_) => string != this.getCounterString() && BDFDB.ReactUtils.forceUpdate(this),
              );
              return BDFDB.ReactUtils.createElement(
                "div",
                BDFDB.ObjectUtils.exclude(
        ...
        CustomComponents.Checkbox =
          reactInitialized &&
          class BDFDB_Checkbox extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleMouseDown(e) {
                  ...
                  getStyle() {
                    ...
                    this.props.color =
                      typeof this.props.getColor == "function"
                        ? this.props.getColor(this.props.value)
                        : this.props.color;
                    if (Internal.LibraryComponents.Checkbox.Types)
                  ...
                  getColor() {
                    return this.props.value
                      ? Internal.LibraryComponents.Checkbox.Types &&
                        this.props.type === Internal.LibraryComponents.Checkbox.Types.INVERTED
                        ? Internal.DiscordConstants.Colors.WHITE
                        : this.props.color
                      : "transparent";
                  }
                  handleChange(e) {
                    this.props.value =
                      typeof this.props.getValue == "function"
                        ? this.props.getValue(this.props.value, e, this)
                        : !this.props.value;
                  ...
                  render() {
                    let label = this.props.children
                      ? BDFDB.ReactUtils.createElement("div", {
                          className: BDFDB.DOMUtils.formatClassName(
                            BDFDB.disCN.checkboxlabel,
                            this.props.disabled
                              ? BDFDB.disCN.checkboxlabeldisabled
                              : BDFDB.disCN.checkboxlabelclickable,
                            this.props.reverse
                              ? BDFDB.disCN.checkboxlabelreversed
                              : BDFDB.disCN.checkboxlabelforward,
                          ),
                          style: { lineHeight: this.props.size + "px" },
                          children: this.props.children,
                        })
                      : null;
                    return BDFDB.ReactUtils.createElement("label", {
                      className: BDFDB.DOMUtils.formatClassName(
                        this.props.disabled
                          ? BDFDB.disCN.checkboxwrapperdisabled
                          : BDFDB.disCN.checkboxwrapper,
                        this.props.align,
                        this.props.className,
                      ),
                      children: [
                        ...
                        !this.props.displayOnly &&
                          BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.FocusRingScope,
                            {
                              children: BDFDB.ReactUtils.createElement("input", {
                                ...
                                onMouseEnter:
                                  !this.props.disabled && this.handleMouseEnter.bind(this),
                                onMouseLeave:
                                  !this.props.disabled && this.handleMouseLeave.bind(this),
                        ...
                        BDFDB.ReactUtils.createElement("div", {
                          className: BDFDB.DOMUtils.formatClassName(
                            BDFDB.disCN.checkbox,
                            BDFDB.disCN["checkbox" + this.props.shape],
                            this.props.value && BDFDB.disCN.checkboxchecked,
                          ),
                          style: Object.assign(
                            {
                          ...
                          children: BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.Checkmark,
                            {
                              size: "null",
                              width: 18,
                              height: 18,
        ...
        if (CustomComponents.Checkbox) {
          ...
          CustomComponents.Checkbox.Shapes = { BOX: "box", ROUND: "round" };
          Internal.setDefaultProps(CustomComponents.Checkbox, {
            type: CustomComponents.Checkbox.Types.INVERTED,
            shape: CustomComponents.Checkbox.Shapes.ROUND,
          });
        }

        CustomComponents.Clickable =
          reactInitialized &&
          class BDFDB_Clickable extends Internal.LibraryModules.React.Component {
            handleClick(e) {
              if (typeof this.props.onClick == "function") this.props.onClick(e, this);
            }
            handleContextMenu(e) {
            ...
            render() {
              return BDFDB.ReactUtils.createElement(
                Internal.NativeSubComponents.Clickable,
                Object.assign({}, this.props, {
                  className: BDFDB.DOMUtils.formatClassName(
                    this.props.className,
                    (this.props.className || "").toLowerCase().indexOf("disabled") == -1 &&
                      BDFDB.disCN.cursorpointer,
                  ),
                  onClick: this.handleClick.bind(this),
                  onContextMenu: this.handleContextMenu.bind(this),
        ...
        CustomComponents.CollapseContainer =
          reactInitialized &&
          class BDFDB_CollapseContainer extends Internal.LibraryModules.React.Component {
            render() {
              ...
              this.props.collapsed =
                ...
                (this.props.collapseStates[this.props.title] ||
                  this.props.collapseStates[this.props.title] === undefined);
              ...
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  ...
                  this.props.mini
                    ? BDFDB.disCN.collapsecontainermini
                    : BDFDB.disCN.collapsecontainer,
                  this.props.className,
                ),
                id: this.props.id,
                children: [
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                    ...
                    onClick: (e) => {
                      ...
                      if (typeof this.props.onClick == "function")
                        this.props.onClick(this.props.collapsed, this);
                      BDFDB.ReactUtils.forceUpdate(this);
                    },
                    children: BDFDB.ReactUtils.createElement(
                      Internal.LibraryComponents.FormTitle.Title,
                      {
                        tag:
                          Internal.LibraryComponents.FormTitle.Tags &&
                          Internal.LibraryComponents.FormTitle.Tags.H5,
        ...
        Internal.setDefaultProps(CustomComponents.CollapseContainer, {
          collapsed: true,
          mini: true,
        });

        CustomComponents.ColorPicker =
          reactInitialized &&
          class BDFDB_ColorPicker extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
              if (!this.state) this.state = {};
              this.state.isGradient =
                props.gradient && props.color && BDFDB.ObjectUtils.is(props.color);
            ...
            handleColorChange(color) {
              let changed = false;
              if (color != null) {
                changed = !BDFDB.equals(
                  this.state.isGradient
                    ? this.props.color[this.state.selectedGradientCursor]
                    : this.props.color,
                  color,
                );
                if (this.state.isGradient)
                  this.props.color[this.state.selectedGradientCursor] = color;
                else this.props.color = color;
              } else changed = true;
              if (changed) {
                if (typeof this.props.onColorChange == "function")
                  this.props.onColorChange(BDFDB.ColorUtils.convert(this.props.color, "RGBCOMP"));
                BDFDB.ReactUtils.forceUpdate(this);
              }
            }
            render() {
              ...
              let hexRegex = this.props.alpha
                ? /^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i
                : /^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i;

              let selectedColor =
                BDFDB.ColorUtils.convert(
                  this.state.isGradient
                    ? this.props.color[this.state.selectedGradientCursor]
                    : this.props.color,
                  colorFormat,
                ) || BDFDB.ColorUtils.convert("#000000FF", colorFormat);
              let currentGradient = this.state.isGradient
                ? Object.entries(this.props.color, colorFormat)
                : [
                    [0, selectedColor],
                    [1, selectedColor],
                  ];
              ...
              let hexColor = BDFDB.ColorUtils.convert(
                selectedColor,
                this.props.alpha ? "HEXA" : "HEX",
              );
                ...
                children: [
                      ...
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: BDFDB.ReactUtils.createElement("div", {
                          ...
                          style: {
                            ...
                            left: 0,
                            cursor: "crosshair",
                            backgroundColor: BDFDB.ColorUtils.convert([h, "100%", "50%"], "RGB"),
                          },
                          onClick: (event) => {
                            let rects = BDFDB.DOMUtils.getRects(
                              BDFDB.DOMUtils.getParent(
                                BDFDB.dotCN.colorpickersaturationcolor,
                                event.target,
                              ),
                            );
                            this.handleColorChange(
                              BDFDB.ColorUtils.convert(
                                [
                                  h,
                                  BDFDB.NumberUtils.mapRange(
                                    [rects.left, rects.left + rects.width],
                                    [0, 100],
                                    event.clientX,
                                  ) + "%",
                                  BDFDB.NumberUtils.mapRange(
                                    [rects.top, rects.top + rects.height],
                                    [100, 0],
                                    event.clientY,
                                  ) + "%",
                                  a,
                              ...
                              children: [
                                ...
                                BDFDB.ReactUtils.createElement("div", {
                                  ...
                                  style: {
                                    ...
                                    cursor: "crosshair",
                                    left: s,
                                    top: `${BDFDB.NumberUtils.mapRange([0, 100], [100, 0], parseFloat(v))}%`,
                                  },
                                  children: BDFDB.ReactUtils.createElement("div", {
                                    style: {
                                      width: 4,
                        ...
                        children: BDFDB.ReactUtils.createElement("div", {
                          ...
                          children: BDFDB.ReactUtils.createElement("div", {
                            ...
                            onClick: (event) => {
                              let rects = BDFDB.DOMUtils.getRects(
                                BDFDB.DOMUtils.getParent(
                                  BDFDB.dotCN.colorpickerhuehorizontal,
                                  event.target,
                                ),
                              );
                              this.handleColorChange(
                            ...
                            children: [
                              ...
                              BDFDB.ReactUtils.createElement("div", {
                                ...
                                style: {
                                  position: "absolute",
                                  cursor: "ew-resize",
                                  left: `${BDFDB.NumberUtils.mapRange([0, 360], [0, 100], h)}%`,
                                },
                                children: BDFDB.ReactUtils.createElement("div", {
                                  style: {
                                    marginTop: 1,
                            ...
                            BDFDB.ReactUtils.createElement("div", {
                              ...
                              children: BDFDB.ReactUtils.createElement("div", {
                                ...
                                style: {
                                  ...
                                  position: "relative",
                                  height: "100%",
                                  background: `linear-gradient(to right, ${BDFDB.ColorUtils.setAlpha([h, s, v], 0, "RGBA")}, ${BDFDB.ColorUtils.setAlpha([h, s, v], 1, "RGBA")}`,
                                },
                                onClick: (event) => {
                                  let rects = BDFDB.DOMUtils.getRects(
                                    BDFDB.DOMUtils.getParent(
                                      BDFDB.dotCN.colorpickeralphahorizontal,
                                      event.target,
                                    ),
                                  );
                                  this.handleColorChange(
                                    BDFDB.ColorUtils.setAlpha(
                                      [h, s, v],
                                      BDFDB.NumberUtils.mapRange(
                                        [rects.left, rects.left + rects.width],
                                        [0, 1],
                                ...
                                children: BDFDB.ReactUtils.createElement("div", {
                                  ...
                                  children: [
                                    ...
                                    this.state.draggingAlphaCursor &&
                                      BDFDB.ReactUtils.createElement("span", {
                            ...
                            BDFDB.ReactUtils.createElement("div", {
                              ...
                              children: BDFDB.ReactUtils.createElement("div", {
                                ...
                                style: {
                                  ...
                                  cursor: "copy",
                                  height: "100%",
                                  background: BDFDB.ColorUtils.createGradient(
                                    currentGradient.reduce(
                                      (colorObj, posAndColor) => (
                                        (colorObj[posAndColor[0]] = posAndColor[1]), colorObj
                                      ),
                                      {},
                                    ),
                                  ),
                                ...
                                onClick: (event) => {
                                  ...
                                  let pos = BDFDB.NumberUtils.mapRange(
                                    [rects.left, rects.left + rects.width],
                                    [0.01, 0.99],
                                    event.clientX,
                                  );
                                  if (Object.keys(this.props.color).indexOf(pos) == -1) {
                                    this.props.color[pos] = BDFDB.ColorUtils.convert(
                                      "#000000FF",
                                      colorFormat,
                                    );
                                ...
                                children: currentGradient.map((posAndColor) =>
                                  BDFDB.ReactUtils.createElement("div", {
                                    className: BDFDB.DOMUtils.formatClassName(
                                      BDFDB.disCN.colorpickergradientcursor,
                                      (posAndColor[0] == 0 || posAndColor[0] == 1) &&
                                        BDFDB.disCN.colorpickergradientcursoredge,
                                      this.state.selectedGradientCursor == posAndColor[0] &&
                                        BDFDB.disCN.colorpickergradientcursorselected,
                                    ),
                                    style: {
                                      position: "absolute",
                                      cursor: "pointer",
                                      left: `${posAndColor[0] * 100}%`,
                                    },
                                    onMouseDown:
                                      posAndColor[0] == 0 || posAndColor[0] == 1
                                        ? (_) => {}
                                        : (event) => {
                                            ...
                                            let mouseMove = (event2) => {
                                              if (
                                                Math.sqrt((event.pageX - event2.pageX) ** 2) > 10
                                              ) {
                                                ...
                                                let cursor = BDFDB.DOMUtils.getParent(
                                                  BDFDB.dotCN.colorpickergradientcursor,
                                                  event.target,
                                                );
                                                let rects = BDFDB.DOMUtils.getRects(
                                                ...
                                                let dragging = (event3) => {
                                                  ...
                                                  if (
                                                    Object.keys(this.props.color).indexOf(pos) == -1
                                                  ) {
                                    ...
                                    children: BDFDB.ReactUtils.createElement("div", {
                                      style: {
                                        background: BDFDB.ColorUtils.convert(
                                          posAndColor[1],
                                          "RGBA",
                                        ),
                                      },
                  ...
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TextInput, {
                    ...
                    onChange: (value) => {
                      ...
                      if (this.props.alpha && (oldLength > 8 || oldLength < 6) && hexLength == 7)
                        value += "FF";
                    ...
                    inputChildren:
                      ...
                      BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TooltipContainer, {
                        ...
                        children: BDFDB.ReactUtils.createElement(
                          ...
                          {
                            className: BDFDB.DOMUtils.formatClassName(
                              BDFDB.disCN.colorpickergradientbutton,
                              this.state.gradientBarEnabled &&
                                BDFDB.disCN.colorpickergradientbuttonenabled,
                            ),
                            children: BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.SvgIcon,
                              {
                            ...
                            onClick: (_) => {
                              ...
                              if (this.state.gradientBarEnabled && !this.state.isGradient)
                                this.props.color = { 0: selectedColor, 1: selectedColor };
                              else if (!this.state.gradientBarEnabled && this.state.isGradient)
                                this.props.color = selectedColor;
                              this.state.isGradient =
                                this.props.color && BDFDB.ObjectUtils.is(this.props.color);
                              this.handleColorChange();
                            },
                          },
                        ),
                      ...
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        onMouseDown: (e) => {
                          ...
                          let mouseMove = (e2) => {
                            ...
                            this.domElementRef.current.style.setProperty(
                              "left",
                              `${left}px`,
                              "important",
                            );
        ...
        CustomComponents.ColorSwatches =
          reactInitialized &&
          class BDFDB_ColorSwatches extends Internal.LibraryModules.React.Component {
            ColorSwatch(props) {
              ...
              let swatch = BDFDB.ReactUtils.createElement("button", {
                ...
                className: BDFDB.DOMUtils.formatClassName(
                  ...
                  props.isSingle && BDFDB.disCN.colorpickerswatchsingle,
                  props.isDisabled && BDFDB.disCN.colorpickerswatchdisabled,
                  props.isSelected && BDFDB.disCN.colorpickerswatchselected,
                  props.isCustom && BDFDB.disCN.colorpickerswatchcustom,
                  props.color == null && BDFDB.disCN.colorpickerswatchnocolor,
                ),
                number: props.number,
                ...
                onClick: (_) => {
                  if (!props.isSelected) {
                    let color =
                      props.isCustom && props.color == null
                        ? swatches.props.color ||
                          swatches.props.defaultCustomColor ||
                          "rgba(0, 0, 0, 1)"
                        : props.color;
                    if (typeof swatches.props.onColorChange == "function")
                      swatches.props.onColorChange(BDFDB.ColorUtils.convert(color, "RGBCOMP"));
                    swatches.props.color = color;
                    swatches.props.customColor = props.isCustom
                      ? color
                      : swatches.props.customColor;
                    swatches.props.customSelected = props.isCustom;
                    BDFDB.ReactUtils.forceUpdate(swatches);
                  }
                },
                style: Object.assign({}, props.style, {
                  background: BDFDB.ObjectUtils.is(props.color)
                    ? BDFDB.ColorUtils.createGradient(props.color)
                    : BDFDB.ColorUtils.convert(props.color, "RGBA"),
                }),
                children: [
                  props.isCustom || props.isSingle
                    ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                        ...
                        color: useWhite
                          ? Internal.DiscordConstants.Colors.WHITE
                          : Internal.DiscordConstants.Colors.BLACK,
                      })
                    : null,
                  props.isSelected && !props.isSingle
                    ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
              ...
              if (props.isCustom || props.isSingle || props.color == null)
                swatch = BDFDB.ReactUtils.createElement(
                  Internal.LibraryComponents.TooltipContainer,
                  {
                    text:
                      props.isCustom || props.isSingle
                        ? BDFDB.LanguageUtils.LanguageStrings.CUSTOM_COLOR
                        : BDFDB.LanguageUtils.LanguageStrings.DEFAULT,
                    tooltipConfig: { type: props.isSingle ? "top" : "bottom" },
                    children: swatch,
                  },
                );
              if (props.isCustom || props.isSingle)
                swatch = BDFDB.ReactUtils.createElement(
                  Internal.LibraryComponents.PopoutContainer,
                  {
                    children: swatch,
                    ...
                    onOpen: (_) => {
                      ...
                      if (typeof swatches.props.onPickerOpen == "function")
                        swatches.props.onPickerOpen(this);
                    },
                    onClose: (_) => {
                      delete swatches.props.pickerOpen;
                      if (typeof swatches.props.onPickerClose == "function")
                        swatches.props.onPickerClose(this);
                    },
                    renderPopout: (_) =>
                      BDFDB.ReactUtils.createElement(
                        Internal.LibraryComponents.ColorPicker,
                        Object.assign({}, swatches.props.pickerConfig, {
                          color: swatches.props.color,
                          onColorChange: (color) => {
                            if (typeof swatches.props.onColorChange == "function")
                              swatches.props.onColorChange(color);
                            props.color = color;
                            swatches.props.color = color;
              ...
              if (props.isCustom)
                swatch = BDFDB.ReactUtils.createElement("div", {
            ...
            render() {
              this.props.color = BDFDB.ObjectUtils.is(this.props.color)
                ? this.props.color
                : BDFDB.ColorUtils.convert(this.props.color, "RGBA");
              this.props.colors = (
                BDFDB.ArrayUtils.is(this.props.colors)
                  ? this.props.colors
                  : [
                      null,
                      5433630,
              ...
              ).map((c) => BDFDB.ColorUtils.convert(c, "RGBA"));
              this.props.colorRows = this.props.colors.length
                ? [
                    this.props.colors.slice(0, parseInt(this.props.colors.length / 2)),
                    this.props.colors.slice(parseInt(this.props.colors.length / 2)),
                  ]
                : [];
              this.props.customColor =
                !this.props.color ||
                (!this.props.customSelected && this.props.colors.indexOf(this.props.color) > -1)
                  ? null
                  : this.props.color;
              this.props.defaultCustomColor = BDFDB.ObjectUtils.is(this.props.defaultCustomColor)
                ? this.props.defaultCustomColor
                : BDFDB.ColorUtils.convert(this.props.defaultCustomColor, "RGBA");
              this.props.customSelected = !!this.props.customColor;
              this.props.pickerConfig = BDFDB.ObjectUtils.is(this.props.pickerConfig)
                ? this.props.pickerConfig
                : { gradient: true, alpha: true };

              const isSingle = !this.props.colors.length;
              return BDFDB.ReactUtils.createElement("div", {
                className: isSingle
                  ? BDFDB.disCN.colorpickerswatchsinglewrapper
                  : BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.colorpickerswatches,
                      BDFDB.disCN.colorpickerswatchescontainer,
                      this.props.disabled && BDFDB.disCN.colorpickerswatchesdisabled,
                    ),
                children: [
                  ...
                  !isSingle &&
                    BDFDB.ReactUtils.createElement("div", {
                      children: this.props.colorRows.map((row) =>
                        BDFDB.ReactUtils.createElement("div", {
        ...
        CustomComponents.DateInput =
          reactInitialized &&
          class BDFDB_DateInput extends Internal.LibraryModules.React.Component {
            renderFormatButton(props) {
              ...
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.PopoutContainer, {
                ...
                onClose: (instance) =>
                  BDFDB.DOMUtils.removeClass(
                    instance.domElementRef.current,
                    BDFDB.disCN.dateinputbuttonselected,
                  ),
                renderPopout: (instance) => {
                  BDFDB.DOMUtils.addClass(
                    instance.domElementRef.current,
            ...
            renderInfoButton(text, style) {
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TooltipContainer, {
                ...
                tooltipConfig: { type: "bottom", zIndex: 1009, maxWidth: 560 },
                children: BDFDB.ReactUtils.createElement("div", {
                  className: BDFDB.disCN.dateinputbutton,
                  style: Object.assign({}, style),
                  children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                      ...
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: [
                          BDFDB.ReactUtils.createElement("div", {
                            ...
                            children: [
                              BDFDB.ReactUtils.createElement(
                                class DateInputPreview
                                  extends Internal.LibraryModules.React.Component
                                {
                                  componentDidMount() {
                                    formatter = this;
                                  }
                                  render() {
                                    return BDFDB.ReactUtils.createElement(
                                      Internal.LibraryComponents.TextInput,
                                      {
                                        className: BDFDB.disCN.dateinputfield,
                                        placeholder:
                              ...
                              this.renderFormatButton({
                                ...
                                children: [
                                  BDFDB.ReactUtils.createElement(
                                    Internal.LibraryComponents.AutoFocusCatcher,
                                    {},
                                  ),
                                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                                    ...
                                    children: [
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.SettingsLabel,
                                        { label: BDFDB.LanguageUtils.LanguageStrings.LANGUAGE },
                                      ),
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.Select,
                                        {
                                          className: BDFDB.disCN.dateinputfield,
                                          value:
                                            this.props.language != null
                                              ? this.props.language
                                              : "$discord",
                                          options: Object.keys(BDFDB.LanguageUtils.languages).map(
                                            (id) => ({
                                              value: id,
                                              label: BDFDB.LanguageUtils.getName(
                                                BDFDB.LanguageUtils.languages[id],
                                              ),
                                            }),
                                          ),
                                          searchable: true,
                                    ...
                                    children: [
                                      ...
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.Select,
                                        {
                                          className: BDFDB.disCN.dateinputfield,
                                          value:
                                            this.props.timeOffset != null
                                              ? this.props.timeOffset
                                              : defaultOffset,
                                          options: [
                                            -12.0, -11.0, -10.0, -9.5, -9.0, -8.0, -7.0, -6.0, -5.0,
                                            -4.0, -3.5, -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0, 3.5,
                                            4.0, 4.5, 5.0, 5.5, 5.75, 6.0, 6.5, 7.0, 8.0, 8.75, 9.0,
                                            9.5, 10.0, 10.5, 11.0, 12.0, 12.75, 13.0, 14.0,
                                          ].map((offset) => ({
                                          ...
                                          onChange: (value) => {
                                            this.props.timeOffset =
                                              value == defaultOffset ? undefined : value;
                                            this.handleChange.apply(this, []);
                                            BDFDB.ReactUtils.forceUpdate(formatter, preview);
                                          },
                                        },
                          ...
                          BDFDB.ReactUtils.createElement(
                            class DateInputPreview extends Internal.LibraryModules.React.Component {
                              componentDidMount() {
                                preview = this;
                              }
                              render() {
                                return (
                                  !input.props.noPreview &&
                                  BDFDB.ReactUtils.createElement("div", {
                                    ...
                                    children: [
                                      input.props.prefix &&
                                        BDFDB.ReactUtils.createElement("div", {
                                          ...
                                          children:
                                            typeof input.props.prefix == "function"
                                              ? input.props.prefix(input)
                                              : input.props.prefix,
                                        }),
                                      BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.TextScroller,
                                        {
                                          children: Internal.LibraryComponents.DateInput.format(
                                            input.props,
                                      ...
                                      input.props.suffix &&
                                        BDFDB.ReactUtils.createElement("div", {
                                          ...
                                          children:
                                            typeof input.props.suffix == "function"
                                              ? input.props.suffix(input)
                                              : input.props.suffix,
                                        }),
                                    ].filter((n) => n),
                                  })
        ...
        if (CustomComponents.DateInput) {
          CustomComponents.DateInput.getDefaultString = function (language) {
            ...
            return date
              ...
              .replace(date.toLocaleDateString(language), "$date")
              .replace(date.toLocaleTimeString(language, { hourCycle: "h12" }), "$time12")
              .replace(date.toLocaleTimeString(language, { hourCycle: "h11" }), "$time12")
              .replace(date.toLocaleTimeString(language, { hourCycle: "h24" }), "$time")
              .replace(date.toLocaleTimeString(language, { hourCycle: "h23" }), "$time");
          };
          CustomComponents.DateInput.parseDate = function (date, offset) {
            ...
            if (typeof timeObj == "string") {
              ...
              for (let i = 0; i < 12; i++) {
                ...
                timeObj = timeObj.replace(
                  tempDate.toLocaleDateString(language, { month: "long" }),
                  tempDate.toLocaleDateString("en", { month: "short" }),
                );
              }
            ...
            if (
              timeObj.toString() == "Invalid Date" ||
              typeof timeObj.toLocaleDateString != "function"
            )
              timeObj = new Date();
            offset = offset != null && parseFloat(offset);
            if ((offset || offset === 0) && !isNaN(offset))
              timeObj = new Date(
                timeObj.getTime() +
                  (offset - timeObj.getTimezoneOffset() * (-1 / 60)) * 60 * 60 * 1000,
              );
            return timeObj;
          };
          CustomComponents.DateInput.format = function (data, time) {
            ...
            const daysAgo = Math.round(
              (Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()) -
                Date.UTC(timeObj.getFullYear(), timeObj.getMonth(), timeObj.getDate())) /
                (1000 * 60 * 60 * 24),
            );
            const date =
              data.dateString && typeof data.dateString == "string"
                ? Internal.LibraryComponents.DateInput.formatDate(
                    { dateString: data.dateString, language: language },
                    timeObj,
                  )
                : timeObj.toLocaleDateString(language);

            return (
              data.formatString || Internal.LibraryComponents.DateInput.getDefaultString(language)
            )
              .replace(/\$date/g, date)
              .replace(
                /\$time12/g,
                data.timeString && typeof data.timeString == "string"
                  ? Internal.LibraryComponents.DateInput.formatTime(
                      { timeString: data.timeString, language: language },
                      timeObj,
                      true,
                    )
                  : timeObj.toLocaleTimeString(language, { hourCycle: "h12" }),
              )
              .replace(
                /\$time/g,
                data.timeString && typeof data.timeString == "string"
                  ...
                  : timeObj.toLocaleTimeString(language, { hourCycle: "h23" }),
              )
              ...
              .replace(
                /\$agoAmount/g,
                daysAgo < 0 || daysAgo > 1
                  ? Internal.DiscordObjects.Timestamp(timeObj.getTime()).fromNow()
                  : BDFDB.LanguageUtils.LanguageStrings[daysAgo == 1 ? "YESTERDAY" : "TODAY"],
              )
              .replace(
                /\$agoWeekdayS/g,
                daysAgo < 0 || daysAgo > 1
              ...
              .replace(
                /\$agoWeekday/g,
                daysAgo < 0 || daysAgo > 1
                  ? timeObj.toLocaleDateString(language, { weekday: "long" })
                  : BDFDB.LanguageUtils.LanguageStrings[daysAgo == 1 ? "YESTERDAY" : "TODAY"],
              )
              .replace(
                /\$agoDays/g,
                daysAgo < 0
                  ? ""
                  : daysAgo > 1
                    ? BDFDB.LanguageUtils.LanguageStringsFormat(`LAST_PLAYED_PLACEHOLDER`, daysAgo)
                    : BDFDB.LanguageUtils.LanguageStrings[daysAgo == 1 ? "YESTERDAY" : "TODAY"],
              )
              .replace(
                /\$agoDate/g,
          ...
          CustomComponents.DateInput.formatTime = function (data, time, hour12) {
            ...
            let digits = "\\d";
            if (
              BDFDB.LanguageUtils.languages[language] &&
              BDFDB.LanguageUtils.languages[language].numberMap
            ) {
              digits = Object.entries(BDFDB.LanguageUtils.languages[language].numberMap)
                .map((n) => n[1])
                .join("");
              for (let number in BDFDB.LanguageUtils.languages[language].numberMap)
                string = string.replace(
                  new RegExp(number, "g"),
                  BDFDB.LanguageUtils.languages[language].numberMap[number],
                );
            }
            return hour12
              ? timeObj
                  .toLocaleTimeString(language, { hourCycle: "h12" })
                  .replace(
                    new RegExp(
                      `[${digits}]{1,2}[^${digits}][${digits}]{1,2}[^${digits}][${digits}]{1,2}`,
                      "g",
                    ),
                    string,
                  )
        ...
        CustomComponents.EmojiPickerButton =
          reactInitialized &&
          class BDFDB_EmojiPickerButton extends Internal.LibraryModules.React.Component {
            handleEmojiChange(emoji) {
              if (emoji != null) {
                this.props.emoji = emoji.id
                  ? { id: emoji.id, name: emoji.name, animated: emoji.animated }
                  : { id: null, name: emoji.optionallyDiverseSequence, animated: false };
                if (typeof this.props.onSelect == "function")
                  this.props.onSelect(this.props.emoji, this);
                if (typeof this.close == "function" && !BDFDB.ListenerUtils.isPressed(16))
                  this.close();
                BDFDB.ReactUtils.forceUpdate(this);
              }
            }
            render() {
              ...
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.PopoutContainer, {
                children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.EmojiButton, {
                  className: BDFDB.DOMUtils.formatClassName(
                    this.props.className,
                    BDFDB.disCN.emojiinputbutton,
                  ),
                  renderButtonContents: this.props.emoji
                    ? (_) =>
                        BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Emoji, {
                ...
                position:
                  this.props.position || Internal.LibraryComponents.PopoutContainer.Positions.TOP,
                ...
                renderPopout: (instance) => {
                  ...
                  return [
                    ...
                    BDFDB.ReactUtils.createElement(
                      class extends Internal.LibraryModules.React.Component {
                        componentDidMount() {
                          Internal.LibraryComponents.EmojiPickerButton.current = button;
                        }
                        componentWillUnmount() {
                          delete Internal.LibraryComponents.EmojiPickerButton.current;
                        }
                        render() {
                          return null;
                        }
        ...
        Internal.setDefaultProps(CustomComponents.EmojiPickerButton, {
          allowManagedEmojis: false,
          allowManagedEmojisUsage: false,
        });

        CustomComponents.EmptyStateImage =
          reactInitialized &&
          class BDFDB_EmptyStateImage extends Internal.LibraryModules.React.Component {
            render() {
              ...
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                ...
                children: [
                  BDFDB.ReactUtils.createElement("div", {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.pageimage,
                      BDFDB.disCN.margintop20,
                      BDFDB.disCN.marginbottom20,
        ...
        CustomComponents.FavButton =
          reactInitialized &&
          class BDFDB_FavButton extends Internal.LibraryModules.React.Component {
            handleClick() {
              this.props.isFavorite = !this.props.isFavorite;
              if (typeof this.props.onClick == "function")
                this.props.onClick(this.props.isFavorite, this);
              BDFDB.ReactUtils.forceUpdate(this);
            }
            render() {
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                className: BDFDB.DOMUtils.formatClassName(
                  ...
                  BDFDB.disCN.favbutton,
                  this.props.isFavorite && BDFDB.disCN.favbuttonselected,
                  this.props.className,
                ),
                onClick: this.handleClick.bind(this),
                children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                  name: Internal.LibraryComponents.SvgIcon.Names[
                    this.props.isFavorite ? "FAVORITE_FILLED" : "FAVORITE"
                  ],
        ...
        CustomComponents.FileButton =
          reactInitialized &&
          class BDFDB_FileButton extends Internal.LibraryModules.React.Component {
            render() {
              let filter =
                (this.props.filter &&
                  [this.props.filter].flat(10).filter((n) => typeof n == "string")) ||
                [];
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.Button,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    onClick: (e) => {
                      (e.currentTarget || e.target).querySelector("input").click();
                    },
                    children: [
                      ...
                      BDFDB.ReactUtils.createElement("input", {
                        ...
                        onChange: (e) => {
                          ...
                          if (
                            this.refInput &&
                            file &&
                            (!filter.length || filter.some((n) => file.type.indexOf(n) == 0))
                          ) {
                            let reader = new FileReader();
                            reader.onload = (_) => {
                              if (
                                file.size >
                                (!isNaN(this.props.maxSize) && this.props.maxSize > 0
                                  ? this.props.maxSize
                                  : 5.1 * 1024 * 1024)
                              )
                                BDFDB.NotificationUtils.toast(
                                  "File too large to upload (Max 5MB)",
                                ...
                                this.refInput.handleChange(
                                  file.name,
                                  `${this.props.mode == "url" ? "url('" : ""}${`data:${file.type};base64,${btoa(Uint8ToString(new Uint8Array(reader.result)))}`}${this.props.mode ? "')" : ""}`,
                                );
                              }
                            };
        ...
        CustomComponents.FormItem =
          reactInitialized &&
          class BDFDB_FormItem extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement("div", {
                className: this.props.className,
                style: this.props.style,
                children: [
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                    children: [
                      this.props.title != null || this.props.error != null
                      ...
                      .concat([this.props.titleChildren].flat(10))
                      .filter((n) => n),
                  }),
        ...
        CustomComponents.FormTitle =
          reactInitialized &&
          class BDFDB_FormTitle extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                Internal.NativeSubComponents.FormTitle,
                this.props,
              );
            }
          };
        ...
        CustomComponents.GuildSummaryItem =
          reactInitialized &&
          class BDFDB_GuildSummaryItem extends Internal.LibraryModules.React.Component {
            defaultRenderGuild(guild, isLast) {
              ...
              return this.props.switchOnClick
                ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                    className: BDFDB.disCN.guildsummaryclickableicon,
                    onClick: (_) =>
                      Internal.LibraryModules.HistoryUtils.transitionTo(
                        Internal.DiscordConstants.Routes.CHANNEL(
                          guild.id,
                          Internal.LibraryStores.SelectedChannelStore.getChannelId(guild.id),
                        ),
                      ),
                    key: guild.id,
                    tabIndex: -1,
            ...
            renderGuilds() {
              let elements = [];
              let renderGuild =
                typeof this.props.renderGuild != "function"
                  ? this.defaultRenderGuild
                  : this.props.renderGuild;
              let loaded = 0,
                max =
                  this.props.guilds.length === this.props.max
              ...
              while (loaded < max && loaded < this.props.guilds.length) {
                ...
                elements.push(
                  BDFDB.ReactUtils.createElement("div", {
                    className: isLast
                      ? BDFDB.disCN.guildsummaryiconcontainer
                      : BDFDB.disCN.guildsummaryiconcontainermasked,
                    children: guild,
                  }),
                );
                loaded++;
              ...
              if (loaded < this.props.guilds.length) {
                ...
                elements.push(
                  BDFDB.ReactUtils.createElement(Internal.LibraryModules.React.Fragment, {
                    key: "more-guilds",
                    children: this.props.renderMoreGuilds(
                      "+" + rest,
                      rest,
                      this.props.guilds.slice(loaded),
                      this.props,
                    ),
                  }),
                );
            ...
            render() {
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.guildsummarycontainer,
                ),
                ref: this.props._ref,
                children: [this.renderIcon.apply(this), this.renderGuilds.apply(this)]
                  .flat(10)
                  .filter((n) => n),
              });
            }
          };
        Internal.setDefaultProps(CustomComponents.GuildSummaryItem, {
          max: 10,
          renderMoreGuilds: (count, amount, restGuilds, props) => {
            let icon = BDFDB.ReactUtils.createElement("div", {
              className: BDFDB.disCN.guildsummarymoreguilds,
              children: count,
            });
        ...
        CustomComponents.GuildVoiceList =
          reactInitialized &&
          class BDFDB_GuildVoiceList extends Internal.LibraryModules.React.Component {
            render() {
              let channels = Internal.LibraryStores.GuildChannelStore.getChannels(
                this.props.guild.id,
              );
              let voiceChannels = (channels.VOCAL || [])
                .filter((c) => c.channel.type == Internal.DiscordConstants.ChannelTypes.GUILD_VOICE)
                .map((c) => c.channel.id);
              let stageChannels = (channels.VOCAL || [])
                .filter(
                  (c) =>
                    c.channel.type == Internal.DiscordConstants.ChannelTypes.GUILD_STAGE_VOICE &&
                    Internal.LibraryStores.StageInstanceStore.getStageInstanceByChannel(
                      c.channel.id,
                    ),
                )
                .map((c) => c.channel.id);
              let streamOwnerIds =
                Internal.LibraryStores.ApplicationStreamingStore.getAllApplicationStreams()
                  .filter((app) => app.guildId === this.props.guild.id)
                  .map((app) => app.ownerId) || [];
              let streamOwners = streamOwnerIds
                .map((ownerId) => Internal.LibraryStores.UserStore.getUser(ownerId))
                .filter((n) => n);
              let voiceStates = BDFDB.ObjectUtils.toArray(
                Internal.LibraryStores.SortedVoiceStateStore.getVoiceStates(this.props.guild.id),
              ).flat(10);
              let connectedVoiceUsers = voiceStates
                .map(
                  (n) =>
                    voiceChannels.includes(n.voiceState.channelId) &&
                    n.voiceState.channelId != this.props.guild.afkChannelId &&
                    !streamOwnerIds.includes(n.voiceState.userId) &&
                    Internal.LibraryStores.UserStore.getUser(n.voiceState.userId),
                )
                .filter((n) => n);
              let connectedStageUsers = voiceStates
                .map(
                  (n) =>
                    stageChannels.includes(n.voiceState.channelId) &&
                    n.voiceState.channelId != this.props.guild.afkChannelId &&
                    !streamOwnerIds.includes(n.voiceState.userId) &&
                    Internal.LibraryStores.UserStore.getUser(n.voiceState.userId),
                )
        ...
        CustomComponents.KeybindRecorder =
          reactInitialized &&
          class BDFDB_KeybindRecorder extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange(arrays) {
                    this.props.value = arrays.map(
                      (platformKey) =>
                        Internal.LibraryModules.KeyEvents.codes[
                          Internal.LibraryModules.KeyCodeUtils.codeToKey(platformKey)
                        ] || platformKey[1],
                    );
                    if (typeof this.props.onChange == "function")
                      this.props.onChange(this.props.value, this);
                  ...
                  componentDidMount() {
                    if (!this.recorder)
                      this.recorder = BDFDB.ReactUtils.findOwner(this, { name: "KeybindRecorder" });
                  }
                  render() {
                    return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                      ...
                      children: [
                        BDFDB.ReactUtils.createElement(
                          Internal.NativeSubComponents.KeybindRecorder,
                          BDFDB.ObjectUtils.exclude(
                            Object.assign({}, this.props, {
                              defaultValue: [this.props.defaultValue || this.props.value]
                                ...
                                .filter((n) => n)
                                .map((keyCode) => [
                                  Internal.DiscordConstants.KeyboardDeviceTypes.KEYBOARD_KEY,
                                  Internal.LibraryModules.KeyCodeUtils.keyToCode(
                                    (Object.entries(Internal.LibraryModules.KeyEvents.codes).find(
                                      (n) =>
                                        n[1] == keyCode &&
                        ...
                        this.props.reset || this.props.onReset
                          ? BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.TooltipContainer,
                              {
        ...
        CustomComponents.ListRow =
          reactInitialized &&
          class BDFDB_ListRow extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                "div",
                BDFDB.ObjectUtils.exclude(
                      ...
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: [
                          ...
                          typeof this.props.note == "string"
                            ? BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.FormText.Text,
                                {
        ...
        CustomComponents.MemberRole =
          reactInitialized &&
          class BDFDB_MemberRole extends Internal.LibraryModules.React.Component {
            handleClick(e) {
              if (typeof this.props.onClick == "function") this.props.onClick(e, this);
            }
            handleContextMenu(e) {
            ...
            render() {
              let color =
                BDFDB.ColorUtils.convert(this.props.role.colorString, "RGB") ||
                Internal.DiscordConstants.Colors.PRIMARY_300;
              return BDFDB.ReactUtils.createElement("li", {
                className: BDFDB.DOMUtils.formatClassName(
                  BDFDB.disCN.userrole,
                  this.props.className,
                ),
        ...
        CustomComponents.MenuItems.MenuCheckboxItem =
          reactInitialized &&
          class BDFDB_MenuCheckboxItem extends Internal.LibraryModules.React.Component {
            handleClick() {
              if (this.props.state) {
                this.props.state.checked = !this.props.state.checked;
                if (typeof this.props.action == "function")
                  this.props.action(this.props.state.checked, this);
              }
              BDFDB.ReactUtils.forceUpdate(this);
            }
            render() {
              return BDFDB.ReactUtils.createElement(
                ...
                Object.assign({}, this.props, {
                  input:
                    this.props.state && this.props.state.checked
                      ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                          className: BDFDB.disCN.menuicon,
        ...
        CustomComponents.MenuItems.MenuHint =
          reactInitialized &&
          class BDFDB_MenuHint extends Internal.LibraryModules.React.Component {
            render() {
        ...
        CustomComponents.MenuItems.MenuIcon =
          reactInitialized &&
          class BDFDB_MenuIcon extends Internal.LibraryModules.React.Component {
            render() {
        ...
        CustomComponents.MenuItems.MenuControlItem = function (props) {
          ...
          BDFDB.ReactUtils.useLayoutEffect(
            (_) => {
              if (props.isFocused) {
                BDFDB.LibraryStores.AccessibilityStore.keyboardModeEnabled &&
                  controlRef.current &&
                ...
                controlRef.current &&
                  ...
                  controlRef.current.blur(controlRef.current);
            },
            [props.isFocused],
          );
            ...
            Object.assign(
              {
                className: BDFDB.DOMUtils.formatClassName(
                  BDFDB.disCN.menuitem,
                  BDFDB.disCN[
                    `menucolor${((props.color && InternalData.DiscordClasses[`menucolor${props.color.toLowerCase()}`]) || Internal.DiscordConstants.MenuItemColors.DEFAULT || "").toLowerCase()}`
                  ],
                  props.disabled && BDFDB.disCN.menudisabled,
                  props.showDefaultFocus && props.isFocused && BDFDB.disCN.menufocused,
                  !props.showDefaultFocus && BDFDB.disCN.menuhideinteraction,
                ),
                onClick: BDFDB.ReactUtils.useCallback(
                  (_) => {
                    if (
                      !controlRef.current ||
                      !controlRef.current.activate ||
                      !controlRef.current.activate.call(controlRef.current)
                    )
        ...
        CustomComponents.MenuItems.MenuSliderItem =
          reactInitialized &&
          class BDFDB_MenuSliderItem extends Internal.LibraryModules.React.Component {
                ...
                this.props.state.value =
                  Math.round(
                    BDFDB.NumberUtils.mapRange(
                      ...
                      value,
                    ) * Math.pow(10, this.props.digits),
                  ) / Math.pow(10, this.props.digits);
                if (typeof this.props.onValueChange == "function")
                  this.props.onValueChange(this.props.state.value, this);
              }
              BDFDB.ReactUtils.forceUpdate(this);
            }
            ...
            render() {
              ...
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.MenuItems.MenuControlItem,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    label:
                      typeof this.props.renderLabel == "function"
                        ? this.props.renderLabel(
                            Math.round(value * Math.pow(10, this.props.digits)) /
                              Math.pow(10, this.props.digits),
                            this,
                          )
                        : this.props.label,
                    control: (menuItemProps, ref) => {
                      return BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: BDFDB.ReactUtils.createElement(
                          Internal.NativeSubComponents.Slider,
        ...
        Internal.setDefaultProps(CustomComponents.MenuItems.MenuSliderItem, {
          minValue: 0,
          maxValue: 100,
          digits: 0,
        });
        ...
        CustomComponents.ModalComponents.ModalContent =
          reactInitialized &&
          class BDFDB_ModalContent extends Internal.LibraryModules.React.Component {
            render() {
              return this.props.scroller
                ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Scrollers.Thin, {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.modalcontent,
                      this.props.className,
                    ),
                ...
                : BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                    className: BDFDB.DOMUtils.formatClassName(
                      this.props.content && BDFDB.disCN.modalcontent,
                      BDFDB.disCN.modalnoscroller,
                      this.props.className,
                    ),
                    direction:
                      this.props.direction || Internal.LibraryComponents.Flex.Direction.VERTICAL,
                    align: Internal.LibraryComponents.Flex.Align.STRETCH,
                    children: this.props.children,
                  });
            }
          };
        Internal.setDefaultProps(CustomComponents.ModalComponents.ModalContent, {
          scroller: true,
          content: true,
        });

        CustomComponents.ModalComponents.ModalTabContent =
          reactInitialized &&
          class BDFDB_ModalTabContent extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
            ...
            render() {
              return !this.state.open
                ? null
                : BDFDB.ReactUtils.createElement(
                    this.props.scroller ? Internal.LibraryComponents.Scrollers.Thin : "div",
                    Object.assign(BDFDB.ObjectUtils.exclude(this.props, "scroller", "open"), {
                      className: BDFDB.DOMUtils.formatClassName(
                        BDFDB.disCN.modaltabcontent,
                        this.state.open && BDFDB.disCN.modaltabcontentopen,
                        this.props.className,
                      ),
                      children: this.props.children,
                    }),
        ...
        Internal.setDefaultProps(CustomComponents.ModalComponents.ModalTabContent, {
          tab: "unnamed",
        });

        CustomComponents.ModalComponents.ModalFooter =
          reactInitialized &&
          class BDFDB_ModalFooter extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                className: BDFDB.DOMUtils.formatClassName(
                ...
                direction:
                  ...
                  Internal.LibraryComponents.Flex.Direction.HORIZONTAL_REVERSE,
                align: Internal.LibraryComponents.Flex.Align.STRETCH,
                grow: 0,
                shrink: 0,
                children: this.props.children,
        ...
        CustomComponents.MultiInput =
          reactInitialized &&
          class BDFDB_MultiInput extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
            ...
            render() {
              if (this.props.children && this.props.children.props)
                this.props.children.props.className = BDFDB.DOMUtils.formatClassName(
                  this.props.children.props.className,
                  BDFDB.disCN.inputmultifield,
                );
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.inputwrapper,
                  BDFDB.disCN.inputmultiwrapper,
                ),
                children: BDFDB.ReactUtils.createElement("div", {
                  className: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.input,
                    BDFDB.disCN.inputmulti,
                    this.state.focused && BDFDB.disCN.inputfocused,
                  ),
                  children: [
                    BDFDB.ReactUtils.createElement("div", {
                      className: BDFDB.DOMUtils.formatClassName(
        ...
        CustomComponents.ListInput =
          reactInitialized &&
          class BDFDB_ListInput extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange() {
                  ...
                  render() {
                    ...
                    return BDFDB.ReactUtils.createElement(
                      Internal.LibraryComponents.MultiInput,
                      BDFDB.ObjectUtils.exclude(
                        Object.assign({}, this.props, {
                          ...
                          children: this.props.items.map((item) =>
                            BDFDB.ReactUtils.createElement(
                              Internal.LibraryComponents.Badges.TextBadge,
                              {
        ...
        CustomComponents.PaginatedList =
          reactInitialized &&
          class BDFDB_PaginatedList extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
            ...
            handleJump(offset) {
              if (
                offset > -1 &&
                offset < Math.ceil(this.props.items.length / this.props.amount) &&
            ...
            renderPagination(bottom) {
              ...
              return (
                this.props.items.length > this.props.amount &&
                BDFDB.ReactUtils.createElement("nav", {
                  className: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.pagination,
                    bottom ? BDFDB.disCN.paginationbottom : BDFDB.disCN.paginationtop,
                    this.props.mini && BDFDB.disCN.paginationmini,
                  ),
                  children: [
                    BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Paginator, {
                      ...
                      onPageChange: (page) => {
                        this.handleJump(isNaN(parseInt(page)) ? -1 : page - 1);
                      },
                    }),
                    this.props.jump &&
                      BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TextInput, {
                        ...
                        onKeyDown: (event, instance) => {
                          if (event.which == 13)
                            this.handleJump(
                              isNaN(parseInt(instance.props.value)) ? -1 : instance.props.value - 1,
                            );
                        },
                      }),
                  ].filter((n) => n),
                ...
                if (!this.props.alphabetKey) items = this.props.items;
                  ...
                  for (let key of [
                    ...
                    for (let item of unsortedItems)
                      if (
                        item &&
                        item[this.props.alphabetKey] &&
                        ((numbers && !isNaN(parseInt(item[this.props.alphabetKey][0]))) ||
                          item[this.props.alphabetKey].toUpperCase().indexOf(key) == 0)
                      )
                        alphaItems.push(item);
                    for (let sortedItem of alphaItems)
                      BDFDB.ArrayUtils.remove(unsortedItems, sortedItem);
                    alphabet[key] = {
                      items: BDFDB.ArrayUtils.keySort(alphaItems, this.props.alphabetKey),
                      disabled: !alphaItems.length,
                    };
                  }
                  alphabet["?!"] = {
                    items: BDFDB.ArrayUtils.keySort(unsortedItems, this.props.alphabetKey),
                    disabled: !unsortedItems.length,
                  };
                  for (let key in alphabet) items.push(alphabet[key].items);
                  items = items.flat(10);
                }
              }
              return typeof this.props.renderItem != "function" || !items.length
                ? null
                : BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Scrollers.Thin, {
                    className: BDFDB.DOMUtils.formatClassName(
                      this.props.className,
                      BDFDB.disCN.paginationlist,
                      this.props.mini && BDFDB.disCN.paginationlistmini,
                    ),
                    fade: this.props.fade,
                    children: [
                      this.renderPagination(),
                      items.length > this.props.amount &&
                        this.props.alphabetKey &&
                        BDFDB.ReactUtils.createElement("nav", {
                          className: BDFDB.disCN.paginationlistalphabet,
                          children: Object.keys(alphabet).map((key) =>
                            BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                              className: BDFDB.DOMUtils.formatClassName(
                                BDFDB.disCN.paginationlistalphabetchar,
                                alphabet[key].disabled &&
                                  BDFDB.disCN.paginationlistalphabetchardisabled,
                              ),
                              onClick: (_) => {
                                if (!alphabet[key].disabled)
                                  this.handleJump(
                                    Math.floor(
                                      items.indexOf(alphabet[key].items[0]) / this.props.amount,
                                    ),
                                  );
                              },
                              children: key,
                      ...
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: items
                          .slice(
                            this.state.offset * this.props.amount,
                            (this.state.offset + 1) * this.props.amount,
                          )
                          .map((data, i) => {
                            return this.props.renderItem(data, i);
                          })
                          .flat(10)
                          .filter((n) => n),
                      }),
        ...
        Internal.setDefaultProps(CustomComponents.PaginatedList, {
          amount: 50,
          offset: 0,
          mini: true,
          jump: true,
        ...
        CustomComponents.Popout =
          reactInitialized &&
          class BDFDB_Popout extends Internal.LibraryModules.React.Component {
            componentDidMount() {
              this.props.containerInstance.popout = this;
              if (typeof this.props.onOpen == "function")
                this.props.onOpen(this.props.containerInstance, this);
            }
            componentWillUnmount() {
              delete this.props.containerInstance.popout;
              if (typeof this.props.onClose == "function")
                this.props.onClose(this.props.containerInstance, this);
            }
            render() {
              if (!this.props.wrap) return this.props.children;
              let pos =
                typeof this.props.position == "string" ? this.props.position.toLowerCase() : null;
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.PopoutFocusLock, {
                className: BDFDB.DOMUtils.formatClassName(
                  BDFDB.disCN.popoutwrapper,
                  this.props.className,
                  this.props.themed && BDFDB.disCN.popoutthemedpopout,
                  this.props.arrow && BDFDB.disCN.popoutarrow,
                  this.props.arrow &&
                    (pos == "top" ? BDFDB.disCN.popoutarrowtop : BDFDB.disCN.popoutarrowbottom),
                ),
                id: this.props.id,
                onClick: (e) => e.stopPropagation(),
                style: BDFDB.ObjectUtils.extract(
                  this.props,
                  "padding",
                  "height",
                  "maxHeight",
        ...
        CustomComponents.PopoutContainer =
          reactInitialized &&
          class BDFDB_PopoutContainer extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
              this.state = { open: props.open };
            }
            componentDidMount() {
              ...
              this.domElementRef.current =
                BDFDB.ReactUtils.findDOMNode(this, true) ||
                BDFDB.ReactUtils.findValue(this[BDFDB.ReactUtils.instanceKey], "ref", {
                  notNull: true,
                });
            }
            onDocumentClicked() {
            ...
            render() {
              if (!this.props._rendered) {
                ...
                const child =
                  (BDFDB.ArrayUtils.is(this.props.children)
                    ? this.props.children[0]
                    : this.props.children) ||
                  BDFDB.ReactUtils.createElement("div", {
                    style: { height: "100%", width: "100%" },
                  });
                child.props.className = BDFDB.DOMUtils.formatClassName(
                ...
                child.props.onClick = (e, childThis) => {
                  ...
                  if (this.props.killEvent || childProps.killEvent)
                    BDFDB.ListenerUtils.stopEvent(e);
                };
                child.props.onContextMenu = (e, childThis) => {
                  ...
                  if (typeof childProps.onContextMenu == "function")
                    childProps.onContextMenu(e, childThis);
                  if (this.props.killEvent || childProps.killEvent)
                    BDFDB.ListenerUtils.stopEvent(e);
                };
              ...
              return BDFDB.ReactUtils.createElement(Internal.LibraryModules.React.Fragment, {
                children: [
                  ...
                  this.state.open &&
                    BDFDB.ReactUtils.createElement(
                      Internal.LibraryComponents.AppReferencePositionLayer,
                      {
                        onMount: (_) =>
                          BDFDB.TimeUtils.timeout((_) =>
                        ...
                        children: (_) => {
                          const popout = BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.Popout,
                            BDFDB.ObjectUtils.exclude(
                              Object.assign({}, this.props, {
                                ...
                                onClose:
                                  typeof this.props.onClose == "function"
                                    ...
                                    : (_) => {},
                                children:
                                  typeof this.props.renderPopout == "function"
                                    ? this.props.renderPopout(this)
                                    : null,
                              }),
                              "popoutStyle",
                              "popoutClassName",
                          ...
                          const animation = Object.entries(
                            Internal.LibraryComponents.PopoutContainer.Animation,
                          ).find((n) => n[1] == this.props.animation);
                          return !animation ||
                            animation[0] ==
                              Internal.LibraryComponents.PopoutContainer.Animation.NONE
                            ? popout
                            : BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.PopoutCSSAnimator,
                                {
                                  position: this.props.position,
                                  type: Internal.LibraryComponents.PopoutCSSAnimator.Types[
                                    animation[0]
                                  ],
                                  children: popout,
                                },
        ...
        CustomComponents.PopoutCSSAnimator = function (props) {
          ...
          let animationState = BDFDB.ReactUtils.useState(
            (_) => new Internal.LibraryComponents.Timeout(),
          );
          BDFDB.ReactUtils.useEffect((_) => (_) => animationState[0].stop(), [animationState[0]]);
          BDFDB.ReactUtils.useEffect(
            (_) => props.position && animationState[0].start(10, (_) => positionState[1](true)),
            [props.position, animationState[0]],
          );
          const position =
            typeof props.position == "string" && props.position.replace("window_", "");
          const animation = (Object.entries(
            Internal.LibraryComponents.PopoutContainer.Animation,
          ).find((n) => n[1] == props.animation) || ["NONE"])[0].toLowerCase();
          return BDFDB.ReactUtils.createElement("div", {
            className: BDFDB.DOMUtils.formatClassName(
              InternalData.DiscordClasses[`animationcontainer${position}`] &&
                BDFDB.disCN[`animationcontainer${position}`],
              InternalData.DiscordClasses[`animationcontainer${animation}`] &&
                BDFDB.disCN[`animationcontainer${animation}`],
              positionState[0] && BDFDB.disCN.animationcontainerrender,
            ),
            children: props.children,
          });
        };
        if (CustomComponents.PopoutCSSAnimator)
        ...
        CustomComponents.QuickSelect =
          reactInitialized &&
          class BDFDB_QuickSelect extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange(option) {
                    ...
                    if (typeof this.props.onChange == "function")
                      this.props.onChange(option.value || option.key, this);
                    BDFDB.ReactUtils.forceUpdate(this);
                  }
                  render() {
                    let options = (
                      BDFDB.ArrayUtils.is(this.props.options) ? this.props.options : [{}]
                    ).filter((n) => n);
                    let selectedOption = BDFDB.ObjectUtils.is(this.props.value)
                      ? this.props.value
                      : options[0] || {};
                    return BDFDB.ReactUtils.createElement("div", {
                      className: BDFDB.DOMUtils.formatClassName(
                        this.props.className,
                        BDFDB.disCN.quickselectwrapper,
                      ),
                      children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                        ...
                        children: [
                          ...
                          BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                            ...
                            onClick: (event) => {
                              Internal.LibraryModules.ContextMenuUtils.openContextMenu(event, (_) =>
                                BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Menu, {
                                  navId: "bdfdb-quickselect",
                                  onClose:
                                    Internal.LibraryModules.ContextMenuUtils.closeContextMenu,
                                  className: this.props.popoutClassName,
                                  children: BDFDB.ContextMenuUtils.createItem(
                                    Internal.LibraryComponents.MenuItems.MenuGroup,
                                    {
                                      children: options.map((option, i) => {
                                        let selected =
                                          (option.value && option.value === selectedOption.value) ||
                                          (option.key && option.key === selectedOption.key);
                                        return BDFDB.ContextMenuUtils.createItem(
                                          Internal.LibraryComponents.MenuItems.MenuItem,
                                          {
                                            label: option.label,
                                            ...
                                            action: selected
                                              ? null
                                              : (event2) => this.handleChange.bind(this)(option),
                                          },
                                        );
                                      }),
                                    },
        ...
        CustomComponents.RadioGroup =
          reactInitialized &&
          class BDFDB_RadioGroup extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange(value) {
                  ...
                  render() {
                    return BDFDB.ReactUtils.createElement(
                      Internal.NativeSubComponents.RadioGroup,
                      Object.assign({}, this.props, { onChange: this.handleChange.bind(this) }),
                    );
                  }
                },
                this.props,
        ...
        CustomComponents.Scrollers = new Proxy(
          {},
          {
            get: function (_, item) {
              if (item == "AUTO")
                return Internal.LibraryComponents.ScrollerBase(
                  BDFDB.disCN.scrollerauto,
                  BDFDB.disCN.scrollerfade,
                  BDFDB.disCN.scrollercustomtheme,
                );
              else if (item == "Thin")
                return Internal.LibraryComponents.ScrollerBase(
                  BDFDB.disCN.scrollerthin,
                  BDFDB.disCN.scrollerfade,
                  BDFDB.disCN.scrollercustomtheme,
                );
              else if (item == "None")
                return Internal.LibraryComponents.ScrollerBase(
                  BDFDB.disCN.scrollernone,
                  BDFDB.disCN.scrollerfade,
                  BDFDB.disCN.scrollercustomtheme,
                );
              else return "div";
        ...
        CustomComponents.SearchBar =
          reactInitialized &&
          class BDFDB_SearchBar extends Internal.LibraryModules.React.Component {
            handleChange(query) {
            ...
            handleClear() {
              this.props.query = "";
              if (this.props.changeOnClear && typeof this.props.onChange == "function")
        ...
        CustomComponents.Select =
          reactInitialized &&
          class BDFDB_Select extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange(value) {
                  ...
                  render() {
                    return BDFDB.ReactUtils.createElement("div", {
                      className: BDFDB.DOMUtils.formatClassName(
                        this.props.className,
                        BDFDB.disCN.selectwrapper,
                      ),
                      children: BDFDB.ReactUtils.createElement(
                        Internal.NativeSubComponents.SearchableSelect,
                        BDFDB.ObjectUtils.exclude(
                          Object.assign({}, this.props, {
        ...
        CustomComponents.SettingsGuildList =
          reactInitialized &&
          class BDFDB_SettingsGuildList extends Internal.LibraryModules.React.Component {
            render() {
              this.props.disabled = BDFDB.ArrayUtils.is(this.props.disabled)
                ? this.props.disabled
                : [];
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                className: this.props.className,
                ...
                children: [
                  this.props.includeDMs && {
                    name: BDFDB.LanguageUtils.LanguageStrings.DIRECT_MESSAGES,
                    acronym: "DMs",
                    id: Internal.DiscordConstants.ME,
                    getIconURL: (_) => {},
                  },
                ]
                  .concat(
                    Internal.LibraryStores.SortedGuildStore.getFlattenedGuildIds().map(
                      Internal.LibraryStores.GuildStore.getGuild,
                    ),
                  )
                  .filter((n) => n)
                  .map((guild) =>
                    BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TooltipContainer, {
                      ...
                      children: BDFDB.ReactUtils.createElement("div", {
                        className: BDFDB.DOMUtils.formatClassName(
                          this.props.guildClassName,
                          BDFDB.disCN.settingsguild,
                          this.props.disabled.includes(guild.id) &&
                            BDFDB.disCN.settingsguilddisabled,
                        ),
                        children: BDFDB.ReactUtils.createElement(
                          Internal.LibraryComponents.GuildIcon,
                          {
                            guild: guild,
                            size:
                              this.props.size || Internal.LibraryComponents.GuildIcon.Sizes.MEDIUM,
                          },
                        ),
                        onClick: (e) => {
        ...
        CustomComponents.SettingsPanel =
          reactInitialized &&
          class BDFDB_SettingsPanel extends Internal.LibraryModules.React.Component {
            componentDidMount() {
            ...
            componentWillUnmount() {
              if (
                BDFDB.ObjectUtils.is(this.props.addon) &&
                typeof this.props.addon.onSettingsClosed == "function"
              )
                this.props.addon.onSettingsClosed();
            }
            render() {
              let panelItems = [
              ...
              return BDFDB.ReactUtils.createElement("div", {
                key:
                  ...
                  `${this.props.addon.name}-settingsPanel`,
                id:
                  this.props.addon && this.props.addon.name && `${this.props.addon.name}-settings`,
                className: BDFDB.disCN.settingspanel,
                children: [
                  this.props.addon.changeLog &&
                    !BDFDB.ObjectUtils.isEmpty(this.props.addon.changeLog) &&
                    BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TooltipContainer, {
        ...
        CustomComponents.SettingsPanelList =
          reactInitialized &&
          class BDFDB_SettingsPanelInner extends Internal.LibraryModules.React.Component {
            render() {
              return this.props.children
                ? BDFDB.ReactUtils.createElement("div", {
                    className: BDFDB.DOMUtils.formatClassName(
                      this.props.className,
                      BDFDB.disCN.settingspanellistwrapper,
                      this.props.mini && BDFDB.disCN.settingspanellistwrappermini,
                    ),
                    children: [
                      this.props.dividerTop
                        ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.FormDivider, {
                            className: this.props.mini
                              ? BDFDB.disCN.marginbottom4
                      ...
                      typeof this.props.title == "string"
                        ? BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.FormTitle.Title,
                            {
                              ...
                              tag:
                                Internal.LibraryComponents.FormTitle.Tags &&
                                Internal.LibraryComponents.FormTitle.Tags.H3,
                              children: this.props.title,
                            },
                          )
                        : null,
                      ...
                      this.props.dividerBottom
                        ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.FormDivider, {
                            className: this.props.mini
                              ? BDFDB.disCN.margintop4
                              : BDFDB.disCN.margintop8,
                          })
                        : null,
                    ],
                  })
        ...
        CustomComponents.SettingsItem =
          reactInitialized &&
          class BDFDB_SettingsItem extends Internal.LibraryModules.React.Component {
            constructor(props) {
              super(props);
              if (!this.state) this.state = {};
            ...
            render() {
              if (
                ...
                !["BUTTON", "SELECT", "SLIDER", "SWITCH", "TEXTINPUT"].includes(
              ...
              if (this.props.mini && childComponent.Sizes)
                this.props.size = childComponent.Sizes.MINI || childComponent.Sizes.MIN;
              let label = this.props.label
                ? this.props.tag
                  ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.FormTitle.Title, {
                      className: BDFDB.DOMUtils.formatClassName(
                        this.props.labelClassName,
                        BDFDB.disCN.marginreset,
                      ),
                      tag: this.props.tag,
                      children: this.props.label,
                    })
              ...
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.settingsrowcontainer,
                  margin != null &&
                    ((InternalData.DiscordClasses[`marginbottom${margin}`] &&
                      BDFDB.disCN[`marginbottom${margin}`]) ||
                      (margin == 0 && BDFDB.disCN.marginreset)),
                ),
                id: this.props.id,
                children: [
                  this.props.dividerTop
                  ...
                  BDFDB.ReactUtils.createElement("div", {
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.settingsrow,
                      this.state.disabled && BDFDB.disCN.settingsrowdisabled,
                    ),
                    ...
                    children: [
                      BDFDB.ReactUtils.createElement("div", {
                        ...
                        children: [
                          label && !this.props.basis
                            ? BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.Flex.Child,
                                { grow: 1, shrink: 1, wrap: true, children: label },
                              )
                            : label,
                          typeof this.props.note == "string"
                            ? BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.Flex.Child,
                                {
                                  ...
                                  children: BDFDB.ReactUtils.createElement(
                                    Internal.LibraryComponents.TextElement,
                                    {
                                      ...
                                      children: BDFDB.ReactUtils.createElement(
                                        Internal.LibraryComponents.TextScroller,
                                        { speed: 2, children: this.props.note },
                                      ),
                                    },
                                  ),
                                },
                      ...
                      BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex.Child, {
                        ...
                        children: [
                          ...
                          BDFDB.ReactUtils.createElement(
                            ...
                            BDFDB.ObjectUtils.exclude(
                              Object.assign(
                                BDFDB.ObjectUtils.exclude(this.props, "className", "id", "type"),
                                this.props.childProps,
                                {
                                  disabled: this.state.disabled,
                                  onChange: this.handleChange.bind(this),
        ...
        CustomComponents.SettingsLabel =
          reactInitialized &&
          class BDFDB_SettingsLabel extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TextScroller, {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.settingsrowtitle,
                  this.props.mini && BDFDB.disCN.settingsrowtitlemini,
                  BDFDB.disCN.cursordefault,
                ),
                speed: 2,
                children: this.props.label,
              });
        ...
        CustomComponents.SettingsList =
          reactInitialized &&
          class BDFDB_SettingsList extends Internal.LibraryModules.React.Component {
            componentDidMount() {
              this.checkList();
            }
            componentDidUpdate() {
            ...
            checkList() {
              ...
              if (list && !this.props.configWidth) {
                ...
                headers.shift();
                if (BDFDB.DOMUtils.getRects(headers[0]).width == 0)
                  BDFDB.TimeUtils.timeout((_) => this.resizeList(headers));
            ...
            renderHeaderOption(props) {
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  props.className,
                  BDFDB.disCN.eyebrow,
                  props.clickable && BDFDB.disCN.cursorpointer,
                ),
                onClick: (_) => {
                  if (typeof this.props.onHeaderClick == "function")
                    this.props.onHeaderClick(props.label, this);
                },
                onContextMenu: (_) => {
                  if (typeof this.props.onHeaderContextMenu == "function")
                    this.props.onHeaderContextMenu(props.label, this);
                },
                children: BDFDB.ReactUtils.createElement("span", { children: props.label }),
              });
            }
            renderItem(props) {
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.Card,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    className: BDFDB.DOMUtils.formatClassName(
                      [this.props.cardClassName, props.className]
                        .filter((n) => n)
                        .join(" ")
                        .indexOf(BDFDB.disCN.card) == -1 && BDFDB.disCN.cardprimary,
                      BDFDB.disCN.settingstablecard,
                            ...
                            BDFDB.ReactUtils.createElement("div", {
                              ...
                              children: BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.TooltipContainer,
                                {
                                  ...
                                  children: BDFDB.ReactUtils.createElement(
                                    Internal.LibraryComponents.Checkbox,
                                    {
                                      disabled: props.disabled,
                                      cardId: props.key,
                                      settingId: setting,
                                      shape:
                                        Internal.LibraryComponents.Checkbox.Shapes &&
                                        Internal.LibraryComponents.Checkbox.Shapes.ROUND,
                                      type:
                                        Internal.LibraryComponents.Checkbox.Types &&
                                        Internal.LibraryComponents.Checkbox.Types.INVERTED,
            ...
            render() {
              this.props.settings = BDFDB.ArrayUtils.is(this.props.settings)
                ? this.props.settings
                : [];
              this.props.renderLabel =
                typeof this.props.renderLabel == "function"
                  ...
                  : (data) => data.label;
              this.props.data = (
                BDFDB.ArrayUtils.is(this.props.data) ? this.props.data : [{}]
              ).filter((n) => n);

              let wrapperWidth =
                this.props.configWidth && this.props.configWidth * this.props.settings.length;
              let isHeaderClickable =
                typeof this.props.onHeaderClick == "function" ||
                typeof this.props.onHeaderContextMenu == "function";
              ...
              let header = BDFDB.ReactUtils.createElement("div", {
                ...
                style:
                  this.props.vertical && this.props.biggestWidth
                    ? { marginTop: this.props.biggestWidth - 15 || 0 }
                    : {},
                children: [
                  this.renderHeaderOption({
                    className: BDFDB.DOMUtils.formatClassName(
                      BDFDB.disCN.settingstableheadername,
                      BDFDB.disCN.settingstableheader,
                    ),
                  ...
                  BDFDB.ReactUtils.createElement("div", {
                    ...
                    children: this.props.settings.map((setting) =>
                      this.renderHeaderOption({
                        className: BDFDB.DOMUtils.formatClassName(
                          BDFDB.disCN.settingstableheaderoption,
                          BDFDB.disCN.settingstableheader,
                          this.props.vertical && BDFDB.disCN.settingstableheadervertical,
                        ),
                    ...
                    children: [
                      ...
                      !usePagination
                        ? this.props.data.map((data) =>
                            this.renderItem(Object.assign({}, data, { wrapperWidth })),
                          )
                        : BDFDB.ReactUtils.createElement(
                            Internal.LibraryComponents.PaginatedList,
                            Object.assign({}, this.props.pagination, {
                              ...
                              onJump: (offset, instance) => {
                                ...
                                if (typeof this.props.pagination.onJump == "function")
                                  this.props.pagination.onJump(offset, this, instance);
                              },
                            }),
                          ),
                    ].filter((n) => n),
        ...
        CustomComponents.SettingsSaveItem =
          reactInitialized &&
          class BDFDB_SettingsSaveItem extends Internal.LibraryModules.React.Component {
            saveSettings(value) {
              if (!BDFDB.ArrayUtils.is(this.props.keys) || !BDFDB.ObjectUtils.is(this.props.plugin))
                return;
              ...
              if (BDFDB.ObjectUtils.is(this.props.plugin) && option) {
                ...
                newC = BDFDB.ObjectUtils.is(newC)
                  ? BDFDB.ObjectUtils.deepAssign({}, data, newC)
                  : newC;
            ...
            render() {
              if (
                typeof this.props.type != "string" ||
                !["SELECT", "SLIDER", "SWITCH", "TEXTINPUT"].includes(this.props.type.toUpperCase())
              )
                return null;
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.SettingsItem,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, { onChange: this.saveSettings.bind(this) }),
                  "keys",
                  "key",
                  "plugin",
                ),
        ...
        CustomComponents.SidebarList =
          reactInitialized &&
          class BDFDB_SidebarList extends Internal.LibraryModules.React.Component {
            ...
            render() {
              let items = (BDFDB.ArrayUtils.is(this.props.items) ? this.props.items : [{}]).filter(
                (n) => n,
              );
              ...
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  this.props.className,
                  BDFDB.disCN.sidebarlist,
                ),
                children: [
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Scrollers.Thin, {
                    className: BDFDB.DOMUtils.formatClassName(
                      this.props.sidebarClassName,
                      BDFDB.disCN.sidebar,
                    ),
                    fade: true,
                  ...
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Scrollers.Thin, {
                    className: BDFDB.DOMUtils.formatClassName(
                      ...
                      BDFDB.disCN.sidebarcontent,
                    ),
                    fade: true,
                    children: [selectedElements]
                      .flat(10)
                      .filter((n) => n)
                      .map((data) => renderElement(data)),
                  }),
                ],
              });
            }
        ...
        CustomComponents.Slider =
          reactInitialized &&
          class BDFDB_Slider extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleMarkerRender(marker) {
                    ...
                    if (typeof this.props.digits == "number")
                      newMarker =
                        Math.round(newMarker * Math.pow(10, this.props.digits)) /
                        Math.pow(10, this.props.digits);
                    return newMarker;
                  }
                  handleValueChange(value) {
                    ...
                    if (typeof this.props.digits == "number")
                      newValue =
                        Math.round(newValue * Math.pow(10, this.props.digits)) /
                        Math.pow(10, this.props.digits);
                    ...
                    if (typeof this.props.onValueChange == "function")
                      this.props.onValueChange(
                        BDFDB.NumberUtils.mapRange([0, 100], this.props.edges, newValue),
                        this,
                      );
                    BDFDB.ReactUtils.forceUpdate(this);
                  }
                  ...
                  render() {
                    ...
                    if (!BDFDB.ArrayUtils.is(this.props.edges) || this.props.edges.length != 2)
                      this.props.edges = [
                        this.props.min || this.props.minValue,
                        this.props.max || this.props.maxValue,
                      ];
                    let defaultValue = BDFDB.NumberUtils.mapRange(
                      [0, 100],
                      this.props.edges,
                      value,
                    );
                    if (typeof this.props.digits == "number")
                      defaultValue =
                        Math.round(defaultValue * Math.pow(10, this.props.digits)) /
                        Math.pow(10, this.props.digits);
                    return BDFDB.ReactUtils.createElement(
                      Internal.NativeSubComponents.Slider,
                      BDFDB.ObjectUtils.exclude(
                        Object.assign({}, this.props, {
                          ...
                          markers:
                            typeof this.props.markerAmount == "number"
                              ? Array.from(Array(this.props.markerAmount).keys()).map(
                                  (_, i) => (i * (this.props.maxValue - this.props.minValue)) / 10,
                                )
                              : undefined,
        ...
        CustomComponents.SvgIcon =
          reactInitialized &&
          class BDFDB_Icon extends Internal.LibraryModules.React.Component {
            render() {
              if (BDFDB.ObjectUtils.is(this.props.name)) {
                ...
                let props = Object.assign(
                  ...
                  this.props.name.defaultProps,
                  this.props,
                );
                for (let key in props)
                  this.props.iconSVG = this.props.iconSVG.replace(
                    new RegExp(`%%${key}`, "g"),
                    props[key],
                  );
              }
              if (this.props.iconSVG) {
                let icon = BDFDB.ReactUtils.elementToReact(
                  BDFDB.DOMUtils.create(this.props.iconSVG),
                );
                if (BDFDB.ReactUtils.isValidElement(icon)) {
                  icon.props.className = BDFDB.DOMUtils.formatClassName(
                    !this.props.nativeClass && BDFDB.disCN.svgicon,
                    icon.props.className,
                    this.props.className,
                  );
                  icon.props.style = Object.assign({}, icon.props.style, this.props.style);
                  icon.props = Object.assign(
                    {},
                    BDFDB.ObjectUtils.extract(
                      this.props,
                      "onClick",
        ...
        const SwitchInner = function (props) {
          let reducedMotion = BDFDB.ReactUtils.useContext(
            Internal.LibraryModules.PreferencesContext.AccessibilityPreferencesContext,
          ).reducedMotion;
          ...
          let animation = Internal.LibraryComponents.Animations.useSpring({
            ...
            state: state[0] ? (props.value ? 0.7 : 0.3) : props.value ? 1 : 0,
          });
          let fill = animation.state.to({ output: [props.uncheckedColor, props.checkedColor] });
          ...
          return BDFDB.ReactUtils.createElement(
            Internal.LibraryComponents.Animations.animated.div,
            {
              className: BDFDB.DOMUtils.formatClassName(
                props.className,
                BDFDB.disCN.switch,
                props.value && BDFDB.disCN.switchchecked,
                mini && BDFDB.disCN.switchmini,
              ),
              onMouseDown: (_) => {
              ...
              children: [
                BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Animations.animated.svg, {
                  ...
                  children: [
                    BDFDB.ReactUtils.createElement(
                      Internal.LibraryComponents.Animations.animated.rect,
                      {
                        fill: "white",
                        x: animation.state.to({ range: [0, 0.3, 0.7, 1], output: [4, 0, 0, 4] }),
                        y: animation.state.to({ range: [0, 0.3, 0.7, 1], output: [0, 1, 1, 0] }),
                    ...
                    BDFDB.ReactUtils.createElement("svg", {
                      ...
                      children: [
                        BDFDB.ReactUtils.createElement(
                          Internal.LibraryComponents.Animations.animated.path,
                          {
                            fill: fill,
                            d: animation.state.to({
                              range: [0, 0.3, 0.7, 1],
                              output: reducedMotion.enabled
                                ? [
                                    SwitchIconPaths.a.TOP,
                                    SwitchIconPaths.a.TOP,
                                    SwitchIconPaths.c.TOP,
                                    SwitchIconPaths.c.TOP,
                                  ]
                                : [
                                    SwitchIconPaths.a.TOP,
                                    SwitchIconPaths.b.TOP,
                                    SwitchIconPaths.b.TOP,
                                    SwitchIconPaths.c.TOP,
                                  ],
                            }),
                            ...
                            d: animation.state.to({
                              ...
                              output: reducedMotion.enabled
                                ? [
                                    ...
                                    SwitchIconPaths.a.BOTTOM,
                                    SwitchIconPaths.c.BOTTOM,
                                    SwitchIconPaths.c.BOTTOM,
                                  ]
                                : [
                                    SwitchIconPaths.a.BOTTOM,
                                    SwitchIconPaths.b.BOTTOM,
                                    SwitchIconPaths.b.BOTTOM,
                                    SwitchIconPaths.c.BOTTOM,
                                  ],
                            }),
                  ...
                  BDFDB.ObjectUtils.exclude(
                    Object.assign({}, props, {
                      ...
                      className: BDFDB.DOMUtils.formatClassName(
                        ...
                        BDFDB.disCN.switchinner,
                      ),
                      tabIndex: props.disabled ? -1 : 0,
                      onKeyDown: (e) => {
                        if (!props.disabled && !e.repeat && (e.key == " " || e.key == "Enter"))
                          state[1](true);
                      },
                      onKeyUp: (e) => {
                      ...
                      onChange: (e) => {
                        ...
                        if (typeof props.onChange == "function")
                          props.onChange((e.currentTarget || e.target).checked, e);
                      },
                      checked: props.value,
                      disabled: props.disabled,
                    }),
        ...
        CustomComponents.Switch =
          reactInitialized &&
          class BDFDB_Switch extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange() {
        ...
        if (CustomComponents.Switch) {
          CustomComponents.Switch.Sizes = { DEFAULT: "default", MINI: "mini" };
        ...
        CustomComponents.TabBar =
          reactInitialized &&
          class BDFDB_TabBar extends Internal.LibraryModules.React.Component {
            handleItemSelect(item) {
              this.props.selectedItem = item;
              if (typeof this.props.onItemSelect == "function") this.props.onItemSelect(item, this);
              BDFDB.ReactUtils.forceUpdate(this);
            ...
            render() {
              ...
              return BDFDB.ReactUtils.createElement(
                Internal.NativeSubComponents.TabBar,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    selectedItem: selectedItem,
                    onItemSelect: this.handleItemSelect.bind(this),
                    children: items.map((data) =>
                      BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TabBar.Item, {
                        className: BDFDB.DOMUtils.formatClassName(
                          this.props.itemClassName,
                          selectedItem == data.value && this.props.itemSelectedClassName,
                        ),
        ...
        if (CustomComponents.TabBar) {
          CustomComponents.TabBar.Types = { SIDE: "side", TOP: "top", TOP_PILL: "top-pill" };
          CustomComponents.TabBar.Looks = { GREY: "grey", BRAND: "brand", CUSTOM: "custom" };
        }

        CustomComponents.Table =
          reactInitialized &&
          class BDFDB_Table extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                Internal.NativeSubComponents.Table,
                Object.assign({}, this.props, {
                  className: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.table,
                    this.props.className,
                  ),
                  headerCellClassName: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.tableheadercell,
                    this.props.headerCellClassName,
                  ),
                  sortedHeaderCellClassName: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.tableheadercellsorted,
                    this.props.sortedHeaderCellClassName,
                  ),
                  bodyCellClassName: BDFDB.DOMUtils.formatClassName(
                    BDFDB.disCN.tablebodycell,
                    this.props.bodyCellClassName,
                  ),
                  onSort: (sortKey, sortDirection) => {
                    this.props.sortDirection =
                      this.props.sortKey != sortKey &&
                      sortDirection == Internal.LibraryComponents.Table.SortDirection.ASCENDING &&
                      this.props.columns.filter((n) => n.key == sortKey)[0].reverse
                        ? Internal.LibraryComponents.Table.SortDirection.DESCENDING
                        : sortDirection;
                    this.props.sortKey = sortKey;
                    ...
                    if (typeof this.props.onSort == "function")
                      this.props.onSort(this.props.sortKey, this.props.sortDirection);
                    BDFDB.ReactUtils.forceUpdate(this);
                  },
                }),
              );
        ...
        CustomComponents.TextArea =
          reactInitialized &&
          class BDFDB_TextArea extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                class extends Internal.LibraryModules.React.Component {
                  handleChange(e) {
                  ...
                  render() {
                    return BDFDB.ReactUtils.createElement(
                      Internal.NativeSubComponents.TextArea,
                      Object.assign({}, this.props, {
                        onChange: this.handleChange.bind(this),
        ...
        CustomComponents.TextElement =
          reactInitialized &&
          class BDFDB_TextElement extends Internal.LibraryModules.React.Component {
            render() {
              let color =
                this.props.color != undefined
                  ? this.props.color && Internal.DiscordConstants.ColorsCSS[this.props.color]
                  : Internal.DiscordConstants.ColorsCSS[
                      CustomComponents.TextElement.Colors.STANDARD
                    ];
        ...
        CustomComponents.TextGradientElement =
          reactInitialized &&
          class BDFDB_TextGradientElement extends Internal.LibraryModules.React.Component {
            render() {
              if (this.props.gradient && this.props.children)
                return BDFDB.ReactUtils.createElement("span", {
                  children: this.props.children,
                  ref: (instance) => {
                    ...
                    if (ele) {
                      ...
                      ele.style.setProperty(
                        ...
                        BDFDB.ColorUtils.convert(this.props.gradient[0], "RGBA"),
                        "important",
                      );
        ...
        CustomComponents.TextInput =
          reactInitialized &&
          class BDFDB_TextInputInner extends Internal.LibraryModules.React.Component {
            handleChange(e, e2) {
              let value = (e = BDFDB.ObjectUtils.is(e) ? (e.currentTarget || e.target).value : e);
              if (this.props.type == "number") value = parseInt(value);
              this.props.value =
                this.props.valuePrefix && !value.startsWith(this.props.valuePrefix)
                  ? this.props.valuePrefix + value
                  : value;
              this.props.file = e2 = BDFDB.ObjectUtils.is(e2)
                ? (e2.currentTarget || e2.target).value
                : e2;
              if (typeof this.props.onChange == "function")
                this.props.onChange(
                  this.props.type == "file" ? this.props.file : this.props.value,
                  this,
                );
              BDFDB.ReactUtils.forceUpdate(this);
            }
            handleInput(e) {
              if (typeof this.props.onInput == "function")
                this.props.onInput(
                  BDFDB.ObjectUtils.is(e) ? (e.currentTarget || e.target).value : e,
                  this,
                );
            }
            handleKeyDown(e) {
            ...
            componentDidMount() {
              if (this.props.type == "file") {
                let navigatorInstance = BDFDB.ReactUtils.findOwner(this, {
                  name: "BDFDB_FileButton",
                });
              ...
              let inputChildren = [
                  ...
                  BDFDB.ObjectUtils.exclude(
                    Object.assign({}, this.props, {
                      className: BDFDB.DOMUtils.formatClassName(
                        ...
                        this.props.focused && BDFDB.disCN.inputfocused,
                        this.props.error || this.props.errorMessage
                          ? BDFDB.disCN.inputerror
                          : this.props.success && BDFDB.disCN.inputsuccess,
                        this.props.disabled && BDFDB.disCN.inputdisabled,
                        this.props.editable && BDFDB.disCN.inputeditable,
                      ),
                      type:
                        this.props.type == "color" || this.props.type == "file"
                          ? "text"
                          : this.props.type,
                      onChange: this.handleChange.bind(this),
                ...
                this.props.type == "color"
                  ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex.Child, {
                      wrap: true,
                      children: BDFDB.ReactUtils.createElement(
                        Internal.LibraryComponents.ColorSwatches,
                        {
                          colors: [],
                          color:
                            this.props.value && this.props.mode == "comp"
                              ? BDFDB.ColorUtils.convert(this.props.value.split(","), "RGB")
                              : this.props.value,
                          onColorChange: (color) =>
                            this.handleChange.apply(this, [
                              !color
                                ? ""
                                : this.props.mode == "comp"
                                  ? BDFDB.ColorUtils.convert(color, "RGBCOMP").slice(0, 3).join(",")
                                  : BDFDB.ColorUtils.convert(
                                      color,
                                      this.props.noAlpha ? "RGB" : "RGBA",
                                    ),
                            ]),
                          ...
                          pickerConfig: {
                            gradient: false,
                            alpha: this.props.mode != "comp" && !this.props.noAlpha,
                          },
                        },
                      ),
                    })
                ...
                this.props.type == "file"
                  ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.FileButton, {
              ...
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  ...
                  this.props.type == "number" &&
                    ((this.props.size &&
                      Internal.LibraryComponents.TextInput.Sizes[this.props.size.toUpperCase()] &&
                      BDFDB.disCN["inputnumberwrapper" + this.props.size.toLowerCase()]) ||
                      BDFDB.disCN.inputnumberwrapperdefault),
                  this.props.className,
                ),
                style: this.props.style,
                children: [
                  this.props.type == "number"
                          ...
                          BDFDB.ReactUtils.createElement("div", {
                            ...
                            onClick: (e) => {
                              ...
                              if (isNaN(max) || (!isNaN(max) && newV <= max))
                                this.handleNumberButton.bind(this)(
                                  e._targetInst,
                                  isNaN(min) || (!isNaN(min) && newV >= min) ? newV : min,
                                );
                            },
                          }),
                          BDFDB.ReactUtils.createElement("div", {
                            ...
                            onClick: (e) => {
                              ...
                              if (isNaN(min) || (!isNaN(min) && newV >= min))
                                this.handleNumberButton.bind(this)(
                                  e._targetInst,
                                  isNaN(max) || (!isNaN(max) && newV <= max) ? newV : max,
                                );
                            },
                          }),
                        ],
                    ...
                    : null,
                  inputChildren.length == 1
                    ? inputChildren[0]
                    : BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Flex, {
                        wrap: Internal.LibraryComponents.Flex.Wrap.NO_WRAP,
                        align: Internal.LibraryComponents.Flex.Align.CENTER,
                        children: inputChildren.map((child, i) =>
                          i != 0
                            ? BDFDB.ReactUtils.createElement(
                                Internal.LibraryComponents.Flex.Child,
                                { shrink: 0, children: child },
                              )
                            : child,
                        ),
                      }),
        ...
        CustomComponents.TextInput.Sizes = { MINI: BDFDB.disCN.inputmini };

        CustomComponents.TextScroller =
          reactInitialized &&
          class BDFDB_TextScroller extends Internal.LibraryModules.React.Component {
            render() {
              let scrolling,
                scroll = (_) => {};
              return BDFDB.ReactUtils.createElement("div", {
                className: BDFDB.DOMUtils.formatClassName(
                  BDFDB.disCN.textscroller,
                  this.props.className,
                ),
                ...
                ref: (instance) => {
                  ...
                  if (ele && ele.parentElement) {
                    ...
                    Animation.interpolate({
                      ...
                      outputRange: [
                        0,
                        (BDFDB.DOMUtils.getRects(ele.firstElementChild).width -
                          BDFDB.DOMUtils.getRects(ele).width) *
                          -1,
                      ],
                    }).addListener((v) => {
                      ele.firstElementChild.style.setProperty(
                        "display",
                        v.value == 0 ? "inline" : "block",
                        "important",
                      );
                    ...
                    scroll = (p) => {
                      ...
                      let w =
                        p +
                        parseFloat(ele.firstElementChild.style.getPropertyValue("left")) /
                          (innerWidth - outerWidth);
                      w = isNaN(w) || !isFinite(w) ? p : w;
                      w *= innerWidth / (outerWidth * 2);
                      Internal.LibraryModules.AnimationUtils.parallel([
                        Internal.LibraryModules.AnimationUtils.timing(Animation, {
                          toValue: p,
                          duration: (Math.sqrt(w ** 2) * 4000) / (parseInt(this.props.speed) || 1),
                        }),
                      ]).start();
                    };
                  }
                ...
                onMouseEnter: (e) => {
                  ...
                  if (
                    BDFDB.DOMUtils.getRects(target).width <
                      BDFDB.DOMUtils.getRects(target.firstElementChild).width ||
                    target.firstElementChild.style.getPropertyValue("display") != "inline"
                  ) {
                    scrolling = true;
                    scroll(1);
                  }
        ...
        CustomComponents.TooltipContainer =
          reactInitialized &&
          class BDFDB_TooltipContainer extends Internal.LibraryModules.React.Component {
            updateTooltip(text) {
            ...
            render() {
              let child =
                (typeof this.props.children == "function"
                  ...
                  : BDFDB.ArrayUtils.is(this.props.children)
                    ? this.props.children[0]
                    : this.props.children) || BDFDB.ReactUtils.createElement("div", {});
              ...
              child.props.onMouseEnter = (e, childThis) => {
                ...
                if (
                  !shown &&
                  !target.__BDFDBtooltipShown &&
                  !(this.props.onlyShowOnShift && !e.shiftKey) &&
                  !(this.props.onlyShowOnCtrl && !e.ctrlKey)
                ) {
                  target.__BDFDBtooltipShown = shown = true;
                  this.tooltip = BDFDB.TooltipUtils.create(
                    target,
                    typeof this.props.text == "function"
                      ? this.props.text(this, e)
                      : this.props.text,
                    Object.assign(
                      { note: this.props.note, delay: this.props.delay },
                      this.props.tooltipConfig,
                      {
                        onHide: (tooltip, anker) => {
                          ...
                          if (
                            this.props.tooltipConfig &&
                            typeof this.props.tooltipConfig.onHide == "function"
                          )
                            this.props.tooltipConfig.onHide(tooltip, anker);
                        },
                      },
                    ),
                  );
                  ...
                  if (typeof childProps.onMouseEnter == "function")
                    childProps.onMouseEnter(e, childThis);
                }
              };
              child.props.onMouseLeave = (e, childThis) => {
                if (typeof this.props.onMouseLeave == "function") this.props.onMouseLeave(e, this);
                if (typeof childProps.onMouseLeave == "function")
                  childProps.onMouseLeave(e, childThis);
              };
              child.props.onClick = (e, childThis) => {
                if (typeof this.props.onClick == "function") this.props.onClick(e, this);
                if (typeof childProps.onClick == "function") childProps.onClick(e, childThis);
                if (typeof this.props.text == "function")
                  this.updateTooltip(this.props.text(this, e));
              };
              child.props.onContextMenu = (e, childThis) => {
                if (typeof this.props.onContextMenu == "function")
                  this.props.onContextMenu(e, this);
        ...
        if (CustomComponents.TooltipContainer)
        ...
        CustomComponents.UserPopoutContainer =
          reactInitialized &&
          class BDFDB_UserPopoutContainer extends Internal.LibraryModules.React.Component {
            render() {
              return BDFDB.ReactUtils.createElement(
                Internal.LibraryComponents.PopoutContainer,
                BDFDB.ObjectUtils.exclude(
                  Object.assign({}, this.props, {
                    wrap: false,
                    renderPopout: (instance) =>
                      BDFDB.ReactUtils.createElement(Internal.LibraryComponents.UserPopout, {
                        user: Internal.LibraryStores.UserStore.getUser(this.props.userId),
                        currentUser: Internal.LibraryStores.UserStore.getUser(
        ...
        CustomComponents.UserMention =
          reactInitialized &&
          class BDFDB_UserMention extends Internal.LibraryModules.React.Component {
            render() {
              let user =
                this.props.user || Internal.LibraryStores.UserStore.getUser(this.props.userId);
              ...
              let mention = BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                className: this.props.className,
                onContextMenu: (event) =>
                  BDFDB.UserUtils.openMenu(user, guildId, channel.id, event),
                children:
                  "@" +
                  BDFDB.LibraryModules.UserNameUtils.getName(guildId, this.props.channelId, user),
              });
              return this.props.inlinePreview
                ? mention
                : BDFDB.ReactUtils.createElement(
                    Internal.LibraryComponents.UserPopoutContainer,
                    Object.assign({}, this.props, {
        ...
        const VideoInner = function (props) {
          ...
          return props.ignoreMaxSize ||
            (props.naturalWidth <= Internal.DiscordConstants.MAX_VIDEO_WIDTH &&
              props.naturalHeight <= Internal.DiscordConstants.MAX_VIDEO_HEIGHT) ||
            (props.naturalWidth <= Internal.DiscordConstants.MAX_VIDEO_HEIGHT &&
              props.naturalHeight <= Internal.DiscordConstants.MAX_VIDEO_WIDTH)
            ? BDFDB.ReactUtils.createElement(Internal.LibraryComponents.VideoForwardRef, {
                ref: ref,
        ...
        CustomComponents.Video =
          reactInitialized &&
          class BDFDB_Video extends Internal.LibraryModules.React.Component {
            render() {
        ...
        Internal.LibraryComponents = new Proxy(LibraryComponents, {
          get: function (_, item) {
            ...
            if (CustomComponents[item])
              LibraryComponents[item] = LibraryComponents[item]
                ? Object.assign({}, LibraryComponents[item], CustomComponents[item])
                : CustomComponents[item];

            if (
              LibraryComponents[item] &&
              typeof LibraryComponents[item] == "object" &&
              LibraryComponents[item].Z &&
              LibraryComponents[item].Z["$$typeof"]
            )
              LibraryComponents[item] = LibraryComponents[item].Z;
            else if (
              LibraryComponents[item] &&
              typeof LibraryComponents[item] == "object" &&
              LibraryComponents[item].A &&
              LibraryComponents[item].A["$$typeof"]
            )
              LibraryComponents[item] = LibraryComponents[item].A;
            ...
            if (NativeComponent && typeof NativeComponent != "string") {
              for (let key in NativeComponent)
                if (
                  key != "displayName" &&
                  key != "name" &&
                  (typeof NativeComponent[key] != "function" ||
                    key.charAt(0) == key.charAt(0).toUpperCase())
                ) {
                  if (key == "defaultProps")
                    LibraryComponents[item][key] = Object.assign(
                      {},
                      LibraryComponents[item][key],
                      NativeComponent[key],
                    );
                  else if (!LibraryComponents[item][key])
                    LibraryComponents[item][key] = NativeComponent[key];
                }
        ...
        const RealFilteredMenuItems = Object.keys(RealMenuItems).filter(
          (type) =>
            typeof RealMenuItems[type] == "function" &&
            RealMenuItems[type]
              .toString()
              .replace(/[\n\t\r]/g, "")
        ...
        for (let type of RealFilteredMenuItems) {
          let children = BDFDB.ObjectUtils.get(
            BDFDB.ReactUtils.hookCall(
              Internal.LibraryComponents.Menu,
              {
                hideScroller: true,
                children: BDFDB.ReactUtils.createElement(RealMenuItems[type], {}),
              },
              true,
            ),
            "props.children.props.children.props.children",
          );
          let menuItem = (BDFDB.ArrayUtils.is(children) ? children : [])
            .flat(10)
            .filter((n) => n)[0];
          if (menuItem) {
            let menuItemsProps = BDFDB.ReactUtils.findValue(menuItem, "menuItemProps");
            if (menuItemsProps && menuItemsProps.id == "undefined-empty")
              MappedMenuItems.MenuGroup = type;
            else if (menuItemsProps && menuItemsProps.role) {
              switch (menuItemsProps.role) {
                case "menuitemcheckbox":
                  MappedMenuItems.MenuCheckboxItem = type;
                  break;
                case "menuitemradio":
                  MappedMenuItems.MenuRadioItem = type;
                  break;
                case "menuitem": {
                  if (Object.keys(menuItem.props).includes("children"))
                    MappedMenuItems.MenuControlItem = type;
                  else if (Object.keys(menuItem.props).includes("hasSubmenu"))
                    MappedMenuItems.MenuItem = type;
                  break;
                }
              }
            } else {
              let key = BDFDB.ReactUtils.findValue(menuItem, "key");
              if (typeof key == "string" && key.startsWith("separator"))
                MappedMenuItems.MenuSeparator = type;
            }
          }
        }
        LibraryComponents.MenuItems = new Proxy(
          RealFilteredMenuItems.reduce((a, v) => ({ ...a, [v]: v }), {}),
          {
            get: function (_, item) {
              if (CustomComponents.MenuItems[item]) return CustomComponents.MenuItems[item];
              if (RealMenuItems[item]) return RealMenuItems[item];
              if (MappedMenuItems[item] && RealMenuItems[MappedMenuItems[item]])
                return RealMenuItems[MappedMenuItems[item]];
              return null;
            },
          },
        );
        ...
        Internal.modulePatches = {
          before: ["EmojiPickerListRow", "Menu", "MessageHeader", "SearchBar"],
          after: ["DiscordTag", "NameContainerAvatar", "UserPanelHeader", "UserProfileHeader"],
          componentDidMount: ["Account", "AnalyticsContext"],
          componentDidUpdate: ["Account", "AnalyticsContext"],
        };

        const BDFDB_Patrons = Object.assign({}, InternalData.BDFDB_Patrons),
          BDFDB_Patron_Tiers = Object.assign({}, InternalData.BDFDB_Patron_Tiers);
        Internal._processAvatarMount = function (user, avatar, wrapper) {
          if (!user) return;
          if (
            Node.prototype.isPrototypeOf(avatar) &&
            (avatar.className || "").indexOf(BDFDB.disCN.bdfdbbadgeavatar) == -1
          ) {
            let role = "",
              note = "",
              color,
              link,
              addBadge = Internal.settings.general.showSupportBadges;
            if (BDFDB_Patrons[user.id] && BDFDB_Patrons[user.id].active) {
              link = "https://www.patreon.com/MircoWittrien";
              role =
                BDFDB_Patrons[user.id].text ||
                (BDFDB_Patron_Tiers[BDFDB_Patrons[user.id].tier] || {}).text;
              note =
                BDFDB_Patrons[user.id].text &&
                (BDFDB_Patron_Tiers[BDFDB_Patrons[user.id].tier] || {}).text;
              color = BDFDB_Patrons[user.id].color;
              avatar.className = BDFDB.DOMUtils.formatClassName(
                avatar.className,
                addBadge && BDFDB.disCN.bdfdbhasbadge,
                BDFDB.disCN.bdfdbbadgeavatar,
                BDFDB.disCN.bdfdbsupporter,
                BDFDB.disCN[`bdfdbsupporter${BDFDB_Patrons[user.id].tier}`],
              );
            ...
            if (addBadge && role && !avatar.querySelector(BDFDB.dotCN.bdfdbbadge)) {
              ...
              badge.addEventListener("mouseenter", (_) =>
                BDFDB.TooltipUtils.create(badge, role, {
                  position: "top",
                  note: note,
        ...
        Internal._processAvatarRender = function (user, avatar, className = "") {
          if (
            BDFDB.ReactUtils.isValidElement(avatar) &&
            BDFDB.ObjectUtils.is(user) &&
            (avatar.props.className || "").indexOf(BDFDB.disCN.bdfdbbadgeavatar) == -1
          ) {
            let role = "",
              note = "",
              color,
            ...
            if (role) {
              if (avatar.type == "img")
                avatar = BDFDB.ReactUtils.createElement(
                  Internal.LibraryComponents.Avatars.Avatar,
              ...
              let newProps = { className: className, children: [avatar] };
              avatar = BDFDB.ReactUtils.createElement("div", newProps);
              if (addBadge)
                avatar.props.children.push(
                  BDFDB.ReactUtils.createElement(Internal.LibraryComponents.TooltipContainer, {
                    text: role,
                    note: note,
        ...
        Internal.processAccount = function (e) {
          Internal._processAvatarMount(
            e.instance.props.currentUser,
            e.node.querySelector(BDFDB.dotCN.avatarwrapper),
            e.node,
          );
        };
        Internal.processAnalyticsContext = function (e) {
          if (
            e.instance.props.section != Internal.DiscordConstants.AnalyticsSections.PROFILE_MODAL &&
            e.instance.props.section != Internal.DiscordConstants.AnalyticsSections.PROFILE_POPOUT
          )
            return;
          const user = BDFDB.ReactUtils.findValue(e.instance, "user");
          if (!user) return;
          const avatar =
            e.instance.props.section !=
              Internal.DiscordConstants.AnalyticsSections.PROFILE_POPOUT &&
            e.node.querySelector(BDFDB.dotCN.avatarwrapper);
          const wrapper =
            e.node.querySelector(BDFDB.dotCNC.userpopoutouter + BDFDB.dotCN.userprofilemodal) ||
            e.node;
          if (avatar) Internal._processAvatarMount(user, avatar, wrapper);
        };
        Internal.processDiscordTag = function (e) {
          if (e.instance && e.instance.props && e.returnvalue && e.instance.props.user)
            e.returnvalue.props.user = e.instance.props.user;
        };
        Internal.processEmojiPickerListRow = function (e) {
          if (
            e.instance.props.emojiDescriptors &&
            Internal.LibraryComponents.EmojiPickerButton.current &&
            Internal.LibraryComponents.EmojiPickerButton.current.props &&
            Internal.LibraryComponents.EmojiPickerButton.current.props.allowManagedEmojisUsage
          )
            for (let i in e.instance.props.emojiDescriptors)
              e.instance.props.emojiDescriptors[i] = Object.assign(
                {},
                e.instance.props.emojiDescriptors[i],
                { isDisabled: false },
              );
        };
        Internal.processNameContainerAvatar = function (e) {
          if (
            e.returnvalue &&
            Array.isArray(e.returnvalue.props.children) &&
            e.returnvalue.props.children[0]
          )
            e.returnvalue.props.children[0] =
              Internal._processAvatarRender(
        ...
        Internal.processMenu = function (e) {
          if (
            ...
            (e.instance.props.children ||
              (BDFDB.ArrayUtils.is(e.instance.props.children) && e.instance.props.children.length))
          ) {
            let patchCancel = BDFDB.PatchUtils.patch(
              BDFDB,
              Internal.LibraryModules.ContextMenuUtils,
              "closeContextMenu",
              { instead: (e) => {} },
            );
          ...
          if (
            e.instance.props &&
            e.instance.props.navId == "message-actions" &&
            !e.instance.props.BDFDBpatched
          ) {
            e.instance.props.BDFDBpatched = true;
            let copyChild = BDFDB.ReactUtils.findChild(e.instance, {
              filter: (n) => n && n.props && n.props.id == "copy-link",
            });
            if (!copyChild) return;
            let link = "";
            BDFDB.PatchUtils.patch(
              BDFDB,
              BDFDB.LibraryModules.ClipboardUtils,
              "copy",
              {
                instead: (e) => {
                  link = e.methodArguments[0];
                },
              },
              { once: true },
            );
        ...
        Internal.processMessageHeader = function (e) {
          if (e.instance.props.message && e.instance.props.message.author) {
            if (e.instance.props.avatar && e.instance.props.avatar.props) {
              let child = BDFDB.ReactUtils.findChild(e.instance.props.avatar, {
                filter: (n) => n && n.props && typeof n.props.children == "function",
              });
              ...
              if (renderChildren)
                child.props.children = BDFDB.TimeUtils.suppress((...args) => {
                  ...
                  return (
                    Internal._processAvatarRender(
                      e.instance.props.message.author,
                      renderedChildren,
                      BDFDB.disCN.messageavatar,
                    ) || renderedChildren
                  );
        ...
        Internal.processUserPanelHeader = function (e) {
          ...
          let [children, index] = BDFDB.ReactUtils.findParent(e.returnvalue, {
            filter: (n) => n && n.props && n.props.src && n.props.size,
          });
          if (index > -1)
            children[index] =
              Internal._processAvatarRender(e.instance.props.user, children[index]) ||
              children[index];
        };
        ...
        MyReact.instanceKey = Object.keys(document.querySelector(BDFDB.dotCN.app) || {}).some((n) =>
        ...
        const createElementPatches = {
          before: (e) => {
            ...
            if (typeof e.methodArguments[0] == "function") renderFunction = e.methodArguments[0];
              ...
              for (const path of possibleRenderPaths) {
            ...
            if (PluginStores.modulePatches.before)
              for (const type in PluginStores.modulePatches.before)
                if (Internal.isCorrectModule(renderFunction, type, true)) {
                  let hasArgumentChildren = false,
                    children = [...e.methodArguments].slice(2);
                  if (children.length && e.methodArguments[1].children === undefined) {
                    hasArgumentChildren = true;
                    e.methodArguments[1].children = children;
                  }
                  for (let plugin of PluginStores.modulePatches.before[type].flat(10))
                    Internal.initiatePatch(plugin, type, {
                      arguments: e.methodArguments,
                  ...
                  if (hasArgumentChildren) {
                    ...
                    for (let child of e.methodArguments[1].children)
                      [].push.call(e.methodArguments, child);
            ...
            if (PluginStores.modulePatches.after) {
              let patchFunction = "",
                parentModule = e.methodArguments[0];
              if (parentModule.prototype && typeof parentModule.prototype.render == "function")
                ((parentModule = parentModule.prototype), (patchFunction = "render"));
              ...
              else if (parentModule.type && typeof parentModule.type.render == "function")
                ((parentModule = parentModule.type), (patchFunction = "render"));
              if (patchFunction)
                for (const type in PluginStores.modulePatches.after)
                  if (Internal.isCorrectModule(renderFunction, type, true)) {
                    for (let plugin of PluginStores.modulePatches.after[type].flat(10))
                      if (!BDFDB.PatchUtils.isPatched(plugin, parentModule, patchFunction)) {
                        BDFDB.PatchUtils.patch(
                          plugin,
                          parentModule,
                          patchFunction,
            ...
            if (e.methodArguments[0].prototype)
              for (let patchType of [
                "componentDidMount",
                "componentDidUpdate",
                "componentWillUnmount",
              ]) {
                if (PluginStores.modulePatches[patchType])
                  for (const type in PluginStores.modulePatches[patchType])
                    if (Internal.isCorrectModule(renderFunction, type, true)) {
                      for (let plugin of PluginStores.modulePatches[patchType][type].flat(10))
                        if (
                          !BDFDB.PatchUtils.isPatched(
                            plugin,
                            e.methodArguments[0].prototype,
                            patchType,
                          )
                        ) {
                          BDFDB.PatchUtils.patch(
          ...
          after: (e) => {
            if (
              !e.methodArguments[0] ||
              typeof e.methodArguments[0] != "function" ||
              (e.methodArguments[0].prototype &&
                typeof e.methodArguments[0].prototype.render == "function") ||
              !PluginStores.modulePatches.after
            )
              return;
            else
              for (const type in PluginStores.modulePatches.after)
                if (
                  Internal.isCorrectModule(e.methodArguments[0], type, true) &&
                  !Internal.isCorrectModuleButDontPatch(type)
                ) {
                  for (let plugin of PluginStores.modulePatches.after[type].flat(10))
                    BDFDB.PatchUtils.patch(
                      plugin,
        ...
        if (Internal.LibraryModules.InternalReactUtils)
          for (let key in Internal.LibraryModules.InternalReactUtils)
            if (
              typeof Internal.LibraryModules.InternalReactUtils[key] == "function" &&
              Internal.LibraryModules.InternalReactUtils[key]
                .toString()
                .indexOf("return{$$typeof:") > -1
            )
              BDFDB.PatchUtils.patch(
                BDFDB,
                Internal.LibraryModules.InternalReactUtils,
                key,
                createElementPatches,
                { ignoreErrors: true },
              );
        ...
        BDFDB.PatchUtils.patch(
          BDFDB,
          Internal.LibraryModules.AppearanceSettingsUtils,
          "updateLocale",
          {
            after: (e) => {
              BDFDB.TimeUtils.clear(languageChangeTimeout);
              languageChangeTimeout = BDFDB.TimeUtils.timeout((_) => {
                for (let pluginName in PluginStores.loaded)
                  if (PluginStores.loaded[pluginName].started)
                    BDFDB.PluginUtils.translate(PluginStores.loaded[pluginName]);
              }, 10000);
            },
          },
        );
        ...
        BDFDB.PatchUtils.patch(
          BDFDB,
          Internal.LibraryModules.EmojiStateUtils,
        ...
        const pluginQueue =
          window.BDFDB_Global && BDFDB.ArrayUtils.is(window.BDFDB_Global.pluginQueue)
            ? window.BDFDB_Global.pluginQueue
            : [];

        if (
          BDFDB.UserUtils.me.id == InternalData.myId ||
          BDFDB.UserUtils.me.id == "350635509275557888"
        ) {
          BDFDB.DevUtils = {};
            ...
            window.t = {
              $filter: (prop) =>
                [...strings]
                  .flat(10)
                  .filter((n) => typeof n == "string")
                  .every((string) => prop.toLowerCase().indexOf(string.toLowerCase()) > -1),
            };
            for (let i in BDFDB.DevUtils.req.c)
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                let m = BDFDB.DevUtils.req.c[i].exports;
                if (m && typeof m == "object")
                  for (let j in m) if (window.t.$filter(j)) window.t[j + "_" + i] = m;
                if (m && typeof m == "object" && typeof m.default == "object")
                  for (let j in m.default)
                    if (window.t.$filter(j)) window.t[j + "_default_" + i] = m.default;
              }
            console.clear();
          ...
          BDFDB.DevUtils.findPropFunc = function (...strings) {
            ...
            for (let i in BDFDB.DevUtils.req.c)
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                ...
                if (m && typeof m == "object")
                  for (let j in m)
                    if (window.t.$filter(j) && typeof m[j] != "string") window.t[j + "_" + i] = m;
                if (m && typeof m == "object" && typeof m.default == "object")
                  for (let j in m.default)
                    if (window.t.$filter(j) && typeof m.default[j] != "string")
                      window.t[j + "_default_" + i] = m.default;
              }
            console.clear();
            console.log(window.t);
          };
              ...
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                ...
                if (m && typeof m == "object" && typeof m.default == "object")
                  for (let j in m.default)
                    if (
                      window.t.$filter(j) &&
                      typeof m.default[j] == "string" &&
                      /^[A-z0-9]+\-[A-z0-9_-]{6}$/.test(m.default[j])
                    )
                      window.t[j + "_default_" + i] = m.default;
              }
            console.clear();
                  ...
                  .some(
                    (string) =>
                      (typeof m.displayName == "string" &&
                        m.displayName.toLowerCase().indexOf(string.toLowerCase()) > -1) ||
                      (m.name == "string" &&
                        m.name.toLowerCase().indexOf(string.toLowerCase()) > -1),
                  ),
            };
            for (let i in BDFDB.DevUtils.req.c)
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                let m = BDFDB.DevUtils.req.c[i].exports;
                if (m && (typeof m == "object" || typeof m == "function") && window.t.$filter(m))
                  window.t[(m.displayName || m.name) + "_" + i] = m;
                if (
                  m &&
                  (typeof m == "object" || typeof m == "function") &&
                  m.default &&
                  (typeof m.default == "object" || typeof m.default == "function") &&
                  window.t.$filter(m.default)
                )
                  window.t[(m.default.displayName || m.default.name) + "_" + i] = m.default;
              }
            console.clear();
            console.log(window.t);
          };
          BDFDB.DevUtils.findCodeAny = function (...strings) {
            window.t = {
              $filter: (m) => Internal.checkModuleStrings(m, strings, { ignoreCase: true }),
            };
            for (let i in BDFDB.DevUtils.req.c)
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                let m = BDFDB.DevUtils.req.c[i].exports;
                if (m && typeof m == "function" && window.t.$filter(m))
                  window.t["module_" + i] = { string: m.toString(), func: m };
                if (m && m.__esModule) {
                  for (let j in m)
                    if (m[j] && typeof m[j] == "function" && window.t.$filter(m[j]))
                      window.t[j + "_module_" + i] = {
                        string: m[j].toString(),
                        func: m[j],
                        module: m,
                      };
                  if (m.default && (typeof m.default == "object" || typeof m.default == "function"))
                    for (let j in m.default)
                      if (
                        m.default[j] &&
                        typeof m.default[j] == "function" &&
                        window.t.$filter(m.default[j])
                      )
                        window.t[j + "_module_" + i + "_default"] = {
                          string: m.default[j].toString(),
                          func: m.default[j],
                          module: m,
                        };
                }
              }
            for (let i in BDFDB.DevUtils.req.m)
              if (
                typeof BDFDB.DevUtils.req.m[i] == "function" &&
                window.t.$filter(BDFDB.DevUtils.req.m[i])
              )
                window.t["function_" + i] = {
                  string: BDFDB.DevUtils.req.m[i].toString(),
                  func: BDFDB.DevUtils.req.m[i],
                };
            console.clear();
            console.log(window.t);
          };
              ...
              if (BDFDB.DevUtils.req.c.hasOwnProperty(i)) {
                ...
                if (m && typeof m == "object" && !BDFDB.ArrayUtils.is(m) && Object.keys(m).length) {
                  ...
                  for (let j in m) {
                    ...
                    if (typeof m[j] == "string" && /^[A-z0-9]+\-[A-z0-9_-]{6}$/.test(m[j]))
                      stringlib = true;
                  }
                ...
                if (
                  ...
                  !BDFDB.ArrayUtils.is(m.default) &&
                  Object.keys(m.default).length
                ) {
                  var string = true,
                    stringlib = false;
                  for (let j in m.default) {
          ...
          BDFDB.DevUtils.listen = function (strings) {
            ...
            BDFDB.DevUtils.listen.p = BDFDB.PatchUtils.patch(
              "WebpackSearch",
              BDFDB.ModuleUtils.findByProperties(strings),
              strings[0],
              {
                after: (e) => {
                  console.log(e);
          ...
          BDFDB.DevUtils.generateLanguageStrings = function (strings, config = {}) {
            ...
            const languages = BDFDB.ArrayUtils.removeCopies(
              BDFDB.ArrayUtils.is(config.languages)
                ? config.languages
                : ["en"]
                    .concat(
                      BDFDB.ModuleUtils.findByProperties("getLanguages")
                        .getLanguages()
                        .filter((n) => n.enabled)
                        .map((n) => {
                    ...
                    .filter(
                      (n) => n && !n.startsWith("en-") && !n.startsWith("$") && n != language,
                    ),
            ).sort();
            let translations = {};
            ...
            let text = Object.keys(translations[language])
              .map((k) => translations[language][k])
              .join("\n\n");
              ...
              next = (lang) => {
                if (!lang) {
                  let formatTranslation = (l, s, i) => {
                    l = l == "en" ? "default" : l;
                    return (
                      (config.cached && config.cached[l] && config.cached[l][stringKeys[i]]) ||
                      (translations[language][i][0] == translations[language][i][0].toUpperCase()
                        ? BDFDB.StringUtils.upperCaseFirstChar(s)
                        : s)
                    );
                  };
                  let format = config.asObject
                    ? (l, isNotFirst) => {
                  ...
                  let result = Object.keys(translations)
                    .filter((n) => n != "en")
                    .sort()
                    .map((l, i) => format(l, i))
                    .join("");
                  ...
                  requestFunction(
                    `https://translate.googleapis.com/translate_a/single?client=gtx&sl=${language}&tl=${lang}&dt=t&dj=1&source=input&q=${encodeURIComponent(text)}`,
                    (error, response, result) => {
                      if (!error && result && response.statusCode == 200) {
                        try {
                          callback(
                            JSON.parse(result)
                              .sentences.map((n) => n && n.trans)
                              .filter((n) => n)
                              .join(""),
                          );
                        } catch (err) {
                        ...
                        if (response.statusCode == 429) {
                          BDFDB.NotificationUtils.toast("Too many Requests", { type: "danger" });
                        } else {
        ...
        if (libraryCSS)
          BDFDB.DOMUtils.appendLocalStyle(
            "BDFDB",
            libraryCSS
              .replace(/[\n\t\r]/g, "")
              .replace(/\[REPLACE_CLASS_([A-z0-9_]+?)\]/g, (a, b) => BDFDB.dotCN[b]),
          );
    ...
    getSettingsPanel(collapseStates = {}) {
      ...
      let getString = (type, key, property) => {
        return BDFDB.LanguageUtils.LibraryStringsCheck[`settings_${key}_${property}`]
          ? BDFDB.LanguageUtils.LibraryStringsFormat(
              `settings_${key}_${property}`,
              BDFDB.BDUtils.getSettingsProperty("name", BDFDB.BDUtils.settingsIds[key]) ||
                BDFDB.StringUtils.upperCaseFirstChar(key.replace(/([A-Z])/g, " $1")),
            )
          : Internal.defaults[type][key][property];
      };
      return (settingsPanel = BDFDB.PluginUtils.createSettingsPanel(BDFDB, {
        ...
        children: (_) => {
          ...
          for (let key in Internal.settings.choices)
            settingsItems.push(
              BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SettingsSaveItem, {
                type: "Select",
                plugin: Internal,
                keys: ["choices", key],
                ...
                options: Object.keys(
                  Internal.DiscordConstants[Internal.defaults.choices[key].items] || {},
                ).map((p) => ({ value: p, label: BDFDB.LanguageUtils.LibraryStrings[p] || p })),
                searchable: true,
              }),
            );
          for (let key in Internal.settings.general) {
            let nativeSetting =
              BDFDB.BDUtils.settingsIds[key] &&
              BDFDB.BDUtils.getSettings(BDFDB.BDUtils.settingsIds[key]);
            let disabled =
              typeof Internal.defaults.general[key].isDisabled == "function" &&
              Internal.defaults.general[key].isDisabled({
                value: Internal.settings.general[key],
                nativeValue: nativeSetting,
              });
            let hidden =
              typeof Internal.defaults.general[key].isHidden == "function" &&
              Internal.defaults.general[key].isHidden({
                value: Internal.settings.general[key],
                nativeValue: nativeSetting,
              });
            if (!hidden)
              settingsItems.push(
                BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SettingsSaveItem, {
                  ...
                  note:
                    (typeof Internal.defaults.general[key].hasNote == "function"
                      ? Internal.defaults.general[key].hasNote({
                          value: Internal.settings.general[key],
                          nativeValue: nativeSetting,
                          disabled: disabled,
                        })
                      : Internal.defaults.general[key].hasNote) &&
                    getString("general", key, "note"),
                  value:
                    (typeof Internal.defaults.general[key].getValue == "function"
                      ? Internal.defaults.general[key].getValue({
                          value: Internal.settings.general[key],
                          nativeValue: nativeSetting,
                          disabled: disabled,
                        })
                      : true) &&
                    (Internal.settings.general[key] || nativeSetting),
                  onChange:
                    typeof Internal.defaults.general[key].onChange == "function"
                      ? Internal.defaults.general[key].onChange
                      : (_) => {},
                }),
              );
          }
          settingsItems.push(
            BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SettingsItem, {
              type: "Button",
              ...
              labelChildren: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.Clickable, {
                children: BDFDB.ReactUtils.createElement(Internal.LibraryComponents.SvgIcon, {
                  ...
                  height: 20,
                  onClick: (_) =>
                    BDFDB.ModalUtils.open(Internal, {
                      header: "Plugins",
                      subHeader: "",
                      contentClassName: BDFDB.disCN.marginbottom20,
                      text: BDFDB.ObjectUtils.toArray(
                        Object.assign(
                          {},
                          window.PluginUpdates && window.PluginUpdates.plugins,
                          PluginStores.updateData.plugins,
              ...
              onClick: (_) => {
                let toast = BDFDB.NotificationUtils.toast(
                  `${BDFDB.LanguageUtils.LanguageStrings.CHECKING_FOR_UPDATES} - ${BDFDB.LanguageUtils.LibraryStrings.please_wait}`,
                  { type: "info", timeout: 0, ellipsis: true },
                );
                BDFDB.PluginUtils.checkAllUpdates().then((outdated) => {
                  ...
                  if (outdated > 0)
                    ...
                    BDFDB.NotificationUtils.toast(
                      BDFDB.LanguageUtils.LibraryStrings.update_check_complete,
                      { type: "success" },
                    );
                });
              },
