  ...
  return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started)
    ...
    : (([Plugin, BDFDB]) => {
        return class RevealAllSpoilers extends Plugin {
          onLoad() {
            this.modulePatches = { after: ["Spoiler"] };
          }
                ...
                after: (e2) => {
                  ...
                  let parent =
                    BDFDB.DOMUtils.getParent(
                      e2.methodArguments[0].shiftKey
                        ? BDFDB.dotCN.messageswrapper
                        : BDFDB.dotCN.message,
                      e2.methodArguments[0].target,
                    ) || e2.methodArguments[0].target.parentElement;
                  if (parent)
                    for (let spoiler of parent.querySelectorAll(BDFDB.dotCN.spoilerhidden))
                      if (
                        !BDFDB.DOMUtils.getParent(BDFDB.dotCN.messagerepliedmessagepreview, spoiler)
                      )
                        spoiler.click();
                },
              },
