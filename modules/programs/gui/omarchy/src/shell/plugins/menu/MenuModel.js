...
function guardHelpers() {
  // NixOS adaptation: package presence is command presence. The system
  // closure is declarative, so the executable set on PATH is the source of
  // truth; there is no pacman database to query.
  return 'declare -A __omarchy_pkgs=()\n'
    + 'mapfile -t __omarchy_pkg_names < <(compgen -c | sort -u 2>/dev/null)\n'
    + 'for __omarchy_pkg in "${__omarchy_pkg_names[@]}"; do __omarchy_pkgs[$__omarchy_pkg]=1; done\n'
    + '__omarchy_pkg_has() { [[ -n ${__omarchy_pkgs[$1]-} ]] && return 0; return 1; }\n'
